...
We present a fast matrix free method for approximating operators, $\Aop$, that have locally supported non-negative impulse responses. Such operators arise throughout computational science. Examples include Schur complements in Schur complements methods for solving partial differential equations (PDEs) \cite{TOBYPAPER}, Poincare-Steklov operators in domain decomposition methods (e.g., Dirichlet-to-Neumann maps), covariance operators in spatial statistics, blurring operators in imaging, and Hessians in distributed parameter PDE-constrained optimization and inverse problems.
...
\end{equation}
where $\Aker:\Omega \times \Omega \rightarrow \mathbb{R}$ is the integral kernel. Here $\Aop u \in L^2(\Omega)'$ is the linear functional that results from applying $\Aop$ to $u\in L^2(\Omega)$, and $\left(\Aop u\right)(v)$ is the scalar that results from applying that linear functional to $v \in L^2(\Omega)$. 
...
In this paper we form hierarchical matrix approximations of operators $\Aop$ with locally supported, non-negative impulse responses that are accessible matrix-free.
\begin{description}
...
The impulse response $\impulseresponse_x$ may be interpreted as a scaled probability distribution because of the non-negative impulse response property. Let $\spatialvol:\Omega \rightarrow \mathbb{R}$,
\begin{equation*}
\spatialvol(x) := \int_\Omega \impulseresponse_x(y) dy
\end{equation*}
denote the spatially varying scaling factor (the ``volume'' of $\impulseresponse_x$), let 
\begin{equation*}
\widehat{\impulseresponse}_x := \impulseresponse_x / \spatialvol(x)
\end{equation*}
denote the normalized version of $\impulseresponse_x$, and let $\mu:\Omega \rightarrow \mathbb{R}^d$, $\Sigma:\Omega \rightarrow \mathbb{R}^{d \times d},$
\begin{equation*}
\spatialmean(x) := \mathbb{E}(\widehat{\impulseresponse}_x), \qquad
\spatialcov(x) := \Var(\widehat{\impulseresponse}_x),
\end{equation*}
denote the spatially varying mean and covariance of $\widehat{\impulseresponse}_x$, respectively. In Section \ref{eq:mean_and_covariance_estimation} we will see that it is possible to compute $V(x)$, $\mu(x)$, and $\Sigma(x)$ for all $x \in \Omega$ simultaneously, via a procedure that involves applying $\Aop^T$ to a small number of polynomial functions (three functions if $\gdim=1$, six if $\gdim=2$, ten if $\gdim=3$). 

Since a probability distribution concentrates within a small number of standard deviations of its mean, knowing $\mu(x)$ and $\Sigma(x)$ permits us to form an ellipsoid shaped estimate for the support of $\phi_x$, \emph{without} computing $\phi_x$. This allows us to compute large numbers of impulse responses, $\impulseresponse_{x_i}$, in ``batches.'' We compute one batch of $\impulseresponse_{x_i}$'s by applying $\Aop$ to a weighted sum of point sources (Dirac comb) associated with a collection of points $x_i$ scattered throughout $\Omega$. The batch of points $x_i$ are chosen via a greedy ellipsoid packing algorithm so that the support of $\impulseresponse_{x_i}$ and the support of $\impulseresponse_{x_j}$ do not overlap (or do not overlap much) if $i \neq j$. Because these supports do not overlap, we can post-process the response of $\Aop$ to the Dirac comb to recover the functions $\impulseresponse_{x_i}$ associated with all points $x_i$ in the batch---with one application of $\Aop$, we recover many $\impulseresponse_{x_i}$. This is illustrated in Figure FIG. The process is repeated to get more batches, until a desired number of impulse responses is reached.

Once the scattered impulse responses $\impulseresponse_{x_i}$ are computed, we approximate the integral kernel $\Phi(y,x)$ at arbitrary points $(y,x)$ by interpolation. The details of the interpolation will be described in Section SECTION, but the key principle behind the interpolation is an assumption that we call \emph{local mean-displacement invariance}. Specifically, we make the assumption that
\begin{equation}
\Phi(y,x) = \phi_x(y) \approx \phi_{x_i}(y - \mu(x) + \mu(x_i))
\end{equation}
if $x_i$ is close to $x$. This is shown in Figure FIG, and discussed in greater detail in Section SEC. We approximate $\phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for the a small number of sample points $x_i$ near $x$. The weights in the linear combination are determined via radial basis function interpolation.

The ability to rapidly evaluate approximate kernel entries $\Phi(y,x)$ allows us to construct an H-matrix approximation, $\boldsymbol{\Phi}_H$, of $\Phi$, using the conventional adaptive cross H-matrix construction method. In this method, one forms low-rank approximations of off-diagonal blocks of the matrix by sampling rows and columns of those blocks \cite{HACA}. We then use H-matrix methods to convert $\boldsymbol{\Phi}_H$ into an H-matrix approximation of $\Aop$, denoted $\mathbf{A}_H$. Schematically the overall H-matrix approximation proceeds as follows:
\begin{equation*}
...
\end{equation*}
One may use H-matrix methods to perform further useful linear algebra operations involving $\mathbf{A}_H$, including matrix-matrix addition, matrix-matrix multiplication, matrix-vector products, matrix factorization, and matrix inversion. WHY DO THESE OPERATIONS: KRYLOV SOLVERS, SAMPLING STRATEGIES, CAN'T DO WITH A. These H-matrix methods are fast and scale well to large problems.

Often we are interested in approximating operators $\Aop$ that are symmetric, or symmetric positive semi-definite. If $\Aop$ is symmetric, impulse responses of $\Aop$ are also impulse responses of $\Aop^T$, which may be interpolated in the variable $y$ instead of $x$ to yield a different approximation of $\Phi(y,x)$. In this case, we combine information from impulse responses of both $\Aop$ and $\Aop^T$ in the interpolation procedure for approximating $\Phi(y,x)$, to get a more accurate approximation at negligible additional cost. Intuitively, impulse responses of $\Aop$ can be thought of as ``columns'' of $\Phi$, impulse responses of $\Aop^T$ can be thought of as ``rows'' of $\Phi$, and $\Phi(y,x)$ can be thought of as an unknown entry of $\Phi$. When $\Aop$ is symmetric, computing a column of $\Aop$ also gives us a row of $\Aop$ for free, so we approximate $\Phi(y,x)$ using information from a combination of known columns and known rows, rather than just known columns.

When $\Aop$ is symmetric positive semi-definite, $\mathbf{A}_H$ may be indefinite due to errors in the approximation. In this case, we modify the H-matrix representation of $\mathbf{A}_H$ to make it positive semi-definite, by applying a specially chosen rational matrix function to $\mathbf{A}_H$.
...
Let $x$ and $x'$ be points in $\Omega$ that are close to each other, and consider the following three approximations, ordered by increasing level of sophistication:
\begin{align}
...
\end{align}
Approximations \eqref{eq:translate1} and \eqref{eq:translate2} are standard. Approximation \eqref{eq:translate3}, which we use,  is new. We call operators that satisfy \eqref{eq:translate3} ``locally mean-displacement invariant.'' These approximations are illustrated in Figure FIG.

Approximation \eqref{eq:translate1} follows from the assumption that $\Phi$ is smooth. Operators satisfying \eqref{eq:translate1} can be well-approximated via low-rank interpolative factorization of $\Phi$.
% of the form
%\begin{equation}
...
However, even if $\Phi$ is smooth (which is typically the case asymptotically) the relevant Lipschitz constant for continuity, and hence the required rank in the low rank approximation, can be large, which makes algorithms based on assumption \eqref{eq:translate1} computationally expensive. This is the case, for example, for Hessians in PDE constrained inverse problems with highly informative data. As the data become more informative, typically the ``mass'' of $\Phi(y,x)$ concentrates nearer and nearer to the ``diagonal'' $y=x$, the Lipschitz constant increases, and the required rank in the approximation blows up.
...
We use approximation \eqref{eq:translate3}, which is an improvement upon \eqref{eq:translate1} and \eqref{eq:translate2} that generalizes both. On one hand, if \eqref{eq:translate1} holds, then $\mu(x) \approx \mu(x_i)$, and so \eqref{eq:translate3} holds. On the other hand, translating a distribution translates its mean, so if \eqref{eq:translate2} holds, then $\mu(x_i)-\mu(x) \approx x_i - x$, so again \eqref{eq:translate3} holds. Furthermore, approximation \eqref{eq:translate3} can hold in situations where neither \eqref{eq:translate1} nor \eqref{eq:translate2} holds. For example, \eqref{eq:translate3} will hold when an operator is locally translation-invariant with respect to a rotated frame of reference, while \eqref{eq:translate2} will not hold in this case (see Figure FIG). Additionally, \eqref{eq:translate3} generalizes to operators $\Aop:L^2(\Omega_1) \rightarrow L^2(\Omega_2)'$ that map between function spaces on different domains $\Omega_1$ and $\Omega_2$, and even operators that map between domains with different spatial dimensions. In contrast, \eqref{eq:translate2} does not naturally generalize to operators that map between function spaces on different domains, because the formula $y-x+x'$ requires $y \in \Omega_2$ and $-x+x' \in \Omega_1$ to be added together. 
...
\section{Method}
\label{sec:method}
In this section we describe how we compute the impulse response moments $V,\mu,\Sigma$ and use these moments to estimate the supports of the impulse responses $\phi_x$, how we choose the sample points $x_i$ (Section \ref{sec:sample_point_selection}), how we compute the impulse response batches $\phi_{x_i}$ (Section \ref{sec:get_impulse_response}), how we interpolate impulse response to approximate integral kernel entries $\Phi(y,x)$, and how we construct the H-matrix approximation $\mathbf{A}_h$ of the operator $\mathcal{A}$ after the problem has been discretized with finite elements. We also describe how to (optionally) modify $\mathbf{A}_h$ to force it to be symmetric positive semi-definite. The complete algorithm is shown in Algorithm \ref{alg:construct_Atilde}. 
...
We use a greedy ellipsoid packing algorithm to choose as many points as possible per batch, while ensuring that there is no overlap between the support ellipsoids, $E_{x_i}$, associated with the sample points, $x_i$, within a batch.
...
\begin{enumerate}
	...
	\item We make sure that $V(p)$ is not too small, in the sense $V(p) > \tau V_\text{max}$, where $V_\text{max}$ is the largest value of $V(q)$ over all points $q$ in the initial set of candidate points, and $\tau$ is a small threshold parameter (we use $\tau=10^{-5}$).
\end{enumerate}
If $p$ passes both these checks (i.e., if $p$ is sufficiently far from the other points and $V(p)$ is not too small) then we add $p$ to $\pointbatch_k$. Otherwise we discard $p$. This process repeats until there are no more points in $P$.  This is detailed in Algorithm \ref{alg:point_choice}. We determine whether $E_p \cap E_q = \{\}$ using the ellipsoid intersection test described in Appendix \ref{sec:fast_ellipsoid_intersection_test}.

We repeat the process to construct several batches of points $\pointbatch_1, \pointbatch_2, \dots$, until the number of batches exceeds a desired threshold. In our implementation, for each batch the set of candidate points $P$ is initialized as the set of all Lagrange nodes for the finite element basis functions used to discretize the problem.
%, except those points in previously chosen batches.
...
We compute impulse responses $\impulseresponse_{x_i}$ in batches by applying $\Aop$ to a Dirac combs. The Dirac comb, $\diraccomb_k$, associated with a batch of sample points, $\pointbatch_k$, is the following weighted sum of Dirac distributions (point sources) centered at the points $x_i \in \pointbatch_k$:
\begin{equation*}
...
\end{equation}
Each point source $\delta_{x_i}$ is scaled by $1/V(x_i)$ so that the resulting scaled impulse responses within a batch are all comparable in magnitude. The support of an impulse response is estimated to be contained within an ellipsoid, but this is just an approximation. A small portion of the impulse response will reside outside of the ellipsoid. Without this scaling, the neglected portion of an impulse response corresponding to a point $x_i$ could overwhelm the impulse response corresponding to a nearby point $x_j$, when $V(x_i)$ is much larger than $V(x_j)$. We are not in danger of dividing by zero here, because our ellipsoid packing procedure from Section SEC excludes $x_i$ from consideration to be a sample point if $V(x_i)$ is smaller than a predetermined threshold.


By linearity, the impulse response batch $\combresponse_k$ may be written as follows:
\begin{equation}
	\label{eq:phi_b}
...
\label{eq:varphi_eval}
	...
	\begin{cases}
		V(x_i)\combresponse_k(z), & z \in E_{x_i} \\
		0, & \text{otherwise}
	\end{cases}
\end{equation}
for all $x_i \in \pointbatch_k$. By performing one matrix-vector product, \eqref{eq:dirac_comb_H_action}, we recover the impulse responses $\impulseresponse_{x_i}$ associated with every point $x_i \in \pointbatch_k$. 



\subsection{Approximate integral kernel entries}

Here we describe how we compute approximate kernel entries $\Phi(y,x)$ for arbitrary pairs of points $(y,x) \subset \Omega \times \Omega$. To that end, we define the function
\begin{equation}
\label{eq:fxyxp}
f_{y,x}(x') := \frac{V(x)}{V(x')}\phi_{x_i}\left(y - \mu(x) + \mu(x')\right).
\end{equation}
It follows from inspection that 
\begin{equation*}
\Phi(y,x) = f_{y,x}(x),
\end{equation*}
and from the discussion in Section SEC, we expect that $\Phi(y,x) \approx f_{y,x}(x')$ when $x'$ is close to $x$. The factor $\frac{V(x)}{V(x_i)}$, which was not discussed in Section SEC, is a form of preconditioning that improves the quality of this approximation in situations where $V(x)$ varies rapidly. 
...
We approximate $\Phi(y,x)$ by performing radial basis function interpolation of the function $f_{y,x}(x')$ in the variable $x'$. The data used in the interpolation are the following (point,value) pairs
\begin{equation}
\left(x_i, f_{y,x}(x_i)\right), \qquad i=1,\dots,k,
\end{equation}
and $(x, f_{y,x}(x))$ is the (point,value) pair we approximate via interpolation.
...
In numerical experiments we use $k=10$. The $k$ nearest sample points are found by querying a precomputed KD-tree [CITE] of all sample points. 

From the definition of $f_{y,x}$ in \eqref{eq:fxyxp}, we see that $f_{y,x}(x_i)$ is only defined if $y - \mu(x) + \mu(x_i) \in \Omega$, because $\phi_x$ is only defined on $\Omega$. But it may happen that $y - \mu(x) + \mu(x_i) \notin \Omega$, even if $y,x,x_i \in \Omega$. In this case, the point $x_i$ is considered invalid, and not used in the interpolation, even if it is close to $x$. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) [CITE] of the mesh cells used to discretize the problem.

We approximate $\Phi(y,x)$ using radial basis function interpolation of the following form:
\begin{equation}
\Phi(y,x) = f_{y,x}(x) \approx \sum_{i=1}^k w_i~ b\left(\|x-x_i\|\right),
\end{equation}
...
Bw = f,
\end{equation}
where the coefficient matrix $B \in \mathbb{R}^{k \times k}$ has entries 
\begin{equation*}
B_{ij} = b\left(\|x_i - x_j\|\right),
\end{equation*}
and the right hand side vector $f \in \mathbb{R}^k$ has entries $f_i = f_{y,x}(x_i)$, $i=1,\dots,k$. We use polyharmonic spline radial basis functions [CITE], which are defined as follows:
\begin{equation}
b_i(r) := \begin{cases}
r^d, & d=1,3,5,\dots \\
...
We evaluate $f_{y,x}(x_i)$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
\label{eq:ellipsoid_check93}
(y - \mu(x))^T \Sigma^{-1}(x) (y - \mu(x))  \le \tau^2.
\end{equation}
If \eqref{eq:ellipsoid_check93} does not hold, then $y - \mu(x) + \mu(x_i) \notin E_{x_i}$ (i.e., $y - \mu(x) + \mu(x_i)$ is outside the estimated support of $\impulseresponse_{x_i}$), so we set $f_{y,x}(x_i)=0$ for the purpose of interpolation. If \eqref{eq:ellipsoid_check93} does hold, then $y - \mu(x) + \mu(x_i) \in E_{x_i}$. In this case, we look up the batch index $b$ for the batch of sample points that contains $x_i$, and evaluate
\begin{equation}
f_{y,x}(x_i) = \frac{V(x)}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right) = V(x)\eta_b\left(y - \mu(x) + \mu(x_i)\right),
\end{equation}
where $\eta_b$ is the impulse response batch function that we have computed and stored.
Evaluating $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$ requires some care, because the point $y - \mu(x) + \mu(x_i)$ is typically not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints.
To evaluate $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$, we determine which mesh cell contains $y - \mu(x) + \mu(x_i)$, then we evaluate the relevant finite element basis functions on that mesh cell. Fortunately, the mesh cell containing $y - \mu(x) + \mu(x_i)$ was already determined as a side effect of querying the AABB tree of mesh cells when we checked whether $y - \mu(x) + \mu(x_i) \in \Omega$. 

\subsubsection{Improvements based on symmetry}
If $\Aop$ is symmetric, impulse responses of $\Aop$ are also impulse responses of $\Aop^T$. Let $\{y_i\}_{i=1}^k$ be the $k$ nearest sample points to $y$, excluding points $y_i$ such that $x - \mu(y) + \mu(y_i) \notin \Omega$. We could form an alternative approximation of $\Phi(y,x)$ by interpolating the (point, value) pairs
\begin{equation}
\label{eq:phi_mu_shift_y}
\left(y_i,f_{x,y}(y_i)\right), \qquad i=1,\dots,k,
\end{equation}
in the same manner as before, but with $x$ and $y$ now taking opposite roles. We improve the interpolation accuracy at negligible additional cost by combining both of these approximations. Specifically, we combine both approximations by placing function values 
\begin{equation*}
\left(\widetilde{f}_x\right)_i := \frac{V(x)}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right), \qquad i=1,\dots,k,
\end{equation*}
at the points $x_i-x$, placing function values
\begin{equation*}
\left(\widetilde{f}_y\right)_i := \frac{V(y)}{V(y_i)}\phi_{y_i}\left(x - \mu(y) + \mu(y_i)\right), \qquad i=1,\dots,k,
\end{equation*}
at the points $y_i-y$, and interpolating the combination of all $2k$ of these points and values at zero.

In detail, define 
\begin{equation*}
\widetilde{x_i} := x_i - x \qquad \text{and} \qquad \widetilde{y_i} := y_i - y.
\end{equation*}
and let
\begin{equation*}
\widetilde{B} = \begin{bmatrix}
\widetilde{B}_{xx} & \widetilde{B}_{xy} \\
\widetilde{B}_{yx} & \widetilde{B}_{xx}
\end{bmatrix}, \qquad \widetilde{w} = \begin{bmatrix}\widetilde{w}_x \\ \widetilde{w}_y\end{bmatrix}, \qquad \widetilde{f} = \begin{bmatrix}
\widetilde{f}_x \\ \widetilde{f}_y
\end{bmatrix},
\end{equation*}
be a $2k \times 2k$ block matrix and $2k \times 1$ block vectors, respectively, with the block entries of $\widetilde{B}$ be given by
\begin{align*}
\left(\widetilde{B}_{xx}\right)_{ij} = b\left(\|\widetilde{x}_i - \widetilde{x}_j\|\right), &\qquad
\left(\widetilde{B}_{xy}\right)_{ij} = b\left(\|\widetilde{x}_i - \widetilde{y}_j\|\right), \\
\left(\widetilde{B}_{yx}\right)_{ij} = b\left(\|\widetilde{y}_i - \widetilde{x}_j\|\right), &\qquad \left(\widetilde{B}_{yy}\right)_{ij} = b\left(\|\widetilde{y}_i - \widetilde{y}_j\|\right),
\end{align*}
for $i,j=1,\dots,k$. We approximate $\Phi(y,x)$ as follows:
\begin{equation}
\Phi(y,x) \approx \sum_{i=1}^k \left(w_x\right)_i~ b\left(\|\widetilde{x}_i\|\right) + \sum_{i=1}^k \left(w_y\right)_i~ b\left(\|\widetilde{y}_i\|\right),
\end{equation}
where $\widetilde{w}$ is the solution of the following regularized least-squares problem,
\begin{equation*}
\min_{\widetilde{w}} \frac{1}{2}\|\widetilde{B} \widetilde{w} - \widetilde{f}\|^2 + \frac{\alpha}{2}\|\widetilde{w}\|^2,
\end{equation*}
for a small regularization parameter $\alpha$. 
We solve the regularized least squares problem by QR factorization of the block $4k \times 2k$ matrix $\begin{bmatrix}\widetilde{B} & I\end{bmatrix}^T$. We regularize the problem because of numerical issues that arise if there are points $\widetilde{x}_i$ and $\widetilde{y}_j$ in the interpolation that are very close to each other, a situation that arises often in practice. Indeed, situations where $\widetilde{x}_i = \widetilde{y}_j$ are inevitable when a regular grid is used to discretize the problem. In this case, $\widetilde{B}$ is (nearly) singular, so it is numerically ill-advised to solve the standard interpolation equation $\widetilde{B} \widetilde{w} = \widetilde{f}$. 

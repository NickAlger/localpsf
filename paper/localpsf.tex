...
%	The method may be used to approximate other operators, including certain Schur complements, Poincare-Steklov operators, covariance operators, and blurring operators. 
	The method computes impulse responses of the operator at a collection of scattered points, and interpolates translated and scaled versions of these impulse responses to approximate entries of the integral kernel. We compute a large number of impulse responses by applying the operator to a small number of Dirac combs associated with ``batches'' of point sources. By solving an ellipsoid packing problem, we compute as many impulse responses as possible per batch, while ensuring that the supports of the impulse responses within each batch do not overlap. Support ellipsoids are estimated a-priori based on impulse response moments, which are computed by applying the operator to a small number of polynomial functions. The interpolation uses a ``local mean displacement invariance'' approximation, which generalizes classical local translation invariance. The ability to rapidly evaluate kernel entries allows us to construct a hierarchical matrix (H-matrix) approximation of the operator. Further matrix computations are performed with H-matrix methods. We apply the method to approximate the Hessians in an ice sheet flow PDE constrained inverse problem, and an advection-diffusion PDE constrained inverse problem. Numerical results demonstrate that the proposed method outperforms existing methods, and can approximate high rank operators using only a small number of operator applies.
\end{abstract}
...
but cannot easily access entries of $\Aop$'s integral kernel. Evaluating the maps in~\eqref{eq:eval_maps} may require, as a sub procedure, solving PDEs or other costly computations. By ``non-negative integral kernel,'' we mean that entries of the integral kernel are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Batches, $\eta_b$, of normalized impulses, $\impulseresponse_{x}$, for the ice sheet inverse problem data misfit Gauss-Newton Hessian described in Section~\ref{sec:numerical_results}. 
...
We form a hierarchical matrix (H-matrix)~\cite{BormGrasedyckHackbusch03,Hackbusch99} approximation of $\Aop$\todo{here a reviewer might complain that $\Aop$ is an infinite dimensional operator and H-matrix methods are only defined for finite dimensional operators}. H-matrices are a matrix format in which the rows and columns of the matrix are re-ordered, then the matrix is recursively subdivided into blocks, in such a way that many off-diagonal blocks are low rank, even though the matrix as a whole may be high rank. 
...
Hessian approximations and preconditioners are highly desirable because the Hessian is central to efficient solution of inverse problems in both the deterministic and Bayesian settings. When solving the deterministic optimization problem with Newton-type methods, the Hessian is the coefficient operator for the linear system that must be solved or approximately solved at every Newton iteration. Good Hessian preconditioners reduce the number of iterations required to solve these Newton linear systems with Krylov methods such as the conjugate gradient method~\cite{Saad03}. In the Bayesian setting, the inverse of the Hessian is the covariance of a local Gaussian approximation of the posterior. This Gaussian distribution can be used directly as an approximation of the posterior, or it can be utilized as a proposal for Markov chain Monte-Carlo methods for drawing samples from the posterior. For instance, see~\cite{KimEtAl21,PetraEtAl14} and the references therein. 
...
The integral kernel, $\Aker:\Omega \times \Omega \rightarrow \mathbb{R}$, exists in principle but is not easily accessible. In this section we describe how to extend the action of $\Aop$ to distributions, which allows us to define impulse responses (Section~\ref{sec:impulse_response_2}), we then state the conditions on $\Aop$ that our method requires (Section~\ref{sec:conditions_2}), and detail finite element discretization (Section~\ref{sec:finite_element_kernel}).
...
\paragraph{Lumped mass matrix} At the continuum level, $\Aker$ is assumed to be non-negative. However, entries of $\Aker_h$, involve inverse mass matrices, which typically contain negative numbers. If there are too many negative numbers, or if the negative numbers are too large, our algorithm will yield less accurate approximations and may potentially fail. We therefore recommend replacing the mass matrix, $\massmatrix$, with a positive diagonal \emph{lumped mass} approximation. 
In our numerical results, we use the lumped mass matrix in which the $i$th diagonal entry of the lumped mass matrix is the sum of all entries in the $i$th row of the mass matrix. Other mass lumping techniques may be used~\cite{Hughes12}.
...
We form our H-matrix approximation $\mathbf{A}_H \approx \mathbf{A}$ by forming an H-matrix representation $\mathbf{\Aker}_H$ of $\mathbf{\Aker}$ then multiplying $\mathbf{\Aker}$ with mass matrices $\massmatrix$ per~\eqref{eq:boldA} to form
$
\mathbf{A}_H = \massmatrix \mathbf{\Aker}_H \massmatrix.
$
In our case $\massmatrix$ is a lumped mass matrix and is therefore diagonal, so these matrix-matrix multiplications are trivial. If a non-diagonal mass matrix is used, one may form a H-matrix representation of the mass matrix, then perform the matrix-matrix multiplications in \eqref{eq:boldA} using H-matrix methods.
...
The dense matrix $\mathbf{\Aker}$ is never formed. 
...
We modify $\mathbf{A}_H$ to make it symmetric and remove negative eigenvalues. First, we symmetrize $\mathbf{A}_H$ via
$
...
\paragraph{(3) Performing linear algebra operations with the H-matrix} It is well known that H-matrix methods for matrix-vector products, matrix-matrix addition, matrix-matrix multiplication, matrix factorization, matrix inversion, and low rank updates require
\begin{equation}
...
%Constructing an H-matrix representation of the regularization term in the Hessian requires one matrix inversion and two matrix-matrix multiplications. 
%Then, we use one matrix factorization to form a preconditioner for the Newton linear system. 
...
Symmetrizing $\mathbf{A}_H$ requires one matrix-matrix addition. Flipping negative eigenvalues to be positive requires a handful (typically around 5) matrix-matrix additions and matrix factorizations to factor the required shifted linear systems, and a number of factorized solves with these shifted linear systems that is proportional to the number of erroneous negative eigenvalues.
...
We use our method to approximate the Newton (or Gauss-Newton) Hessians
...
optimization problems, whose cost functions consist of a data misfit term
...
regularization parameter (which controls the overall strength of the regularization) using Morozov's discrepancy
principle~\cite{Vogel02}. 
%The optimal value of $\gamma$ is used for
...
$\|\bm{g}_k\|/\|\bm{g}_0\| < 10^{-6}$. Systems~\eqref{eq:newton_system} are solved inexactly using an inner PCG
iteration, which terminates when the norm of the linear system residual is less than $\min \left(0.5,
\sqrt{\|\bm{g}_k\|}\right)\|\bm{g}_k\|$. The advection-diffusion inverse problem is linear, so Newton's method converges in one iteration. In this case the Newton linear system,~\eqref{eq:newton_system}, is solved using PCG, using termination tolerances described in Section \ref{sec:adv}.
...
are formed by adding $\mathbf{A}_H$ to $\mathbf{R}_H$ using fast H-matrix arithmetic. We modify $\widetilde{\bm{H}}$ to be symmetric positive definite via the procedure described in Section~\ref{sec:flip_eigs}. We factor $\bm{\preconditioner}$,
using fast H-matrix methods, then use the factorization as a
preconditioner. We approximate $\bm{H}_\text{gn}$ rather than $\bm{H}$ because $\bm{H}$ more often has negative values in its integral kernel. Our results show that~$\widetilde{\bm{H}}$ is a good preconditioner for both~$\bm{H}_\text{gn}$ and~$\bm{H}$. 
...
restricted to the top surface of the geometry. We use $5\%$ noise in all cases, except for Figure \ref{fig:stokes_reconstructions} and Table \ref{tab:condition_number} where the noise is varied from $1\%$ to $25\%$ and the regularization determined by the Morozov discrepancy principle for each noise level. The true basal sliding
...
%	\end{subfigure}
	...
	%		\label{fig:newtoniter4}
    \end{subfigure}
	\caption{The log basal sliding parameter, with colorscale as in Figure~\ref{fig:true_beta}, computed from the PDE-constrained optimization problem with noise levels: $25\%$ (left), $5.0\%$ (middle), and $1.0\%$ (right).}
...
\begin{table}
	...
	\caption{Convergence history for solving the Stokes inverse problem using inexact Newton PCG. The Newton iteration is terminated when $||\mathbf{g}_k|| < 10^{-6} ||\mathbf{g}_0||$, where $\mathbf{g}_i$ is the gradient at the $i$th iteration. Preconditioners shown are our method with five batches (PSF (5)) constructed at the third iteration, regularization preconditioning (REG), and no preconditioning (NONE). Columns titled \#CG show the number of PCG iterations used to solve the Newton system for $\bm{\widehat{\basalfriction}}_k$. Columns titled $\|\bm{g}\|$ show the $l^2$ norm of the gradient at $\bm{\basalfriction}_k$. Columns titled \#Stokes show the total number of Stokes PDE solves performed in each Newton iteration. This consists of Stokes solves for $u$ performed during the line search going from $\bm{\basalfriction}_{k-1}$ to $\bm{\basalfriction}_k$, plus one adjoint Stokes solve to compute the gradient at $\bm{\basalfriction}_k$, plus one incremental forward and one incremental adjoint Stokes solve per PCG iteration for solving the Newton system. Under PSF (5) and in row Iter 3, we write $6+22$ to indicate that $6$ Stokes solves were used during the standard course of the iteration, and $22$ Stokes solves were used to build the PSF (5) preconditioner.}
	\label{tab:newton_convergence_table}
\end{table}

Table~\ref{tab:newton_convergence_table} shows the performance of our preconditioner for accelerating the solution of the optimization problem to reconstruct $\mathbf{q}$ from observations with $5\%$ noise. We build the PSF (5) preconditioner in the third Gauss-Newton iteration, and reuse it for all subsequent Gauss-Newton and Newton iterations. No preconditioning is used in the Gauss-Newton iterations before the PSF (5) preconditioner is built. We compare our method with the most commonly used existing preconditioners: no preconditioning (NONE), and preconditioning by the regularization term in the Hessian (REG). Using PSF (5) reduces the total number of Stokes PDE solves substantially compared to regularization preconditioning and no preconditioning\footnote{Interestingly, we observe that no preconditioning outperforms regularization preconditioning.}. In Figure~\ref{fig:stokes_reconstructions} we show reconstructions for $1\%$, $5\%$, and $25\%$ noise.
%
%
\begin{figure}
    ...
    \end{subfigure}
	\caption{Left (\ref{fig:cg_convergence}) shows the convergence history for solving $\bm{H} \bm{x} = \bm{b}$ using PCG, where $\bm{b}$ has i.i.d. random entries drawn from the standard Gaussian distribution and $\bm{H}$ is evaluated at the solution of the inverse problem. 
%	The reference solution $\bm{x}$ is determined by solving $\bm{H} \bm{x} = \bm{b}$. 
%	Here we use $\gamma=7.3 \times 10^3$, which is determined by the Morozov discrepancy principle when using $5\%$ noise.
	Results in these figures are shown for our preconditioner with 1, 5, and 25 batches (PSF (1), PSF (5), and PSF(25), respectively), regularization preconditioning (REG), and no preconditioning (NONE). The preconditioner is constructed using $\bm{H}_\text{gn}$.
...
$ 
The matrix $\bm{\preconditioner}$ is one of the PSF (1), PSF (5), or PSF (25) Gauss-Newton Hessian approximations, the regularization Hessian (REG), or the identity matrix (NONE). The PSF preconditioners cluster the eigenvalues of the Hessian near one, with more batches yielding better clustering. 
%The regularization preconditioner clusters the trailing eigenvalues but amplifies leading eigenvalues. 
...
In Table~\ref{tab:condition_number}, we show the condition number of the preconditioned Hessian using our preconditioners, for a variety of noise levels ranging from $1\%$ to $25\%$. The condition number using our PSF preconditioners is extremely small (ranging between 1 and 10) and relatively stable over this range of noise levels. PSF (25) performs better than PSF (5), which performs better than PSF (1), as expected. All PSF preconditioners outperform no preconditioning and regularization preconditioning for all noise levels. No preconditioning outperforms regularization preconditioning for all noise levels except $25\%$. 
%We report condition number results for the preconditioned Hessian, $\mathbf{H}$, demonstrating that the PSF preconditioners built based on the Gauss-Newton Hessian, $\bm{H}_\text{gn}$, are also good preconditioner for $\bm{H}$.
%As expected, regularization preconditioning performs well for large $\gamma$ and poorly for small $\gamma$. Our PSF preconditioners outperform no preconditioning and regularization preconditioning for medium and small $\gamma$, and perform similarly to regularization preconditioning for large~$\gamma$.
...
Reynolds number 50, with boundary conditions $v(x)=(0,1)$ on the left boundary, $v(x)=(0,-1)$ on the right boundary, and $v(x)=(0,0)$ on the top and bottom boundaries, as in~\cite[Section 3]{PetraStadler11}. We use a checkerboard image for the initial condition~\ref{fig:adv_initial_condition} and add $5\%$ multiplicative noise to the synthetic observations at the final time, $T$. We use $\kappa=3.2$e${-1}$ and $T=1.0$ for all results, except for Table \ref{tab:adv_convergence} and Figure \ref{tbl:adv_vary_kappa_tf} where we vary $\kappa$ and $T$ to study the impact of these parameters on the impulse responses and our preconditioner's effectiveness. The initial condition, velocity field, noisy observations, and reconstructed initial condition are shown in Figure~\ref{fig:adv_inversion1}.

In Table~\ref{tab:adv_convergence} we show the number of PCG iterations, $k$, required to solve the Newton linear system to a relative error tolerance of $\|\widehat{\mathbf{q}} - \widehat{\mathbf{q}}_k\| < 10^{-6} \|\widehat{\mathbf{q}}\|$. The solution of the Newton system that we compare to, $\widehat{\mathbf{q}}$, is found via another PCG iteration with a relative residual tolerance of $10^{-11}$. We show results for $T$ ranging from $0.5$ to $2.0$ and $\kappa$ ranging from $10^{-4}$ to $10^{-3}$, using our PSF preconditioner with $1$, $5$, and $25$ batches, regularization preconditioning, and no preconditioning. PSF preconditioning performs better than regularization preconditioning and no preconditioning, with more batches yielding better results. The PSF preconditioners perform best in the high-rank regime where $T$ and $\kappa$ are small, and the observations are highly informative about the initial condition. This makes sense given the effect of $T$ and $\kappa$ on the shape of the impulse responses, as shown in Figure \ref{tbl:adv_vary_kappa_tf}. As $\kappa$ increases, the impulse responses grow wider and as $T$ increases the impulse responses grow longer. This means that the impulse responses become more local as $\kappa$ and $T$ decrease, so more impulse responses can be computed per batch.

In Figure~\ref{fig:adv_cg_convergence} we show the convergence of PCG for solving $\bm{H}\bm{x}=\bm{b}$, where $\bm{b}$ has i.i.d. random entries drawn from a standard Gaussian distribution. The preconditioners used, $\widetilde{\bm{H}}$, are our PSF preconditioner with $1$, $5$, or $25$ batches, regularization preconditioning, and no preconditioning. PCG converges fastest with the PSF preconditioners, with more batches yielding faster convergence. In Figure~\ref{fig:adv_generalized_eigenvalues}, we show the eigenvalues for the generalized eigenvalue problem $\bm{H}\bm{u}=\lambda \widetilde{\bm{H}}\bm{u}$, where the $\widetilde{\bm{H}}$ are the preconditioners stated above. The PSF preconditioners cluster the eigenvalues of the Hessian near one, and the regularization preconditioner clusters the trailing eigenvalues but amplifies leading eigenvalues. For the PSF preconditioner, more batches yields better clustering.
...
\begin{figure}
	...
	%\end{subfigure}
	\caption{(\ref{fig:adv_initial_condition}) True initial condition. (\ref{fig:adv_velocity}) Velocity field. Color indicates magnitude of velocity vector. (\ref{fig:adv_observations}) Noisy observations of concentration at final time. (\ref{fig:adv_reconstruction}) Reconstructed initial condition.}
...
\begin{table}
	\begin{center}
		...
		\endgroup
	\end{center}
	\caption{Number of preconditioned conjugate gradient iterations required to achieve a relative $l^2$ error of $10^{-6}$ \todo{we should be more specific about what the stopping criteria is, I expect that it is based on the linear system residual and if so we should state that, in order to distinguish it from the Stokes results earlier} for solving the Newton linear system in the advection-diffusion inverse problem. Iteration counts are shown for a variety of different diffusion parameters $\kappa$, simulation times $T$, and preconditioners.}
...
\begin{figure}
	\centering
	{
		...
		\begin{tabular}{cccc}
			& $T=0.5$ & $T=1.0$ & $T=2.0$ \\
			$\kappa=1.0$e${-4}$  & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T0.5_K1.e-4.png} & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T1.0_K1.e-4.png} & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T2.0_K1.e-4.png} \\
			...
			$\kappa=1.0$e${-3}$ & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T0.5_K1.e-3.png} & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T1.0_K1.e-3.png} & \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T2.0_K1.e-3.png}
		\end{tabular}
	}

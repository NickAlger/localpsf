...
\end{equation*}
is the data misfit, which measures the mismatch between the observed data and the predicted data based on a candidate parameter $\basalfriction$. Here $\bm{u} = \bm{u}(\basalfriction)$ denotes the velocity field solving $\ref{Stokeseqn}$ for the given parameter $\basalfriction$. The second term,
\begin{equation*}
	J_r(\basalfriction) := \frac{1}{2}\int_{\Omega}\basalfriction\mathcal{K}^{2}\basalfriction\,dx
\end{equation*}
is a Bilaplacian regularization term. Specifically, $\mathcal{K}$ is the inverse of the solution operator for the following elliptic PDE:
\begin{align*}
-\gamma\Delta u +\delta\,u&=s\,\,\,\,\text{ in }\Gamma_{b},\\
\gamma \bm{\nabla}u\cdot\bm{n}+\mu\,u&=0\,\,\,\text{ on }\partial\Gamma_{b}.
\end{align*}
The constant $\delta$ is chosen so that the correlation length of functions drawn from $N(0,\mathcal{L}^{-2})$ is $1/10$th of the radius of the domain. In all results shown except one, $\gamma$ is chosen so that the solution satisfies the Morozov discrepancy principle, that is,
\begin{equation*}
	\frac{1}{2}\int_{\Gamma_{t}}||\bm{d} - \bm{T}\bm{u}||_{2}^{2}\,dS = \frac{1}{2}\int_{\Gamma_{t}}||\boldsymbol{\xi}||_{2}^{2}\,dS.
\end{equation*}
The exception is Figure \ref{fig:gamma_sweep}, where we vary $\gamma$ to study the effect of changing $\gamma$ on the quality of our preconditioner.

The inverse problem is solved with an inexact-Newton conjugate gradient (CG) scheme. The initial guess for the parameter, $\basalfriction_0$, is the constant function with value $\basalfriction_0(x)=10.5$, which is halfway between the maximum and minimum values for $\basalfriction_\text{true}$. The parameter value is updated iteratively according to the formula
\begin{equation*}
...
\begin{equation}
	...
	H p_k = - g_k,
\end{equation}
and $\alpha_k \in \mathbb{R}$ is a step length parameter chosen to satisfy the Armijo conditions \cite{ARMIJO}. Here $g_k$ is the gradient of $J$ with respect to $\basalfriction$ at $\basalfriction_k$, and $H$ is either the Hessian or Gauss-Newton Hessian of $J$ with respect to $\basalfriction$ at $\basalfriction_k$. The Gauss-Newton Hessian is used for the first five iterations, and the Hessian is used for all subsequent iterations. The method terminates when the norm of the gradient drops by a factor of $10^8$. The Newton system, \eqref{eq:newton_system}, is solved inexactly to the tolerance $\min \left(0.5, \sqrt{||g_k||}\right)||g_k||$, using the preconditioned conjugate gradient method, which only requires the application of the (Gauss-Newton) Hessian to vectors. Each application of the (Gauss-Newton) Hessian to a vector requires solving one forward and one adjoint Stokes PDE of the form \eqref{Stokeseqn} , but with different right hand sides.

We use the framework described in this paper to generate Hessian preconditioners. We use our methodology to build H-matrix approximations of the data-misfit component of the Gauss-Newton Hessian. That is, the term in the Gauss-Newton Hessian that arises from the data misfit term $J_d$. We approximate the Gauss-Newton Hessian rather than the Hessian because the Hessian often has negative values in its integral kernel when one is away from the optimal point. At the discrete level, straightforward calculation shows that the regularization term in both the Hessian and the Gauss-Newton Hessian is given by
\begin{equation*}
	\bm{K} \bm{M}^{-1} \bm{K},
\end{equation*}
where $\bm{K}$ is the finite element discretization of the elliptic operator $\mathcal{K}$, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix approximations of these matrices using standard techniques for approximating sparse matrices with H-matrices, then invert $\bm{M}^{-1}$, and multiply these matrices together using H-matrix methods. We then add this H-matrix to the H-matrix for the data misfit term constructed using our method, and factor the resulting Hessian approximation using fast H-matrix factorization methods \cite{HLIBPRO}. We construct preconditioners using our method with 1, 5, and 25 impulse responses batches. The resulting preconditioners are denoted by PSF (1), PSF (5), and PSF (25), respectively. We compare the performance of these preconditioners with the most commonly used existing preconditioners, which are no preconditioning (denoted by NONE), and preconditioning by the regularization operator (denoted REG).
...
\begin{figure}[H]
	...
	%\end{subfigure}
	\caption{The log basal friction parameter $\basalfriction$ after $1$ (left), $3$ (second from left), and $4$ (third from left) Newton iterates and the final Newton iterate (right). The preconditioner is constructed between Newton iterations 3 and 4.} 
\end{figure} 

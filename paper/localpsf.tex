...
The core content of this paper is specifying (a) how we choose the points, $x_i$, (b) how we compute the convolution kernels, $\varphi_i$, and (c) what weighting functions $w_i$ that we use. Briefly, we compute one ``batch'' of $\varphi_i$'s by applying $\mathcal{A}$ to a sum of point sources, $\delta_{x_i},$ at many points $x_i$ (a ``Dirac comb''). That is, we compute
\begin{equation*}

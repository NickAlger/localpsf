...
Here we define the PDE-free spacially varying blurring problem, used
in Sections~\ref{sec:intro} and~\ref{sec:prelims} for visual
illustration of various stages and aspects of the proposed method.  To
study the influence of the presence of negative numbers in the
integral kernel, i.e., the robustness of the proposed method to
violations of the non-negative kernel assumption discussed in
Section~\ref{sec:impulse_response_2}, this example is set up so that
the impulse response, $\phi_x$, corresponding to the integral kernel
is a bumpy blob that is centered near $x$, and is rotated and scaled
in a manner that depends on $x$ (see Figure
\ref{fig:frog_kernel_impulse_responses}). The closed form expression
of the integral kernel is given by
%
...
(x^1 + x^2) \pi / 2$, $g(x) = x^1(1-x^1)x^2(1-x^2)$, and $C = L^2
\operatorname{diag}(c_1, c_2)$. The constant $L$ controls how wide the
blob is (hence the support of the impulse response), $a$ controls the
magnitude of the bumps, and $(c_1, c_2)$ control the width of the
Gaussian. We note that when $a=0$, $\phi_x$ is a Gaussian, as $a$
increases, $\phi_x$ becomes more bumpy. For instance, when $a > 1$,
the integral kernel contains negative values, which violates the
Assumption 3 (see Section~\ref{sec:conditions_2}).

...
Once the scattered impulse responses $\impulseresponse_{x_i}$ are computed, we approximate the integral kernel $\Phi(y,x)$ at arbitrary points $(y,x)$ by interpolation. The details of the interpolation will be described in Section SECTION, but the key idea is that
\begin{equation}
\Phi(y,x) = \phi_x(y) \approx \phi_{x_i}(y - \mu(x) + \mu(x_i))
\end{equation}
if $x_i$ is close to $x$. This is shown in Figure FIG. We therefore approximate $\phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for the $k$-nearest points $x_i$ to $x$. Here $k$ is a user-defined parameter; we use $k=10$. The weights in the linear combination are determined via radial basis function interpolation.

The ability to rapidly evaluate approximate kernel entries $\Phi(y,x)$ allows us to construct an H-matrix approximation, $\boldsymbol{\Phi}_H$, of $\Phi$, using the conventional adaptive cross H-matrix construction method. In this method, one forms low-rank approximations of blocks of the matrix by sampling rows and columns of those blocks \cite{HACA}. We then use H-matrix methods to convert $\boldsymbol{\Phi}_H$ into an H-matrix approximation of $\Aop$, denoted $\mathbf{A}_H$. Schematically the overall H-matrix approximation proceeds as follows:
\begin{equation*}
...
Let $x$ and $x'$ be points in $\Omega$ that are close to each other, and consider the following three approximations:
\begin{align}
...
\end{align}
Approximations \eqref{eq:translate1} and \eqref{eq:translate2} are standard. Approximation \eqref{eq:translate3}, which we use,  is new. These approximations are illustrated in Figure FIG.
...
However, even if $\Phi$ is smooth (which is typically the case asymptotically) the relevant Lipschitz constant for continuity, and hence the required rank in the low rank approximation, can be large, and this makes algorithms based on assumption \eqref{eq:translate1} highly inefficient. This is the case, for example, for Hessians in PDE constrained inverse problems with highly informative data. As the data become more informative, typically the ``mass'' of $\Phi(y,x)$ concentrates nearer and nearer to the ``diagonal'' $y=x$, the Lipschitz constant increases, and the required rank blows up.
...
%\end{equation}
Interpolation using all sample points $x_i$ is too expensive, so we only use the $k$ nearest sample points to $x$. In numerical experiments we use $k=10$. The $k$ nearest sample points are found by querying a precomputed KD-tree [CITE] of all sample points. We exclude points $x_i$ for which $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is not defined. In other words, if $y - \mu(x) + \mu(x_i) \notin \Omega$, then the point $x_i$ is considered invalid, and not used in the interpolation, even it is close to $x$. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) [CITE] of the mesh cells used to discretize the problem.

Once the $k$ nearest (valid) sample points, $\{x_i\}_{i=1}^k$, are found, we approximate $\Phi(y,x)$ using preconditioned radial basis function interpolation of the following form:
\begin{equation}
	\Phi(y,x) \approx \sum_{i=1}^k w_i~ b\left(\|x-x_i\|\right).
\end{equation}
For radial basis functions, $b_i$, we use polyharmonic splines. That is, 
\begin{equation}
b_i(r) = r^d
\end{equation}
if $d$ is odd, and
...
\end{equation}
if $d$ is even.
The vector of weights, $w = (w_1, w_2, \dots, w_k)^T$, is found as the solution to the $k \times k$ linear system
\begin{equation}
	Bw = f,
\end{equation}
...
The point $y - \mu(x) + \mu(x_i)$ is typically not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints. Although it is typically recommended that one avoid evaluating finite element functions at arbitrary points [CITE], here we have no choice. We evaluate $\frac{1}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
\label{eq:ellipsoid_check93}
	(y - \mu(x))^T \Sigma^{-1}(x) (y - \mu(x))  \le \tau^2.
\end{equation}
If \eqref{eq:ellipsoid_check93} does not hold, then $y - \mu(x) + \mu(x_i) \notin E_{x_i}$, so we assume $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)=0$ for the purpose of interpolation. If \eqref{eq:ellipsoid_check93} does hold, then $y - \mu(x) + \mu(x_i) \in E_{x_i}$. In this case, we look up the batch index $b$ for the batch of sample points containing $x_i$, and evaluate
\begin{equation}
	\frac{1}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right) = \eta_b\left(y - \mu(x) + \mu(x_i)\right).
\end{equation}
To evaluate $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$, we recall the mesh cell that $y - \mu(x) + \mu(x_i)$ resides in. This mesh cell was already computed as a side effect of querying the AABB tree of mesh cells when we checked whether $y - \mu(x) + \mu(x_i) \in \Omega$. We then evaluate the relevant finite element basis functions on the cell to evaluate $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$.
...
\left(y_i,\phi_{y_i}\left(x - \mu(y) + \mu(y_i)\right)\right)
\end{equation}
at the point $y$, in the manner described above but with $x$ and $y$ now taking opposite roles. We improve the interpolation accuracy at negligible additional cost by combining both of these approximations. Specifically, we combine both approximations by placing function values 
\begin{equation*}
...
\begin{equation*}
	...
	\end{bmatrix},
\end{equation*}
be the $2k \times 2k$ block matrix and $2k \times 1$ block vectors, respectively, with block entries of $\widetilde{B}$ given by
\begin{align*}
...
We solve the regularized least squares problem by QR factorization of the block $4k \times 2k$ matrix $\begin{bmatrix}\widetilde{B} & I\end{bmatrix}^T$. We regularize the problem because of numerical issues that arise if there are points $\widetilde{x}_i$ and $\widetilde{y}_j$ in the interpolation such that $\widetilde{x}_i \approx \widetilde{y}_j$, a situation that arises often. Indeed, situations where $\widetilde{x}_i = \widetilde{y}_j$ are inevitable when a regular grid is used to discretize the problem. In this case, $\widetilde{B}$ is (nearly) singular, so it is numerically ill-advised to solve the standard interpolation equation $\widetilde{B} \widetilde{w} = \widetilde{f}$. 

...
are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$. Such operators appear as, for example, Hessians in optimization and inverse problems governed by partial differential equations (PDEs)~\cite{BorziSchulz11,Delosreyes15,HinzePinnauUlbrichUlbrich08}, 
%AntilEtAl18
...
\begin{figure}
	...
	\caption{Left: Matrix created by evaluating the integral kernel $\Phi$ for $\Aop$ (Equation~\ref{eq:kernel_representation}) at all pairs of mesh vertices. This illustration is for the integral kernel given in Equation~\ref{eq:frog_kernel} in Section~\ref{sec:frog}. Dark colors indicate large entries and light colors indicate small entries. Rows and columns are ordered according to a kd-tree hierarchical clustering. Right: Impulse responses
...
\begin{itemize}
	...
	\item Negative numbers in the integral kernel affect the method more if they are isolated, and less if they are balanced by nearby positive numbers.
...
For the kernel in the left two columns of Figure \ref{fig:robustness_to_negatives}, negative numbers are interspersed with positive numbers, allowing us to include a large amount of negative numbers in the integral kernel before the method fails. For the kernel in the right two columns, the ellipsoid estimate fails with tiny amounts of negative numbers because the negative numbers are far away from the mean and not balanced by positive numbers at similar distances and angles. In the bottom two rows, we see the aforementioned threshold effect, in which the method rapidly transitions from performing reasonably well to performing poorly with only a small change to the integral kernel. 
...
Here we provide an overview of the operator approximation algorithm. Then, in the subsections (Sections \ref{sec:sample_point_selection}--\ref{sec:flip_eigs}) we present technical details of the algorithm that were not already covered in Section \ref{sec:prerequisites}.
...
$\rbf(r):= \exp\left(-\frac{1}{2}\left(\epsilon\frac{r}{L}\right)^2\right)$
is a Gaussian kernel radial basis function. Here $L:=\diam\left(\{x_i\}_{i=1}^{\numnbr}\right)$ is the diameter of the set of sample points used in the interpolation, and $\epsilon$ is a user-defined shape parameter that controls the width of the kernel function. The vector of weights, $\rbfweight = (\rbfweight_1, \rbfweight_2, \dots, \rbfweight_{\numnbr})^T$, is found as the solution to the $\numnbr \times \numnbr$ linear system
\begin{equation}
B \rbfweight = f,
\end{equation}
where $B \in \mathbb{R}^{\numnbr \times \numnbr}$,
...
The shape parameter, $\epsilon$, mediates a tradeoff between accuracy and stability. Small $\epsilon$ is required for RBF interpolation with Gaussian kernels to achieve high accuracy, but small $\epsilon$ also makes RBF interpolation less robust to errors or nonsmoothness in the function being interpolated. 
%This is illustrated in Figure \ref{fig:RBF_width} for a simple one dimensional interpolation problem. 
For our numerical results involving Hessians in inverse problems governed by PDEs (Sections~\ref{sec:ice}~and~\ref{sec:adv}), high accuracy is not required because the method is used to construct a preconditioner. Hence, for these Hessian approximations we use a conservative choice of $\epsilon=3.0$ to ensure robustness. For our numerical results for blur kernels (Section~\ref{sec:frog}), we use a smaller value of $\epsilon=0.5$ so that the RBF interpolation accuracy is not a limiting factor as we study convergence of the method.
...
All operations that do not require applications of $\Aop$ to vectors are polylog linear, and therefore scalable, in the size of the problem, $N$.
...
\paragraph{(1) Computing impulse response moments and batches} Computing $\spatialvol$, $\spatialmean$, and $\spatialcov$ requires applying $\Aop^T$ to $1$, $\gdim$, and $\gdim(\gdim+1)/2$ vectors, respectively.
...
In both cases, to reconstruct the unknown parameter fields, denoted $q$, the inverse problems are formulated as nonlinear least squares
...
Here we define a PDE-free spatially varying blurring problem, in which the impulse response, $\phi_x$,
is a bumpy blob that is centered
near $x$, and is rotated and scaled in a manner that depends on $x$
(see Figure \ref{fig:frog_kernel_impulse_responses}). This blurring problem is used in
Sections~\ref{sec:intro} and~\ref{sec:prelims} to visually illustrate
various stages and aspects of the proposed method, and robustness (or lack thereof) of the proposed method to violations of the non-negative kernel assumption (Section~\ref{sec:conditions_2}). In this section, the blurring problem is used to compare the proposed method to hierarchical off-diagonal low rank (HODLR) and global low rank (GLR) methods, to study convergence of the method, and to investigate the effect of the ellipsoid size parameter $\tau$.
%To study the
...
study the robustness of the PSF method to violations of Assumption 3
...
Equation~\eqref{eq:frog_kernel} using the proposed PSF method, the
randomized HODLR (hierarchical off diagonal low rank)
method~\cite{Martinsson16,Hartland_2023}, and GLR (global low rank)
approximation using randomized SVD~\cite{HalkoMartinssonTropp11}. For
...
In Figure \ref{fig:PSF_tau_convergence} we show the convergence of the proposed PSF method on the blur kernel as a function of the total number of impulse responses and the number of batches. We show convergence for several ellipsoid size parameters $\tau$, ranging from $2.0$--$4.0$. We observe a roughly linear convergence rate until a limit, which depends on $\tau$, is reached. I.e., the relative error decreases as $\text{constant} \times \left(\# \text{impulse responses}\right)^{-1}$ or $\text{constant} \times \# \text{batches}^{-1}$ until the limit is reached, then the error does not decrease any further. Increasing $\tau$ lowers this limit, allowing the method to achieve higher accuracy. Before this limit is reached, the convergence rate is the same for all $\tau$ as a function the number of impulse responses. This means that the convergence is faster for smaller $\tau$ as a function of the number of batches, since smaller $\tau$ means more impulse responses per batch.
...
\begin{figure}
	...
	\begin{subfigure}{0.47\textwidth}
		...
		\end{center}
	\end{subfigure}
	\caption{Relative error in the PSF approximation of the kernel, $||\boldsymbol{\Phi} - \widetilde{\boldsymbol{\Phi}}||_\text{Fro}/||\boldsymbol{\Phi}||_\text{Fro}$, for different ellipsoid size parameters, $\tau$. Left: Relative error vs. the total number of impulse responses used in the approximation. Right: Relative error vs. the number of impulse response batches. The dashed gray lines show linear convergence rates, i.e., $\text{constant} \times \left(\# \text{impulse responses}\right)^{-1}$ on the left, and  $\text{constant} \times \# \text{batches}^{-1}$ on the right.}
...
The pressure, $\pressure$, is discretized with first order scalar continuous Galerkin finite elements defined on a mesh of tetrahedra. The velocity, $\velocity$, is discretized with second order continuous Galerkin finite elements on the same mesh. The parameter $\basalfriction$ is discretized with first order scalar continuous Galerkin finite elements on the mesh of triangles that results from restricting the tetrahedral mesh to the basal boundary, $\Gamma_b$. Note that $\Gamma_b$ is a two-dimensional surface embedded in three dimensions due to the mountain topography. The proposed PSF method involves translating impulse responses. Hence it requires either a flat domain, or a notion of local parallel transport. We therefore generate a flattened version of $\Gamma_b$, denoted by $\Omega \subset \mathbb{R}^2$, by ignoring the height coordinate. The parameter $\basalfriction$ is viewed as a function on $\Gamma_b$ for the purpose of solving the Stokes equations, and as a function on $\Omega$ for the purpose of building Hessian approximations and defining the regularization.

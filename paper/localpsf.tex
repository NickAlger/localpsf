...
but we cannot easily access entries of $\Aop$'s integral kernel. Evaluating the maps in~\eqref{eq:eval_maps} may require, as a sub procedure, solving PDEs or performing other costly computations. By ``non-negative integral kernel,'' we mean that entries of the integral kernel are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$.
...
Picking the batches of points requires us to estimate the supports of the impulse responses $\phi_x$ \emph{before} we compute them. We form a-priori ellipsoid estimates for the supports of all the $\phi_x$ via a moment method that involves applying $\Aop^T$ to a small number of polynomial functions (see Section~\ref{sec:intromoments}). We were inspired by resolution analysis in seismic imaging, in which
...
are formed by adding $\bm{A}_H$ to $\mathbf{R}_H$ using fast H-matrix arithmetic. We modify $\widetilde{\mathbf{H}}$ to be symmetric and remove negative eigenvalues via the procedure described in Section~\ref{sec:flip_eigs}. We factor $\bm{\preconditioner}$,
...
restricted to the top surface of the geometry. We use $5\%$ noise in all cases, except for Figure \ref{fig:stokes_reconstructions} and Table \ref{tab:condition_number} where the noise is varied from $1\%$ to $25\%$. The true basal sliding
...
\begin{figure}
    ...
    \end{subfigure}
	\caption{Left (\ref{fig:cg_convergence}) shows the convergence history for solving $\bm{H} \bm{x} = \bm{b}$ using PCG, where $\bm{b}$ has i.i.d. random entries drawn from the standard Gaussian distribution and $\mathbf{H}$ is evaluated at the solution of the inverse problem. 

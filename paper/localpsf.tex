...
\begin{subequations}\label{Stokeseqn}
	...
	\bm{u}\cdot\bm{n} =0 \quad\text{and}\quad \bm{T}\left(\bm{\sigma}_{\bm{u}}\bm{n}+\exp\left(\basalfriction\right)\bm{u}\right)
	&=\bm{0}\,\,\,\,\,\,\text{ on }\Gamma_{b},\label{Stokeseqn:4} \\
	\left(\bm{\sigma}_{\bm{u}}\bm{n}+\mu\bm{u}\right)&=\bm{0}\,\,\,\,\,\,\text{ on }\Gamma_{l}.\label{Stokeseqn:5}	
	\end{align}
\end{subequations}
The solution to these equations is the tuple $(\bm{u}, p)$, where $\bm{u}$ is the ice flow velocity field, and $p$ is the pressure field. Here, $\basalfriction$ is the unknown logarithmic
...
We employ Glen's flow law \cite{Glen1955},
...
and $A$ is a flow rate factor. Glen's exponent has been chosen $n=1$, which provides for a linear Stokes model. Note that while the PDE is linear, the parameter-to-solution map, $\basalfriction \mapsto (\bm{u}, p)$, is nonlinear.

The pressure, $p$, is discretized with first order continuous Galerkin finite elements defined on a mesh of tetrahedra discretizing $U$. The velocity, $\bm{u}$, is discretized with second order continuous Galerkin finite elements on the same mesh, as required for discrete stability. The parameter $\basalfriction$ is discretized with first order scalar continuous Galerkin finite elements on the mesh of triangles that results from restricting the tetrahedral mesh to the basal boundary, $\Gamma_b$. 
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\caption{$\bm{u}_\text{true}$.}
	\end{subfigure}
	\caption{True logarithmic basal sliding parameter, $\basalfriction_\text{true}$, (left) and true velocity $\bm{u}_\text{true}$ (right). In the right plot, arrows indicate the direction of $\bm{u}_\text{true}$ and color indicates the magnitude.}
	\label{fig:true_beta_u}
\end{figure} 

The true parameter field, $\basalfriction_\text{true}$, and corresponding velocity field, $\bm{u}_\text{true}$, are shown in Figure \ref{fig:true_beta_u}. We generate synthetic observations by restricting the tangential component of $\bm{u}_\text{true}$ to the top surface to get
\begin{equation*}
\bm{y}_\text{true} = \bm{T} \bm{u}_\text{true} |_{\Gamma_t}.
\end{equation*}
We add $1\%$ multiplicative gaussian noise, $\boldsymbol{\xi}$, to all components of the finite element vector representing $\bm{y}_\text{true}$ to get noisy synthetic observations, $\bm{y}_\text{obs}$, which are used for the inversion. That is, 
\begin{equation*}
\left[\bm{y}_\text{obs}\right]_j := \left[\bm{y}_\text{true}\right]_j + \epsilon \left[\boldsymbol{\xi}\right]_j \left|\left[\bm{y}_\text{true}\right]_j\right|
\end{equation*}
where $\epsilon=0.01$, and the components of the noise function, $\boldsymbol{\xi}_j \sim N(0,1)$, are independent and identically distributed (i.i.d) random numbers drawn from the standard normal distribution. Here $\left[\bm{f}\right]_j$ denotes the $j$th entries of the finite element vector for the discretized versions of the function $\bm{f}$, and $j$ ranges over all entries of the finite element vector. Here we do not use bold to distinguish finite element vectors from their associated functions (as we did previously) because the functions $\bm{y}_\text{true}$, $\bm{y}_\text{obs}$, and $\bm{\xi}$ are already written in bold to denote the fact that they are vector valued.
...
\begin{equation*}
	J_d(\basalfriction) := \frac{1}{2}\int_{\Gamma_{t}}||\bm{d} - \bm{T}\bm{u}||_{2}^{2}\,dS.
\end{equation*}
We call this term the data misfit, because it measures the mismatch between the observed data and the predicted data based on a candidate parameter $\basalfriction$. Here $\bm{u} = \bm{u}(\basalfriction)$ denotes the velocity field solving $\ref{Stokeseqn}$ for the given parameter $\basalfriction$. The second term,
\begin{equation*}
...
\begin{align*}
-\gamma\Delta u +\delta\,u&=s\,\,\,\,\text{ in }\Gamma_{b},\\
\gamma \bm{\nabla}u\cdot\bm{n}+\mu\,u&=0\,\,\,\text{ on }\partial\Gamma_{b}.
\end{align*}
In all results shown except one, $\gamma$ is chosen so that the solution satisfies the Morozov discrepancy principle, that is,
\begin{equation*}
	\frac{1}{2}\int_{\Gamma_{t}}||\bm{d} - \bm{T}\bm{u}||_{2}^{2}\,dS = \frac{1}{2}\int_{\Gamma_{t}}||\boldsymbol{\xi}||_{2}^{2}\,dS.
\end{equation*}
The value of $\gamma$ satisfying the Morozov discrepancy principle is $\gamma=7.3 \times 10^3$. The exception is Figure \ref{fig:gamma_sweep}, where we vary $\gamma$ to study the effect of changing $\gamma$ on the quality of our preconditioner. The constant $\delta$ is chosen so that the correlation length of functions drawn from $N(0,\mathcal{L}^{-2})$, which is given by the formula $L=\sqrt{\gamma/\delta}$, is $1/10$th of the radius of the domain. The function $\basalfriction_0$ is the constant function with value $\basalfriction_0(x)=10.5$, which is halfway between the maximum and minimum values of $\basalfriction_\text{true}$.

The inverse problem is solved with a preconditioned inexact-Newton conjugate gradient scheme \cite{NOCEDAWRIGHT}. The constant function $\basalfriction_0$ is used as the initial guess. The parameter value is updated iteratively according to the formula
\begin{equation*}
	\basalfriction_{k+1} = \basalfriction_k + \alpha_k p_k,
\end{equation*}
where the search direction $p_k$ is an approximate solution to the Newton linear system
\begin{equation}
	\label{eq:newton_system}
	H p_k = - g_k,
\end{equation}
and $\alpha_k \in \mathbb{R}$ is a step length parameter chosen to satisfy the Armijo conditions \cite{ARMIJO}. Here $g_k$ is the gradient of $J$ with respect to $\basalfriction$ at $\basalfriction_k$, and $H$ is either the Hessian or Gauss-Newton Hessian of $J$ with respect to $\basalfriction$. The Gauss-Newton Hessian is Hessian of $J$, except in the data misfit, $J_d$, the nonlinear mapping $\basalfriction \mapsto \bm{T} \bm{u}$ is replaced by its local linearization at $\basalfriction_k$. The Gauss-Newton Hessian is used for the first five iterations, and the Hessian is used for all subsequent iterations.  The method terminates when $||g_k||/||g_0|| < 10^{-8}$. The Newton systems, \eqref{eq:newton_system}, are solved inexactly using an inner conjugate gradient (CG) iteration, which we terminate when $||g_k + H p_k|| < \min \left(0.5, \sqrt{||g_k||}\right)||g_k||$. The preconditioned conjugate gradient method only requires the application of the (Gauss-Newton) Hessian to vectors. Each application of the (Gauss-Newton) Hessian to a vector requires solving one forward and one adjoint Stokes PDE of the form \eqref{Stokeseqn}, but with different right hand sides.

We use the framework described in this paper to generate Hessian preconditioners. We build H-matrix approximations of the data-misfit Gauss-Newton Hessian (the term in the Gauss-Newton Hessian that arises from the data misfit) using 1, 5, and 25 impulse responses batches. We approximate the Gauss-Newton Hessian rather than the Hessian because the Hessian often has negative values in its integral kernel when one is away from the optimal point. Our results show that our preconditioners, which are built using the Gauss-Newton Hessian, are also good preconditioners for the Hessian. For the purpose of building the Hessian approximation, the basal surface, which is a 2D surface embedded in 3D due to the mountain topography, is flattened into the plane by ignoring the height coordinate. This flattening is necessary because our method requires adding points on the basal surface together to get other points on the basal surface, and this is only well-defined in flat space. 
...
\begin{figure}[H]
	...
	%\end{subfigure}
	\caption{The log basal friction parameter $\basalfriction$ after $1$ (left), $3$ (second from left), and $4$ (third from left) Newton iterations, and the final Newton iterate (right). The PSF (5) preconditioner is constructed between Newton iterations 3 and 4 and re-used for all subsequent iterations.}

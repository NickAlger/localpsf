...
in an inverse problem governed by a PDE which models steady state ice
sheet flow~\cite{PetraEtAl12} (Section~\ref{sec:ice}) and transport
and flow of a contaminant~\cite{PetraStadler11}
(Section~\ref{sec:adv}). To reconstruct the unknown parameters, we
formulate an inverse problem governed by the corresponding PDEs. In
particular this problem is formulated as a nonlinear least squares
optimization problem, whose cost function consists of a misfit term
...
all the numerical results except for the studies where study how
...
which we seek to infer an unknown initial contaminant field from
pointwise measurements of its concentration. The problem description
below closely follows the one in~\cite{PetraStadler11}. This PDE
models diffusive transport in a domain $\D \subset \R^d$, which is
depicted in Figure~\ref{fig:adv_inversion1}.  The domain
boundaries $\partial \D$ include the outer boundaries as well as the
...
condition $\ipar \in L^2(\D)$ to observations of the concentration
field $u(\vec{x}, t)$ through solution of the advection-diffusion
equation given by
\begin{equation}\label{eq:ad}
  ...
  \end{aligned}
\end{equation}
Here, $\kappa > 0$ is a diffusivity coefficient, and $T > 0$ is the
final time of observations.  The velocity field $\vec{v}$, shown in
...
details.  The time evolution of the state variable $u$ from a given

...
%		Compute $\bm{\spatialvol}$, $\bm{\spatialmean}$, and $\bm{\spatialcov}$ using Algorithm \ref{alg:varhpi_mean_cov}.
		...
		(optional) Modify $\mathbf{A}_H$ to make $\mathbf{A}_H^{\text{sym}+}$ (Appendix \ref{sec:make_spd})
...
\begin{equation}
\Aker(y,x) \approx \widetilde{\Aker}(y,x) := \sum_{i=1}^{\numnbr} \rbfweight_i~ b\left(\|x-x_i\|\right),
\end{equation}
where $b_i$ are radial basis functions and $\rbfweight_i$ are weights. The vector of weights, $\rbfweight = (\rbfweight_1, \rbfweight_2, \dots, \rbfweight_{\numnbr})^T$, is found as the solution to the $\numnbr \times \numnbr$ linear system
\begin{equation}
B \rbfweight = f,
\end{equation}
where $B \in \mathbb{R}^{\numnbr \times \numnbr}$ has entries
...
\end{cases}
\end{equation}
for $r>0$, and $b(0):=0$. This yields the smoothest interpolant of the given points and values, in a certain least-squares sense \cite{Duchon77}. For more details on radial basis function interpolation, see \cite{Wendland04}. The interpolation procedure is illustrate in Figure \ref{fig:rbf_interpolation}.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{The ice sheet, discretized by a mesh of tetrahedra. Color indicates the height of the base of the ice sheet (i.e., the mountain topography). The radius of the domain is $10$ kilometers, and the maximum height of the mountain is $2.1$ kilometers.}
...
\begin{subequations}\label{Stokeseqn}
	...
	\stress\normalvec+\mu \velocity &=0\,\,\,\,\,\,\text{ on }\Gamma_{l}.\label{Stokeseqn:5}	
	\end{align}
\end{subequations}
...
\nabla\velocity+\nabla\velocity^{\top} \right)$. Here $\eta$ is the effective viscosity and $I$ is the unit matrix. Glen's exponent has been chosen $n=1$, which provides for a linear Stokes model. Note that while the PDE is linear, the parameter-to-solution map, $\basalfriction \mapsto (\velocity, \pressure)$, is nonlinear.

The pressure, $\pressure$, is discretized with first order continuous Galerkin finite elements defined on a mesh of tetrahedra discretizing $U$. The velocity, $\velocity$, is discretized with second order continuous Galerkin finite elements on the same mesh. The parameter $\basalfriction$ is discretized with first order scalar continuous Galerkin finite elements on the mesh of triangles that results from restricting the tetrahedral mesh to the basal boundary, $\Gamma_b$. The basal boundary, $\Gamma_b$ is a 2D surface embedded in 3D due to the mountain topography. We also generate a flattened version of $\Gamma_b$, denoted by $\Omega \subset \mathbb{R}^2$, by ignoring the height coordinate. The parameter $\basalfriction$ is viewed as a function on $\Gamma_b$ for the purpose of solving the Stokes equation, but is viewed as a function on $\Omega$ for the purpose of building the Hessian preconditioner and defining the regularization. 
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{True logarithmic basal sliding parameter, $\basalfriction_\text{true}$, (left) and true velocity $\velocity_\text{true}$ (right). In the right plot, arrows indicate the direction of $\velocity_\text{true}$ and color indicates the magnitude.}
...
\begin{equation*}
	J_r(\basalfriction) := \frac{1}{2}\int_{\Omega}\left| \mathcal{L}(\basalfriction - \basalfriction_0)\right|^2\,dx,
\end{equation*}
is a Bilaplacian regularization term. Specifically, $\mathcal{L}$ is the inverse of the solution operator for the following elliptic PDE:
\begin{align*}
-\gamma\Delta u +\delta\,u&=s\,\,\,\,\text{ in }\Omega,\\
\gamma \nabla u\cdot\normalvec+\mu\,u&=0\,\,\,\text{ on }\partial\Omega.
\end{align*}
Recall that $\Omega$ is the 2D flattened version of the basal surface $\Gamma_b$.
In all results shown except one, $\gamma$ is chosen so that the solution satisfies the Morozov discrepancy principle, that is,
\begin{equation*}
	\frac{1}{2}\int_{\Gamma_{t}}||\observations_\text{obs} - \tangentop\velocity||_{2}^{2}\,dS = \frac{1}{2}\int_{\Gamma_{t}}||\xi||_{2}^{2}\,dS.
\end{equation*}
The value of $\gamma$ satisfying the Morozov discrepancy principle is $\gamma=7.3 \times 10^3$. The exception is Figure \ref{fig:gamma_sweep}, where we vary $\gamma$ to study the effect of changing $\gamma$ on the quality of our preconditioner. The constant $\delta$ is chosen so that the correlation length, $L$, of functions drawn from the normal distribution with covariance $\mathcal{L}^{-2}$, which is given by the formula $L=\sqrt{\gamma/\delta}$, is $1/10$th of the radius of the domain. The function $\basalfriction_0$ is the constant function with value $\basalfriction_0(x)=10.5$, which is halfway between the maximum and minimum values of $\basalfriction_\text{true}$.

The inverse problem is solved with a inexact Newton preconditioned conjugate gradient scheme (PCG) \cite{NOCEDAWRIGHT}. The constant function $\basalfriction_0$ is used as the initial guess. The parameter value is updated iteratively according to the formula
$
	\basalfriction_{k+1} = \basalfriction_k + \alpha_k \searchdir_k,
$
where $\searchdir_k$ approximately solves one of the following Newton linear systems
\begin{equation}
	\label{eq:newton_system}
	H \searchdir_k = - g_k \qquad \text{or} \qquad H_\text{gn}, \searchdir_k = - g_k
\end{equation}
and the step length $\alpha_k \in \mathbb{R}$ is chosen to satisfy the Armijo conditions \cite{ARMIJO}. Here $g_k$ is the gradient of $J$, $H$ is the Hessian of $J$, and $H_\text{gn}$ is the Gauss-Newton Hessian, and these derivatives are evaluated at $\basalfriction_k$. 
...
We use $H_\text{gn}$ for the first five iterations, and $H$ for all subsequent iterations.  The method terminates when $||g_k||/||g_0|| < 10^{-8}$. The systems \eqref{eq:newton_system} are solved inexactly using an inner PCG iteration, which terminates when the norm of the residual for the linear system is less than $\min \left(0.5, \sqrt{||g_k||}\right)||g_k||$. The PCG method only requires applying $H$ (or $H_\text{gn}$) to vectors. Each application of $H$ (or $H_\text{gn}$) to a vector requires solving one incremental forward and one incremental adjoint Stokes PDE of the form \eqref{Stokeseqn}, but with different right hand sides.
...
\begin{figure}
	...
	%\end{subfigure}
	\caption{The log basal friction parameter $\basalfriction$ after $1$ (left), $3$ (second from left), and $4$ (third from left) Newton iterations, and the final Newton iterate (right). The PSF (5) preconditioner is constructed between Newton iterations 3 and 4.}
	\label{fig:newton_iterates} 
\end{figure} 

We use the framework described in this paper to generate Hessian preconditioners. We build H-matrix approximations of the data-misfit Gauss-Newton Hessian (the term in the Gauss-Newton Hessian that arises from the data misfit) using 1, 5, and 25 impulse responses batches. We approximate the Gauss-Newton Hessian rather than the Hessian because the Hessian often has negative values in its integral kernel when one is away from the optimal point. Our results show that our preconditioners, which are built using the Gauss-Newton Hessian, are also good preconditioners for the Hessian. 

The H-matrix approximation of data misfit Gauss-Newton Hessian is added to a H-matrix approximation of the Hessian of the regularization term, $J_r$, to form a H-matrix approximation of the Gauss-Newton Hessian. For the Hessian of the regularization term, straightforward calculation shows that the discrete regularization term in both the Hessian and the Gauss-Newton Hessian is given by
$
(\gamma \bm{K} + \delta \bm{M}) \bm{M}^{-1} (\gamma \bm{K} + \delta \bm{M})
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix approximations of these matrices using standard techniques for approximating sparse matrices with H-matrices. Then we invert $\bm{M}^{-1}$, and multiply these matrices together using H-matrix methods. We factor the overall Gauss-Newton Hessian approximation using fast H-matrix methods, then and use the factorization as a preconditioner \cite{HLIBPRO}. The resulting preconditioners for 1, 5, and 25 impulse response batches are denoted by PSF (1), PSF (5), and PSF (25), respectively. 
...
\begin{table}
	...
	\caption{Convergence history for solving the Stokes inverse problem using the preconditioned inexact Newton-PCG method. Preconditioners shown are our method with five impulse response batches (PSF (5)) constructed at the third iteration, regularization preconditioning (REG), and no preconditioning (NONE). The columns titled \#CG show the number of conjugate gradient iterations used to solve the Newton system at $\basalfriction_k$, for the purpose of computing a search direction towards $\basalfriction_{k+1}$. The columns titled $||g||$ show the $l^2$ norm of the gradient at $\basalfriction_k$. The columns titled \#Stokes show the total number of Stokes PDE solves performed in each Newton iteration. This consists of Stokes solves for $u$ performed during the linesearch going from $\basalfriction_{k-1}$ to $\basalfriction_k$, plus the adjoint Stokes solve to compute the gradient at $\basalfriction_k$, plus one incremental forward and one incremental adjoint Stokes solve per PCG iteration for solving the Newton system at iteration $k$. In the PSF (5) portion of row 3, we write $6+22$ to indicate that 6 Stokes solves were used during the standard course of the iteration, and 22 Stokes solves were used to build the PSF (5) preconditioner. }
	\label{tab:newton_convergence_table}
\end{table}

In Table \ref{tab:newton_convergence_table} we study the performance of our preconditioner for accelerating the solution of optimization problem \eqref{eq:deterministic_optimization_problem}. We build the PSF (5) preconditioner in the third Gauss-Newton iteration, and re-use it for all subsequent Gauss-Newton and Newton iterations. No preconditioner is used in the three Gauss-Newton iterations before the PSF (5) preconditioner is built. We compare our method with the most commonly used existing preconditioners, which are no preconditioning (denoted by NONE), and preconditioning by the regularization term in the Hessian (denoted REG). In Figure \ref{fig:newton_iterates} we show several Newton iterates using our PSF (5) preconditioner. Our results show that our PSF (5) preconditioner substantially outperforms the no preconditioning and regularization preconditioning. Using PSF (5) reduces the total number of Stokes PDE solves by a factor of six compared to no preconditioning, and by a factor of nine compared to regularization preconditioning. Interestingly, we observe that no preconditioning actually outperforms regularization preconditioning here because the noise level, and hence the regularization parameter, $\gamma$, are small.
...
\begin{figure}
	\begin{subfigure}{0.59\textwidth}
		\begin{center}
			...
			title={Generalized eigenvalues, $Hu_{k}=\lambda_{k}\preconditioner u_{k}$}
			]
			\addlegendentry{PSF (1)}
	...
	\begin{subfigure}{0.39\textwidth}
		\begin{center}
			...
			\begin{tabular}{c| c c}
				& $\kappa(\preconditioner^{-1}H)$ &	$\kappa(\preconditioner^{-1}H_{\text{gn}})$ \\
				\hline 
	...
	\end{subfigure}
	\caption{Left (\ref{fig:generalized_eigenvalues}): eigenvalues for generalized eigenvalue problem $H u_k = \lambda_k \preconditioner_k$. Here $H$ is the Hessian and $\preconditioner$ is the PSF approximation constructed using the Gauss-Newton Hessian, $H_\text{gn}$, for 1, 5, and 25 batches, or the regularization operator. Right (\ref{fig:condition_number}): condition number, $\kappa$, for the preconditioned Hessian and Gauss-Newton Hessian for different preconditioners. All operators are evaluated at the parameter $\basalfriction$ that solves the inverse problem for $\gamma$ given by the Morozov discrepancy principle.}
\end{figure}

In Figure \ref{fig:generalized_eigenvalues} we show the generalized eigenvalues $\lambda$ for the generalized eigenvalue problem
\begin{equation*}
	H u = \lambda \preconditioner u.
\end{equation*}
Here $H$ is the Hessian evaluated at the reconstructed parameter $\basalfriction$ for $\gamma$ chosen to satisfy the Morozov discrepancy principle. The matrix $\preconditioner$ is one of the PSF (1), PSF (5), or PSF (25) Gauss-Newton Hessian approximations constructed at that $\basalfriction$, or the regularization Hessian. The PSF preconditioners cluster the eigenvalues of the Hessian near $1$, with more batches yielding better clustering. The regularization preconditioner clusters the trailing eigenvalues but amplifies leading eigenvalues. In Figure \ref{fig:condition_number} we show the condition number of the preconditioned Hessian $\preconditioner^{-1}H$, and preconditioned Gauss-Newton Hessian, $\preconditioner^{-1}H_\text{gn}$. The condition numbers are similar for both the Hessian and the Gauss-Newton Hessian. The PSF preconditioners yield the the smallest condition numbers, with more batches yielding smaller condition numbers. Regularization preconditioning and no preconditioning yield larger condition numbers, with regularization preconditioning actually increasing the condition number as compared to no preconditioning.

...
\begin{thm}[Procedure for computing $v_*$ does not break down]
	If all the sample points $x_i$, $i=1,\dots,r$, are distinct, then PDE \eqref{eq:psi_eq} is uniquely solvable for $\psi_i$, PDE \eqref{eq:theta_eq} is uniquely solvable for $\theta_i$, and linear system \eqref{eq:interpolation_linear_system} is uniquely solvable for $\mathbf{c}$ and $\beta$. That is, the procedure for computing $v_*$ does not break down. Furthermore, $v_* \in H^2_N(\Omega)$.
\end{thm}

\begin{proof}
	Existence and uniqueness for the solutions to PDEs \eqref{eq:psi_eq} and \eqref{eq:theta_eq} follow from the standard elliptic PDE theory. By standard elliptic regularity theory, $\psi_i \in H^1(\Omega)$, so $\psi_i \in L^2(\Omega)$, and so all matrix entries $S_{ij}$ are finite. 
	...
	\begin{equation*}
		0 = a_1 \psi_1 + a_2 \psi_2 + \dots a_r \psi_r.
	\end{equation*}
	Applying the Laplacian to this equation yields
	\begin{equation*}
		0 = a_1 \delta_{x_1} + a_2 \delta_{x_2} + \dots a_r \delta_{x_r} - \left(\sum_{i=1}^r a_i\right)/|\Omega|,
	\end{equation*}
	...
	By standard elliptic regularity theory, the functions $\theta_i$ are in $H^3(\Omega)$, and therefore they are in $H^2(\Omega)$. By construction the functions $\theta_i$ have Neumann zero boundary conditions, so $\theta_i \in H^2_N(\Omega)$. Since $v_*$ is a linear combination of the functions $\theta_i$ and the constant function $C(x)=1$, $v_* \in H^2_N(\Omega)$.
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	In the first line we used the definition of $v_*$ in \eqref{eq:optimal_v}. In the second line we used Lemma \ref{lem:psi_delta_eval}. In the third line we used the facts that, by construction, $\Delta \theta_j = \psi_j$ and $\int_\Omega \theta_j(x) dx = 0$. In the fourth line we used the definition of $S$ in \eqref{eq:S_matrix}. Hence the condition $v_*(x_i) = f_i$ for $i=1,\dots,r$ may be written in matrix form as
	\begin{equation*}

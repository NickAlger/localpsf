...
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix representations of these matrices and combine them using standard sparse H-matrix techniques. We factor the overall Gauss-Newton Hessian approximation, denoted $\bm{\preconditioner}$, using fast H-matrix methods \cite{RONKRIEMANNEMAIL}, then and use the factorization as a preconditioner \cite{HLIBPRO}. 

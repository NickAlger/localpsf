...
where $\{x_i\}_{i=1}^k$ are the $k$ nearest sample points to $x$, excluding sample points for which $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is undefined. That is, excluding sample points such that $y-\mu(x)+\mu(x_i) \notin \Omega$. The number $k$ is a small user defined parameter; in numerical experiments we use $k=10$. We find the $k$ nearest sample points by querying a precomputed KD-tree \cite{kdtree} of all sample points. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) \cite{aabbtree} of the mesh cells used to discretize the problem. 

The idea here is that $\Phi(y,x) \approx f_i$ when $x_i$ is near $x$, so we approximate $\Phi(y,x)$ by interpolating the (point,value) pairs
\begin{equation}
\left(x_i, f_i\right), \qquad i=1,\dots,k,
\end{equation}
at the point $x$. To see that $\Phi(y,x) \approx f_i$, recall from Section \ref{eq:local_mean_displacement_invariance} that $\Phi(y,x) \approx \phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ if $x_i$ is near $x$. Inclusion of the extra factor $\frac{V(x)}{V(x_i)}$ in \eqref{eq:fxyxp} is a form of preconditioning that makes this approximation more robust in the case where $V(x)$ varies widely. 
...
We use the following radial basis function interpolation scheme:

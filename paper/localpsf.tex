...
\end{equation*}
for arbitrary pairs of points $(y,x) \subset \Omega \times \Omega$ via interpolation.
...
Evaluating the impulse response batch $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$ requires some care, because the point $y - \mu(x) + \mu(x_i)$ is generally not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints.
...
\subsubsection{Improved interpolation for symmetric operators}
...
We solve the regularized least squares problem by QR factorization of the block $4k \times 2k$ matrix $\begin{bmatrix}\widehat{B} & I\end{bmatrix}^T$. We regularize the problem because of numerical issues that arise if there are points $\widehat{x}_i$ and $\widehat{y}_j$ in the interpolation that are very close to each other, a situation that arises often in practice. Indeed, situations where $\widehat{x}_i = \widehat{y}_j$ are inevitable when a regular grid is used to discretize the problem. In this case, $\widehat{B}$ is (nearly) singular, so it is numerically ill-advised to solve the standard (un-regularized) interpolation equation $\widehat{B} \widehat{w} = \widehat{f}$. 

...
\ifpdf
\hypersetup{
  pdftitle={Efficient matrix-free point spread function approximation of operators with locally supported non-negative integral kernels, with application to Hessians in PDE constrained inverse problems},
...
% REQUIRED
\begin{abstract}
	We present an efficient matrix-free point spread function (PSF) method for approximating operators that have locally supported non-negative integral kernels. Such operators arise as Schur complements in Schur complement methods for solving partial differential equations (PDEs), Poincare-Steklov operators in domain decomposition methods, covariance operators in spatial statistics, blurring operators in imaging, and Hessians in PDE constrained inverse problems. We compute impulse responses of the operator at a collection of scattered points, then interpolate translated versions of these impulse responses to approximate entries of the integral kernel. The interpolation uses a new approximation we call ``local mean displacement invariance,'' which improves upon classical local translation invariance.
%	Classical H-matrix construction methods require access to matrix entries of the matrix being approximated, and therefore cannot be used to efficiently form H-matrix approximations of operators that are only available through matrix-vector products. 
	...
	We use the method to approximate the Hessian in an ice sheet flow PDE constrained inverse problem. Numerical results demonstrate that our method substantially outperforms existing methods. We are able to approximate high rank operators using a small number of operator applies.
\end{abstract}

% REQUIRED
\begin{keywords}
 	data scalability, Hessian, hierarchical matrix, matrix-free, operator approximation, PDE constrained inverse problems, point spread function, product-convolution, local translation invariance, impulse response
\end{keywords}
...
However, low rank approximation methods suffer from a ``data predicament''---if the data are highly informative about the unknown parameter, then the numerical rank of the data misfit term in the Hessian is large, so a large number of operator applies are required to form an accurate ``low rank'' approximation \cite{Alger19,AlgerEtAl17}. Matrix-free high rank approximation methods are needed.
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Batches of normalized impulse responses for the Stokes inverse problem data misfit Gauss-Newton Hessian, described in Section \ref{sec:numerical_results}. 
...
\begin{figure}
	\begin{center}
		\includegraphics[scale=0.8]{mean_displacement_invariance.pdf}
	\end{center}
...
The ability to rapidly approximate entries of $\Aop$'s integral kernel allows us to form a hierarchical matrix (H-matrix) \cite{Hackbusch99,} approximation of a discretized version of $\Aop$. H-matrices are a matrix format in which the rows and columns of the matrix are re-ordered, then the matrix is recursively subdivided into blocks, in such a way that many off-diagonal blocks are low rank, even though the matrix as a whole may be high rank. 
...
Recently there have been improvements in matrix-free H-matrix construction methods based on the so-called ``peeling process'' \cite{LinYing11,Martinsson11,Martinsson16,MartinssonTropp20}, which we do not use. 
...
Let $\delta_x$ denote the delta distribution\footnote{Recall that the delta distribution $\delta_x:\mathcal{D}(\Omega)\rightarrow \mathbb{R}$ is the linear functional defined by $\delta_x(w) = w(x)$ for all $w\in \mathcal{D}(\Omega)$.} (i.e., point source, impulse) centered at the point $x \in \Omega$.
...
\paragraph{Lumped mass matrix} At the continuum level, $\Aker$ is assumed to be non-negative. However, entries of the discrete integral kernel, $\Aker_h$, involve inverse mass matrices, $\massmatrix^{-1}$, which typically contain negative numbers. If there are too many negative numbers, or if the negative numbers are too large, our algorithm will be less robust. We therefore recommend replacing the mass matrix, $\massmatrix$, with a positive diagonal \emph{lumped mass} approximation. 
...
\end{bmatrix}.
\end{equation*}
By computing one matrix-vector product of $\mathbf{B}^T$ with $\mathbf{w}$, we compute the inner product of each column of $\mathbf{B}$ with $\mathbf{w}$ simultaneously. The operator case is analogous, with the impulse response $\phi_x$ taking the place of a matrix column. We have
\begin{equation}
\label{eq:operator_simultaneous_ips}
\left(\Aop^T w\right)^*(x) = \int_\Omega \Aker(y,x) w(y) dy = \left(\phi_x, w\right)_{L^2(\Omega)}.
\end{equation}
By computing one operator application of $\Aop^T$ to $w$, we compute the inner product of each impulse response $\phi_x$ with $w$, for all points $x$ simultaneously. 
...
\end{equation*}
Hence, we compute $\spatialvol(x)$ for all $x$ simultaneously by applying $\Aop^T$ to $C$. Analogous manipulations show that $\spatialmean(x)$ and $\spatialcov(x)$ may be computed for all points $x$ simultaneously by applying $\Aop^T$ to the functions $L^i$ and $Q^{ij}$, respectively. To summarize the results of doing this, we have
\begin{subequations}
...
\end{equation}
where $\rbfweight_i$ are weights, and $\rbf$ is a polyharmonic spline radial basis function, which is defined as follows:
\begin{equation}
\rbf(r) := \begin{cases}
...
\end{cases}
\end{equation}
for $r>0$, and $\rbf(0):=0$. The vector of weights, $\rbfweight = (\rbfweight_1, \rbfweight_2, \dots, \rbfweight_{\numnbr})^T$, is found as the solution to the $\numnbr \times \numnbr$ linear system
\begin{equation}
B \rbfweight = f,
\end{equation}
where $B \in \mathbb{R}^{\numnbr \times \numnbr}$ has entries
...
$
and $f \in \mathbb{R}^{\numnbr}$ has entries $f_i$ from \eqref{eq:fxyxp}. Polyharmonic spline radial basis function interpolation yields the smoothest interpolant of the given points and values, in a certain least-squares sense \cite{Duchon77}. For more details on radial basis function interpolation, see \cite{Wendland04}. 
...
We form $\mathbf{\Aker}_H$ using the standard geometrical clustering/adaptive cross method implemented within the HLIBpro software package \cite{Kriemann08}. Although $\mathbf{\Aker}$ is a dense $\fedim \times \fedim$ matrix, constructing $\mathbf{\Aker}_H$ only requires evaluation of $O(\hrank^2 \fedim \log \fedim)$ kernel entries $\mathbf{\Aker}_{ij} = \widetilde{\Aker}(\fepoint_i, \fepoint_j)$, and these entries are computed via the radial basis function interpolation method described in Section \ref{sec:approximate_kernel_entries}. Here $\hrank$ is the rank of the highest rank block in the H-matrix. 
...
\end{equation*}
We call this term the data misfit, because it measures the mismatch between the observed data and the predicted data based on a candidate parameter $\basalfriction$. Here $\velocity = \velocity(\basalfriction)$ denotes the velocity field solving $\ref{Stokeseqn}$ for the given parameter $\basalfriction$. The second term,
\begin{equation*}
...
\begin{equation*}
	\frac{1}{2}\int_{\Gamma_{t}}||\observations_\text{obs} - \tangentop\velocity||_{2}^{2}\,dS = \frac{1}{2}\int_{\Gamma_{t}}||\xi||_{2}^{2}\,dS.
\end{equation*}
...
We solve the inverse problem with an inexact Newton preconditioned conjugate gradient (PCG) scheme \cite{NOCEDAWRIGHT}, using $\basalfriction_0$ as the initial guess. The discretized parameter, $\bm{\basalfriction}$ is updated iteratively according to the formula
$
...
\end{equation}
and the step length $\alpha_k \in \mathbb{R}$ is chosen by line search to satisfy the Armijo conditions \cite{ARMIJO}. Here $\bm{g}_k$ is the discretized gradient of $J$, $\bm{H}$ is the discretized Hessian, and $\bm{H}_\text{gn}$ is the discretized Gauss-Newton Hessian, and these derivatives are evaluated at $\bm{\basalfriction}_k$. 
...
We use the framework described in this paper to generate Hessian preconditioners. We build H-matrix approximations of the data misfit Gauss-Newton Hessian (the term in $\bm{H}_\text{gn}$ that arises from the data misfit) using 1, 5, and 25 impulse responses batches. These approximations are denoted by PSF (1), PSF (5), and PSF (25), respectively. The resulting H-matrices, which may be indefinite due to approximation error, are modified to be symmetric positive definite via the procedure described in Appendix \ref{sec:make_spd}, with $l=2$. We approximate $\bm{H}_\text{gn}$ rather than $\bm{H}$ because $\bm{H}$ often has negative values in its integral kernel when one is away from the optimal point. Our results show that our preconditioners are also good preconditioners for $\bm{H}$. The H-matrix approximation of the data misfit Gauss-Newton Hessian is added to a H-matrix approximation of the Hessian of the regularization term to form a H-matrix approximation of $\bm{H}_\text{gn}$. Straightforward calculation shows that the discretized Hessian of the regularization term is given by
$
(\gamma \bm{K} + \delta \bm{M}) \bm{M}^{-1} (\gamma \bm{K} + \delta \bm{M})
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix representations of these matrices and combine them using standard sparse H-matrix techniques. We factor the overall Gauss-Newton Hessian approximation, denoted $\bm{\preconditioner}$, using fast H-matrix methods, then and use the factorization as a preconditioner \cite{HLIBPRO}. 

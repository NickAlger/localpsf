...
Approximation \eqref{eq:translate1} follows from a smoothness assumption on the kernel $\Phi$. Operators satisfying \eqref{eq:translate1} can be well-approximated via low-rank interpolative factorization of the kernel $\Phi$.
% of the form
%\begin{equation}
...
However, such approximations are not suitable if $\Aop$ is high rank.
%, such as Hessians in inverse problems with highly informative data. 

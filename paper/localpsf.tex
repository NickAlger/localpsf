...
We approximate kernel entries $\Phi(y,x)$ by preconditioned radial basis function interpolation of known impulse response values
\begin{equation}
\label{eq:phi_mu_shift32}
	\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right).
\end{equation}
Interpolation using all sample points $x_i$ is too expensive, so we only use the $k$ nearest sample points to $x$, excluding points $x_i$ for which \eqref{eq:phi_mu_shift32} is not defined. That is, we exclude points $x_i$ such that $y - \mu(x) + \mu(x_i) \notin \Omega$. In numerical experiments we use $k=10$. The $k$ nearest sample points are found by querying a precomputed KD-tree [CITE] of all sample points. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) [CITE] of the mesh cells used to discretize the problem.

Once the $k$ nearest sample points, $\{x_i\}_{i=1}^k$, are found, we approximate $\Phi(y,x)$ using preconditioned radial basis function interpolation of the following form:
\begin{equation}
...
The point $y - \mu(x) + \mu(x_i)$ is typically not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints. Although it is advisable to avoid evaluating finite element functions at arbitrary points [CITE], here we have no choice. We evaluate $\frac{1}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
\label{eq:ellipsoid_check93}
	(y - \mu(x))^T \Sigma^{-1}(x) (y - \mu(x))  \le \tau^2.
\end{equation}
If \eqref{eq:ellipsoid_check93} does not hold, then $y - \mu(x) + \mu(x_i) \notin E_{x_i}$, so we use $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)=0$ for the purpose of interpolation. If \eqref{eq:ellipsoid_check93} does hold, then $y - \mu(x) + \mu(x_i) \in E_{x_i}$. In this case, we look up the batch index $b$ for the batch of sample points containing $x_i$, and evaluate
\begin{equation}
...
If $\Aop$ is symmetric, impulse responses of $\Aop$ are also impulse responses of $\Aop^T$. Let $\{y_i\}_{i=1}^k$ be the $k$ nearest sample points to $y$, excluding points $y_i$ such that $x - \mu(y) + \mu(y_i) \notin \Omega$. We could form an alternative approximation of $\Phi(y,x)$ by interpolating the values
\begin{equation}
\label{eq:phi_mu_shift_y}
\phi_{y_i}\left(x - \mu(y) + \mu(y_i)\right),
\end{equation}
in the manner described above, but with $x$ and $y$ now taking opposite roles. By combining both of these approximations, we can improve the interpolation accuracy at negligible additional cost. We combine both approximations by placing function values 
\begin{equation*}

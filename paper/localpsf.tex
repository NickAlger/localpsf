...
We approximate $\Phi(y,x) = \phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for a collection of sample points $x_i$ near $x$. The weights are determined by preconditioned radial basis function interpolation.
...
\section{Method}
\label{sec:method}
In this section we detail our method for computing the H-matrix approximation, $\mathbf{A}_H$, of $\Aop$. This section is organized as follows:
\begin{description}

...
\end{description}
If $\mathcal{A}$ were translation invariant---i.e., if the exact equality $A(y+h, x+h) = A(y,x)$ held for arbitrary $x$, $y$, $h$---then it is straightforward to show that $\mathcal{A}$ would take the form of a convolution operator $\left(\mathcal{A}u\right)(v) = \left\langle v, \varphi_p \ast u\right\rangle_{L^2(\Omega)}$. The convolution kernel,
\begin{equation*}
	\varphi_x(y) := A(y+x,x)
\end{equation*}
is the result of applying $\mathcal{A}$ to a point source at some fixed point $x$, then translating the resulting impulse response function re-center it at zero instead of $x$. Locally translation invariant operators therefore act like convolution operators locally, but the convolution kernel, $\varphi_x$, varies as $x$ changes.

...
\begin{algorithm2e}
	...
	{	
		...
		\For{$b=1,2,\dots,\text{max\_batches}$}{
			...
			Compute $\combresponse_b$ by applying $\Aop$ to the dirac comb for $S_b$ (Section \ref{sec:get_impulse_response})
...
\begin{theorem}[Impulse response moments]
	...
	\begin{equation*}
		\alpha(x) := \int_{\Omega} \impulseresponse_x(y) dy, \qquad
		\mu(x) := \mathbb{E}(\widehat{\impulseresponse}_x), \qquad
		\Sigma(x) := \Var(\widehat{\impulseresponse}_x),
	\end{equation*}
	where $\mathbb{E}(\widehat{\impulseresponse}_x)$ denotes the mean of $\widehat{\impulseresponse}_x$, and $\Var(\widehat{\impulseresponse}_x)$ denotes the variance of $\widehat{\impulseresponse}_x$.	Let  $x^i$ denote the $i^\text{th}$ component of $x$, i.e., $x = \left(x^1, x^2, \dots, x^d\right)$. Let $C$, $\{L^i\}_{i=1}^d$, and ${\{Q^{ij}\}_{i=1}^d}_{j=1}^d$ be the following constant, linear, and quadratic functions:
	\begin{equation*}
		C(x) := 1, \qquad
		L^i(x) := x^i, \qquad
		Q^{ij}(x) = x^i x^j.
...
\begin{algorithm2e}
	...
	{	
		...
		Compute $\alpha = \left(\Aop^T C\right)^*$
		
		\tcp{Compute mean $\mu$}
		\For{$i=1,2,\dots,d$}{
			Form linear function $L^i(x) = x^i$
			
			Compute $\mu^i = \left(\Aop^T L^i\right)^* / \alpha$
		}
		...
		\For{$i=1,2,\dots,d$}{
			\For{$j=1,\dots,i$}{
				...
				Compute $\Sigma^{ij} = \left(\Aop^T Q^{ij}\right)^* / \alpha - \mu^i\cdot \mu^j$
...
Once $p$ is selected, we remove $p$ from $P$. If $p$ is sufficiently far from all of the previously chosen points in the current batch, in the sense that $E_p \cap E_q = \{\}$ for all $q \in S_b$, then we add $p$ to $S_b$. Otherwise we discard $p$. This process repeats until there are no more points in $P$.  This is detailed in Algorithm \ref{alg:point_choice}. We determine whether $E_p \cap E_q = \{\}$ using the fast ellipsoid intersection test described in Appendix \ref{sec:fast_ellipsoid_intersection_test}.
...
We compute convolution kernels $\convkernel_{x_i}$ in batches by applying $\Aop$ to a small number Dirac combs. Each application of $\Aop$ to a Dirac comb yields one batch of impulse responses, $\impulseresponse_{x_i}$, which yields one batch of convolution kernels, $\convkernel_{x_i}$. The Dirac comb, $\dcomb_b$, associated with a batch of sample points, $S_b$, is the sum of Dirac distributions (point sources) centered at the points $x_i \in S_b$. That is,
\begin{equation*}
	\dcomb_b := \sum_{x_i \in S_b} \delta_{x_i}.
\end{equation*}
For each batch $S_b$, we compute the action of $\Aop$ on the associated Dirac comb:
\begin{equation}
...
\begin{equation}
	\label{eq:phi_b}
	\combresponse_b = \left(\Aop \left(\sum_{x_i \in S_b} \delta_{x_i}\right)^*\right)^* = \sum_{x_i \in S_b} \left(\Aop \delta_{x_i}^*\right)^* = \sum_{x_i \in S_b} \impulseresponse_{x_i}.
\end{equation}
...
\label{eq:varphi_eval}
	...
	\end{cases}
\end{equation}
for all $x_i \in S_b$. By performing one matrix-vector product, \eqref{eq:dirac_comb_H_action}, we recover the convolution kernels $\convkernel_{x_i}$ associated with every point $x_i \in S_b$. 
...
\begin{equation}
	...
	\begin{bmatrix} 
		1 & 0 & 1/\mu \\ 0 & 1 & -1/\mu^2 \\ 1 & \lambda_\text{max} & 1/(\lambda_\text{max}+\mu) 
	\end{bmatrix}
	\begin{bmatrix} 
		c_0 \\ c_1 \\ c_2 
	\end{bmatrix}
...
\begin{defn}[Moments of $\impulseresponse_x$]
	...
	We define the spatially varying scaling factor, $\alpha$, the scaled impulse response, $\widehat{\impulseresponse}_x$, the spatially varying mean, $\mu$, and the spatially varying covariance, $\Sigma$, as follows:
	\begin{align*}
		\alpha:\Omega \rightarrow \mathbb{R}, &\qquad \alpha(x) := \int_{\Omega} \impulseresponse_x(z) dz, \\
		\widehat{\impulseresponse}_x:\Omega \rightarrow \mathbb{R}, &\qquad \widehat{\impulseresponse}_x := \impulseresponse_x \big/ \alpha(x), \\
		\mu:\Omega \rightarrow \mathbb{R}^d, &\qquad \mu(x) := \int_\Omega z \widehat{\impulseresponse}_x(z) dz, \\
		\Sigma:\Omega \rightarrow \mathbb{R}^{d \times d}, &\qquad\Sigma(x) := \int_\Omega (z - \mu(x))(z - \mu(x))^T \widehat{\impulseresponse}_x(z) dz.
	\end{align*}
\end{defn}
...
\begin{proof}[Proof of Theorem \ref{thm:vol_mean_cov}]
	...
	\begin{align*}
		\left(v, \alpha\right)_{L^2(\Omega)} =& \int_\Omega v(x) \int_{\Omega} \left(\Aop \delta_x^*\right)^*(z) dz dx\\
		=& \int_\Omega v(x) \int_{\Omega} A(z,x) dz dx \\
		=& \int_\Omega \int_\Omega v(x) A(z,x) C(z) dz dx \\
		=& \left(\Aop^T C\right)(v),
	\end{align*}
	which implies $\alpha = \left(\Aop^T C\right)^*$.
	
	Using similar techniques, we have
	\begin{align*}
		\left(v, \alpha \cdot \mu^i\right)_{L^2(\Omega)} =& \int_\Omega v(x) \alpha(x) \int_{\Omega} z^i \left(\Aop \delta_x^*\right)^*(z)/\alpha(x) dz dx\\
		=& \int_\Omega \int_\Omega v(x) L^i(z) A(z,x) dz dx \\
		=& \left(\Aop^T L^i\right)(v),
	\end{align*}
	which implies $\mu^i = \left(\Aop^T L^i\right)^* / \alpha$. 
	
	Using the identity
	\begin{equation*}
		\Sigma(x) = \int_\Omega zz^T \widehat{\impulseresponse}_x(z) dz - \mu(x) \mu(x)^T,
	\end{equation*}
	we have
	\begin{align*}
		\left(v, \alpha \cdot \left(\Sigma^{ij}+\mu^i \cdot \mu^j\right)\right)_{L^2(\Omega)} =& \int_\Omega v(x) \alpha(x) \int_\Omega z^i z^j \left(\Aop \delta_x^*\right)^*(z) / \alpha(x) dz dx \\
		=& \int_\Omega \int_\Omega v(x) Q^{ij}(z) A(z,x) dz dx \\
		=& \left(\Aop Q^{ij}\right)(v),
	\end{align*}
	which implies $\Sigma^{ij} = \left(\Aop^T Q^{ij}\right)^* / \alpha - \mu^i\cdot\mu^j$.
\end{proof}
...
Let $\overline{\Omega}$ be the closure of $\Omega$, and let $C(\overline{\Omega})$ be the space of continuous functions mapping $\overline{\Omega}\rightarrow \mathbb{R}$. The action of $\Aop$ is extended to distributions $\mu:C\left(\overline{\Omega}\right) \rightarrow \mathbb{R}$ via the formula
\begin{equation*}
	\left(\Aop \mu^*\right)(v) := \int_\Omega v(y) \mu\left(A(y, \cdot)\right) dy, 
\end{equation*}
where $A(y,\cdot)$ is the function $x \mapsto A(y,x)$. This is derived formally as follows:
\begin{align*}
	\left(\Aop \mu^*\right)(v) &= \int_\Omega \int_\Omega v(y) A(y,x) \textrm{``} \mu(x) \textrm{''} dx dy \\
...
\begin{description}
	...
	\item[Distributions:] Let $\mu:C\left(\overline{\Omega}\right)\rightarrow \mathbb{R}$ be a distribution. If $\mu(\febasis_i)$ is well-defined for $i=1,\dots,N$, then the restriction of $\mu$ to domain $V_h$ is a linear functional $\mu_h\in V_h'$. The functional $\mu_h$ has a coefficient dual vector $\boldsymbol{\mu} \in \mathbb{R}^N_{\mathbf{M}^{-1}}$ with entries 
	\begin{equation*}
		\boldsymbol{\mu}_i=\mu(\febasis_i), \quad i=1,\dots,N.
	\end{equation*} 
...
\begin{equation*}
	\boldsymbol{\dcomb}^b_i = \sum_{x_j \in S_b} \febasis_i(x_j), \quad i=1,\dots,N.
\end{equation*}

...
but cannot easily access entries of $\Aop$'s integral kernel. Evaluating the maps in~\eqref{eq:eval_maps} may require solving a subproblem that involves PDEs, or performing other costly computations. By ``non-negative integral kernel,'' we mean that entries of the integral kernel (defined in Equation \ref{eq:kernel_representation}) are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$.

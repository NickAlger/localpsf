...
Note that while the domain of $\mathcal{A}$ is defined as $L^2(\Omega)$, which does not contain the delta distribution, the application of $\mathcal{A}$ is extended to distributions in a natural way, as described in Appendix \ref{app:distributions}. 
...
After discretization, the process of applying $\mathcal{A}$ to a delta distribution, then forming the Riesz representative, amounts to computing $\boldsymbol{\phi}_x = \mathbf{M}^{-1}\mathbf{A} \mathbf{M}^{-1} \boldsymbol{\delta}_{x_i}$, where $\boldsymbol{\phi}_x$, $\mathbf{A}$, and $\boldsymbol{\delta}_x$ are discretized versions of $\phi_x$, $A$, and $\delta_x$, respectively, and $\mathbf{M}$ is a mass matrix (e.g., in a finite element context). 
...
We compute one ``batch'' of $\varphi_i$'s by applying $\mathcal{A}$ to a Dirac comb, then post-processing the result. That is, we compute
\begin{equation}
\label{eq:eta_intro}
...
By linearity, and the expression for $\phi_x$ in \eqref{eq:impulse_response_delta_action}, we have
\begin{equation*}
\eta = \sum_{i=1}^l \phi_{x_i}.
\end{equation*}
Since the support of $\phi_{x_i}$ and the support of $\phi_{x_j}$ do not overlap, we may post-process $\eta$ to recover $\phi_i$, and therefore $\varphi_i$, for all of the points $x_i$ in the batch---by applying $\mathcal{A}$ to one distribution, we recover many $\varphi_i$. This is illustrated in Figure FIG. The process is repeated to get more batches of $\varphi_i$'s, until a desired number of batches is reached. 

...
@dataclass
class ProductConvolutionKernelSimplified:
    ...
    def __post_init__(me):
        assert_equal(me.col_coords.shape, (me.ndim_in, me.gdim_in))
        assert_equal(me.row_coords.shape, (me.ndim_out, me.gdim_out))
    ...
    @property
    def tau(me):
        return me.impulse_response_batches.tau

    @tau.setter
    def tau(me, new_tau):
        me.impulse_response_batches.tau = new_tau

    @property
    def num_neighbors(me):
        return me.impulse_response_batches.num_neighbors

    @num_neighbors.setter
    def num_neighbors(me, new_num_neighbors):
        me.impulse_response_batches.num_neighbors = new_num_neighbors
    ...
    @cached_property
    def ndim_in(me) -> int:
        return me.col_coords.shape[0]

    @cached_property
    def gdim_in(me) -> int:
        return me.col_coords.shape[1]

    @cached_property
    def ndim_out(me) -> int:
        return me.row_coords.shape[0]

    @cached_property
    def gdim_out(me) -> int:
        return me.row_coords.shape[1]

    @cached_property
    def shape(me) -> typ.Tuple[int, int]:
        return (me.ndim_out, me.ndim_in)


def make_product_convolution_kernel_simplified(
        col_batches: ImpulseResponseBatchesSimplified,
        col_coords: np.ndarray, # shape=(ndim_out, gdim_out)
        row_coords: np.ndarray # shape=(ndim_in, gdim_in)
) -> ProductConvolutionKernelSimplified:
    assert_equal(col_coords.shape, (col_batches.ndof_in, col_batches.gdim_in))
    assert_equal(row_coords.shape, (col_batches.ndof_out, col_batches.gdim_out))
    cpp_object = hpro.hpro_cpp.ProductConvolutionKernelRBFColsOnly(col_batches.cpp_object,
                                                                   col_coords, row_coords)
    return ProductConvolutionKernelSimplified(col_batches, cpp_object)

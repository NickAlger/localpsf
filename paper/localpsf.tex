...
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix representations of these matrices using standard sparse H-matrix techniques. Then we invert $\bm{M}^{-1}$, and multiply these matrices together using H-matrix methods. We factor the overall Gauss-Newton Hessian approximation, denoted $\widetilde{H}$, using fast H-matrix methods, then and use the factorization as a preconditioner \cite{HLIBPRO}. The resulting preconditioners for 1, 5, and 25 impulse response batches are denoted by PSF (1), PSF (5), and PSF (25), respectively. 

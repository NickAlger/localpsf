...
\begin{lem}[Preparation for Theorem \ref{thm:wi_alg_is_correct}]
	...
	\left(\psi_j, \Delta h\right)_{L^2(\Omega)} = h(x_j) - \avg(h)
	\end{equation}
...
\begin{proof}
	This follows from integration by parts and the definition of $\psi$ in \eqref{eq:psi_eq}.
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	which implies that the second term in \eqref{eq:L_w_38} is zero. Thus $\nabla_w \mathcal{L}(w, \lambda) = 0$. Since $\nabla \mathcal{L}(w, \lambda)=0$, Algorithm \ref{alg:weighting_functions_incremental} correctly produces the solution to optimization problem \ref{eq:wi_optimization_problem}.
\end{proof}

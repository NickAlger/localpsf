...
A popular choice for methods in this class is to choose the points $x_i$ to be nodes in a regular grid, and interpolate the functions $\varphi_i$ with piecewise linear interpolation [CITE NAGY], or splines [CITE SPLINES]. Adaptive refinement of the grid was proposed in [CITE], and we demonstrated a practical algorithm for adaptive refinement in our previous work [CITE MY PAPER]. The issue with these existing methods is that computing each $\varphi_i$ requires one matrix vector product with $\mathcal{H}$, and each of these matrix vector products is computationally costly. The key innovation in this paper is that we pick points $x_i$ in such a way that many $\varphi_i$ are computed with each matrix vector product. 

Our method of estimating the support of $\varphi_i$ was inspired by resolution analysis [CITE], in which the 

...
\end{equation}
where the weighting functions $w_i$ are the solutions to optimization problems \eqref{eq:wi_optimization_problem}. Then $u$ solves the optimization problem
\begin{equation}
...
\begin{proof}
	For notational convenience, in this proof we write $w$, $\lambda$, and $e$ instead of $w_i$, $\lambda_i$, and $e_i$, respectively. Here subscripts for $\lambda$, and $e$ instead denote components of those vectors. For example, here $\lambda_j$ denotes the $j^\text{th}$ component of the vector $\lambda$, which corresponds to the $j^\text{th}$ of the vector $\lambda_i$ in Algorithm \ref{alg:weighting_functions_incremental}.
	
	In this proof, we will repeatedly make use of the fact that
	\begin{equation}
	\label{eq:inline_wi_lemma}
	\left(\psi_j, \Delta h\right)_{L^2(\Omega)} = h(x_j)
	\end{equation}
	for all $h\in H^2_N(\Omega)$. This follows from integration by parts and the definition of $\psi$ in \eqref{eq:psi_eq}.
	...
	\begin{equation*}
		\mathcal{L}(w,\lambda) = \frac{1}{2}\|\Delta w\|^2 + \sum_{j=1}^r \lambda_j\left(e_j - w(x_j)\right).
	\end{equation*}
	...
	\begin{equation*}
		\nabla_{\lambda_j} \mathcal{L}(w, \lambda) = e_j - w(x_j) 
		= e_j - \alpha + \sum_{k=1}^r \lambda_k \theta_k(x_j).
	\end{equation*}
	Using \eqref{eq:inline_wi_lemma} with $h=\theta_k$, the equation for $\theta$ in \eqref{eq:theta_eq}, and the definition of $S$ in \eqref{eq:defn_of_S_alg}, we have
	\begin{align*}
	...
	\begin{equation*}
		\nabla_\lambda \mathcal{L}(w, \lambda) = e - \alpha \mathbf{1} + S \lambda = e - \alpha \mathbf{1} + S \left(-S^{-1}\left(e - \alpha \mathbf{1}\right)\right) = 0.
	\end{equation*}
	
	Taking the directional derivative of $\mathcal{L}$ with respect to $w$ in direction $h$ and using \eqref{eq:inline_wi_lemma} yields
	\begin{align*}
		\nabla_w \mathcal{L}(w, \lambda)h &= \left(\Delta w, \Delta h\right)_{L^2(\Omega)} - \sum_{j=1}^r \lambda_j h(x_j) \\
		&= \left(\Delta w, \Delta h\right)_{L^2(\Omega)} - \sum_{j=1}^r \lambda_j \left(\psi_j, \Delta h\right)_{L^2(\Omega)} \\
		&= \left(\Delta w - \sum_{j=1}^r \lambda_j \psi_j,~ \Delta h\right)_{L^2(\Omega)}.
	\end{align*}

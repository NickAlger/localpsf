...
\label{eq:local_mean_displacement_invariance}
...
\begin{equation}
\label{eq:fxyxp}
f_i := \frac{V(x)}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right), \qquad i=1,\dots,k,
\end{equation}
...
where $\{x_i\}_{i=1}^k$ are the $k$ nearest sample points to $x$, excluding sample points for which $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is undefined. That is, sample points such that $y-\mu(x)+\mu(x_i) \notin \Omega$. In numerical experiments we use $k=10$. We efficiently find the $k$ nearest sample points by querying a precomputed KD-tree \cite{kdtree} of all sample points, and check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) \cite{aabbtree} of the mesh cells used to discretize the problem. 
%Per the discussion in Section \ref{eq:local_mean_displacement_invariance}, $\Phi(y,x) \approx f_i$ when $x_i$ is close to $x$. The factor $\frac{V(x)}{V(x_i)}$, which was not discussed in Section \ref{eq:local_mean_displacement_invariance}, is a form of preconditioning that improves the quality of this approximation in situations where $V(x)$ varies widely. 

We approximate $\Phi(y,x)$
%compute the approximate kernel entry,
%\begin{equation*}
%\widetilde{\Phi}(y,x) \approx \Phi(y,x)
%\end{equation*}
by interpolating the (point,value) pairs
\begin{equation}
\left(x_i, f_i\right), \qquad i=1,\dots,k
\end{equation}
at the point $x$ using radial basis function interpolation.
...
In detail, we form the approximation $\widetilde{\Phi}(y,x) \approx \Phi(y,x)$ as follows:
\begin{equation}
\widetilde{\Phi}(y,x) := \sum_{i=1}^k w_i~ b\left(\|x-x_i\|\right).
\end{equation}
Here $b_i$ are radial basis functions and $w_i$ are weights. The vector of weights, $w = (w_1, w_2, \dots, w_k)^T$, is found as the solution to the $k \times k$ linear system
\begin{equation}
Bw = f,
\end{equation}
...
\end{cases}
\end{equation}
for $r>0$, and $b_i(0):=0$. For more details on radial basis function interpolation and polyharmonic splines, see \cite{rbf}.
%\begin{equation}
...
We evaluate $f_i$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
\label{eq:ellipsoid_check93}
(y - \mu(x_i))^T \Sigma^{-1}(x_i) (y - \mu(x_i))  \le \tau^2.
\end{equation}
If \eqref{eq:ellipsoid_check93} does not hold, then $y - \mu(x) + \mu(x_i)$ is outside the estimated support of $\impulseresponse_{x_i}$, so we set $f_i=0$. If \eqref{eq:ellipsoid_check93} does hold, then we look up the batch index $b$ such that $x_i \in S_b$, and evaluate
\begin{equation}
f_i = V(x)\eta_b\left(y - \mu(x) + \mu(x_i)\right),
\end{equation}
as per \eqref{eq:fxyxp} and \eqref{eq:varphi_eval}.
...
Note that $y - \mu(x) + \mu(x_i)$ is generally not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints. Hence,
evaluating $\eta_b\left(y - \mu(x) + \mu(x_i)\right)$, requires determining which mesh cell contains $y - \mu(x) + \mu(x_i)$, then evaluating the relevant finite element basis functions on that mesh cell. Fortunately, the mesh cell containing $y - \mu(x) + \mu(x_i)$ was already determined as a side effect of querying the AABB tree of mesh cells when we checked whether $y - \mu(x) + \mu(x_i) \in \Omega$. 

\subsubsection{Improved interpolation for symmetric operators}
\label{sec:symmetric_improvements}
If $\Aop$ is symmetric, impulse responses of $\Aop$ are also impulse responses of $\Aop^T$. Let $\{y_i\}_{i=1}^k$ be the $k$ nearest sample points to $y$, excluding points $y_i$ such that $x - \mu(y) + \mu(y_i) \notin \Omega$. We could form an alternative approximation of $\Phi(y,x)$ by performing the same interpolation as before, but with $x$ and $y$ taking opposite roles. We combine both of these interpolation schemes to improve the approximation accuracy at negligible additional cost. The two interpolation schemes are combined by placing function values 
\begin{equation*}

...
\end{equation*}
where $\{x_i\}_{i=1}^{\numnbr}$ are the $\numnbr$ nearest sample points to $x$, excluding sample points $x_i$ for which $z_i \notin \Omega$. Here $\numnbr$ is a small user defined parameter; e.g., $\numnbr=10$. To find the $\numnbr$ nearest sample points to $x$, we query a precomputed k-d tree \cite{Bentley75} of all sample points.  We check whether $z_i \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) \cite{Ericson04} of the mesh cells used to discretize the problem. Further, let
\begin{equation}
\label{eq:fxyxp}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{The ice sheet, discretized by a mesh of tetrahedra. Color indicates the height of the base of the ice sheet (i.e., the mountain topography). The radius of the domain is $10$ kilometers, the maximum height of the mountain is $2.1$ kilometers, and the average thickness of the ice sheet is 250 meters.}
...
We solve the inverse problem with an inexact Newton preconditioned conjugate gradient (PCG) scheme \cite{NOCEDAWRIGHT}, using $\basalfriction_0$ as the initial guess. The parameter is updated iteratively according to the formula
$
...
\end{equation}
and the step length $\alpha_k \in \mathbb{R}$ is chosen by line search to satisfy the Armijo conditions \cite{ARMIJO}. Here $g_k$ is the gradient of $J$, $H$ is the Hessian, and $H_\text{gn}$ is the Gauss-Newton Hessian, and these derivatives are evaluated at $\basalfriction_k$. 
...
We use $H_\text{gn}$ for the first five iterations, and $H$ for all subsequent iterations.  The Newton iteration terminates when $||g_k||/||g_0|| < 10^{-8}$. Systems \eqref{eq:newton_system} are solved inexactly using an inner PCG iteration, which terminates when the norm of the residual for the linear system is less than $\min \left(0.5, \sqrt{||g_k||}\right)||g_k||$. PCG only requires applying $H$ (or $H_\text{gn}$) to vectors. Each application of $H$ (or $H_\text{gn}$) to a vector requires solving one incremental forward and one incremental adjoint Stokes PDE of the form \eqref{Stokeseqn}, but with different right hand sides.
...
We use the framework described in this paper to generate Hessian preconditioners. We build H-matrix approximations of the data-misfit Gauss-Newton Hessian (the term in $H_\text{gn}$ that arises from the data misfit) using 1, 5, and 25 impulse responses batches. These approximations are labeled by are denoted by PSF (1), PSF (5), and PSF (25), respectively. The resulting H-matrices, which may be indefinite due to approximation error, are modified to be symmetric positive definite via the procedure described in Appendix \ref{sec:make_spd}, with $l=2$. We approximate $H_\text{gn}$ rather than $H$ because $H$ often has negative values in its integral kernel when one is away from the optimal point. Our results show that our preconditioners are also good preconditioners for $H$. The H-matrix approximation of data misfit Gauss-Newton Hessian is added to a H-matrix approximation of the Hessian of the regularization term to form a H-matrix approximation of $H_\text{gn}$. Straightforward calculation shows that the discretized Hessian of the regularization term is given by
$
(\gamma \bm{K} + \delta \bm{M}) \bm{M}^{-1} (\gamma \bm{K} + \delta \bm{M})
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix representations of these matrices and combine them using standard sparse H-matrix techniques. We factor the overall Gauss-Newton Hessian approximation, denoted $\widetilde{H}$, using fast H-matrix methods, then and use the factorization as a preconditioner \cite{HLIBPRO}. 
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Left (\ref{fig:generalized_eigenvalues}): eigenvalues for generalized eigenvalue problem $H u_k = \lambda_k \preconditioner_k$. Here $H$ is the Hessian and $\preconditioner$ is the PSF approximation constructed using the Gauss-Newton Hessian, $H_\text{gn}$, for 1, 5, and 25 batches (PSF (1), PSF (5), and PSF (25), respectively), or the regularization Hessian (REG). Right (\ref{fig:condition_number}): condition number, $\kappa$, for $\widetilde{H}^{-1} H$ and $\widetilde{H}^{-1} H_\text{gn}$ for these different preconditioners, and no preconditioner (NONE). All operators are evaluated at the $\basalfriction$ that solves the inverse problem for $\gamma=7.3 \times 10^3$, as determined by the Morozov discrepancy principle.}
\end{figure}

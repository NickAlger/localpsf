...
\end{equation}
The function $\Aker:\Omega \times \Omega \rightarrow \mathbb{R}$ is the integral kernel, which is not easily accessible to us. The linear functional $\Aop u \in L^2(\Omega)'$ is the result of applying $\Aop$ to $u\in L^2(\Omega)$, and $\left(\Aop u\right)(v)$ is the scalar that results from applying that linear functional to $v \in L^2(\Omega)$. Here $L^2(\Omega)'$ denotes the topological dual of $L^2(\Omega)$ (space of all continuous linear functionals).
...
\end{equation*}
the space $\mathbb{R}^N_\mathbf{M}$ is $\mathbb{R}^N$ with the inner product $(\mathbf{u},\mathbf{v})_\mathbf{M} := \mathbf{u}^T \mathbf{M} \mathbf{v}$, and $\mathbb{R}^N_{\mathbf{M}^{-1}}$ is the analogous space with $\mathbf{M}^{-1}$ replacing $\mathbf{M}$. Direct calculation shows that $\mathbf{R}^N_\mathbf{M}$ and $\mathbf{R}^N_{\mathbf{M}^{-1}}$ are isomorphic to $V_h$ and $V_h'$ as Hilbert spaces, respectively.

...
Our approximation method is defined by how we compute the convolution kernels, $\varphi_i$, (Section \ref{sec:get_impulse_response}) how we choose the points, $x_i$, (Section \ref{sec:choosing_sample_points}) and what weighting functions, $w_i$, that we use (Section \ref{sec:weighting_functions}). 
Briefly, we compute one ``batch'' of $\varphi_i$'s by applying $\mathcal{A}$ to a sum of point sources, $\delta_{x_i},$ at many points $x_i$ (a ``Dirac comb''). That is, we compute
\begin{equation*}

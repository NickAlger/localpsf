...
Operators satisfying \eqref{eq:translate1} can be well-approximated via low-rank interpolative factorization of $\Phi$.
% of the form
%\begin{equation}
...
the required rank in the low rank approximation can be large, which makes algorithms based on assumption \eqref{eq:translate1} computationally expensive. This is the case, for example, for Hessians in PDE constrained inverse problems with highly informative data, as discussed in Section \ref{sec:PDE_hessian_motivation}. 
...
Once the scattered impulse responses $\impulseresponse_{x_i}$ are computed, we approximate the integral kernel $\Phi(y,x)$ at arbitrary points $(y,x)$ by interpolation. The details of the interpolation will be described in Section \ref{sec:approximate_kernel_entries}, but the key principle behind the interpolation is the local mean displacement invariance assumption discussed in Section \ref{eq:local_mean_displacement_invariance}.
...
We approximate $\Phi(y,x) = \phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for a collection of sample points $x_i$ near $x$. The weights in the linear combination are determined via preconditioned radial basis function interpolation.
...
If $\Aop$ is symmetric, we take advantage of symmetry to construct a more accurate approximation at negligible additional cost. In this case, 
...
we combine information from impulse responses of both $\Aop$ and $\Aop^T$ in the interpolation procedure.
...
When $\Aop$ is positive semi-definite, $\mathbf{A}_H$ may be indefinite due to errors in the approximation. In this case, we modify the H-matrix representation of $\mathbf{A}_H$ to make it positive semi-definite, by applying a specially chosen rational matrix function to $\mathbf{A}_H$. This is described in Appendix \ref{sec:make_spd}.

...
where $\mathbf{\Phi} \in \mathbb{R}^{\fedim \times \fedim}$ is the matrix of kernel entries evaluated at pairs of Lagrange nodes:
\begin{equation}
...
\end{equation}
Note that $\mathbf{A}$ is not a Galerkin projection of $\Aop$ onto $V_h$ because of interpolation error in the kernel approximation $\Phi^h \approx \Phi$. 

%We neglect this interpolation error because it is the same magnitude as the finite element approximation error. 

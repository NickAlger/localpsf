...
restricted to the top surface of the geometry. We use $5\%$ noise in all cases, except for Figure \ref{fig:stokes_reconstructions} and Table \ref{fig:condition_number} where the noise is varied from $1\%$ to $25\%$. The true basal sliding
...
In Table~\ref{tab:condition_number}, we show the condition number of the preconditioned Hessian using our preconditioners, for a variety of noise levels ranging from $1\%$ to $25\%$. The condition number using our PSF preconditioners is extremely small (ranging between 1 and 10) and relatively stable over this range of noise levels. PSF (25) performs slightly better than PSF(5), which performs better than PSF (1), as expected. All PSF preconditioners outperform no preconditioning and regularization preconditioning for all noise levels. No preconditioning outperforms regularization preconditioning for all noise levels except $25\%$. We report condition number results for the preconditioned Hessian, $\mathbf{H}$, demonstrating that the PSF preconditioners built based on the Gauss-Newton Hessian, $\bm{H}_\text{gn}$, are also good preconditioner for $\bm{H}$.

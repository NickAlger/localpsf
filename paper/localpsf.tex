...
\end{equation*}
Then we use the high order rational function spectral projection method from \cite{HackbuschKress07} to replace the negative eigenvalues of $\mathbf{A}_H^{\text{sym}}$ with zero (or approximately zero) while keeping the positive eigenvalues unchanged (approximately unchanged).

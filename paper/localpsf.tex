...
In Algorithm \ref{alg:weighting_functions_incremental}, we show how to efficiently compute the functions $w_i$. Briefly:
\begin{enumerate}
...
\item We form $w_i$ as a linear combination of the functions $\theta_i$, and the constant function $C(x)=1$. Details for this linear combination are given in Algorithm \ref{alg:weighting_functions_incremental}. 
\end{enumerate}
In Theorem \ref{thm:wi_alg_is_correct}, we prove that the functions $w_i$ produced by this process indeed solve optimization problem \eqref{eq:wi_optimization_problem}.

Constructing all $r$ of the functions $w_i$ requires solving the Poisson equation $2r$ times with different right hand side sources, and these Poisson PDE solves can be performed cheaply with multigrid. All of the weighting functions, $\{w_i\}_{i=1}^r$, will change if a new point, $x_{r+1}$, is added. However, the dominant computational cost in constructing the weighting functions is the solution of the Poisson PDEs, and these Poisson PDE solves can be performed incrementally---by storing the functions $\psi_i$ and $\theta_i$, we only need to perform two new Poisson PDE solves when a new point is added.
...
%			Initialize $\delta_{x_i}$ as the point source (delta distribution) centered at $x_i$.
			...
			Solve the Neumann Poisson problem \eqref{eq:psi_eq} for $\psi_i$.
...
%			with condition $\int_\Omega \psi_i(x) dx = 0$, where $\nu$ is the normal vector to $\partial \Omega$.
			...
			Solve the Neumann Poisson problem \eqref{eq:theta_eq} for $\theta_i$.
...
\begin{proof}
	...
	\end{equation}
	is therefore a closed affine set in $H^2_N$, because it is the inverse image of a point under a continuous linear functional. Hence the feasible set for optimization problem \eqref{eq:generic_optimization_initial},
	\begin{equation}
	\label{eq:wi_feasible_set}
	X := \{u \in H^2_N : u(x_i)=f_i~\text{for}~ i=1,\dots,r\},
	\end{equation}
	is a closed affine set in $H^2_N$, because it is the intersection of the sets in \eqref{eq:inverse_image_of_point} for $i=1,\dots,r$. The feasible set $X$ is convex because it is affine, and it is non-empty because interpolating the data pairs $\{(x_i, f_i)\}_{i=1}^r$ using any smooth interpolation method yields an element of $X$. The objective function, $\gamma \mapsto \frac{1}{2}||\Delta \gamma||^2$, is convex because it 
	
	We now show that the objective function is strongly convex on the tangent space to $X$,
	\begin{equation*}

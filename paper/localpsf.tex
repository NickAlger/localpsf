...
In general, product-convolution approximations take the form \eqref{eq:product_convolution}, but $\varphi_i$ and $w_i$ may be arbitrary functions. Product-convolution approximations have been used in a wide variety of fields going back several decades [CITE BIG LIST].
For background on product-convolution approximations, we recommend reading the following excellent papers: [CITES]. 

We build upon a line of work in which the convolution kernels, $\varphi_i$, are impulse responses of $\mathcal{A}$ to point sources [CITE BIG LIST]. The primary question in methods of this type are how to choose the points $x_i$, and how to choose the weighting functions $w_i$. In [CITE NAGY], the impulse responses are sampled on a regular rectilinear grid, and interpolated with piecewise linear weighting functions, $w_i$, on a triangle mesh subordinate to that grid. Adaptive refinement of the grid was proposed in [CITE], and we demonstrated a practical algorithm for adaptive refinement in our previous work [CITE MY PAPER]. The issue with all of these existing methods, however, is computing each $\varphi_i$ requires one matrix vector product with $\mathcal{H}$, so that $r$ matrix-vector products are required. This is expensive when $\mathcal{H}$ is the Hessian in a PDE-constrained inverse problem, because each matrix vector product with $\mathcal{H}$ requires solving two PDEs. A key innovation in this paper is that we pick points $x_i$ in such a way that many $\varphi_i$ are computed with each matrix vector product with $\mathcal{H}$, thereby greatly reducing the number of matrix-vector products with $\mathcal{H}$ that are required. 
...
In [CITE], Escande and Weiss explored the idea of approximating linear operators from scattered impulse responses using a product-convolution scheme of the same form as \eqref{eq:product_convolution}. In our previous work [CITE MY PAPER], we choose the point locations with an adaptive grid. However,  

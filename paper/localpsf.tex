...
We solve the regularized least squares problem by QR factorization of the block $4k \times 2k$ matrix $\begin{bmatrix}\widetilde{B} & I\end{bmatrix}^T$. We regularize the problem because of numerical issues that arise if there are points $\widetilde{x}_i$ and $\widetilde{y}_j$ in the interpolation such that $\widetilde{x}_i \approx \widetilde{y}_j$, a situation that arises often in practice. Indeed, situations where $\widetilde{x}_i = \widetilde{y}_j$ are inevitable when a regular grid is used to discretize the problem. In this case, $\widetilde{B}$ is (nearly) singular, so it is numerically ill-advised to solve the standard interpolation equation $\widetilde{B} \widetilde{w} = \widetilde{f}$. 

...
We compute one ``batch'' of $\varphi_i$'s by applying $\mathcal{A}$ to a sum of point sources, 
\begin{equation*}
	\xi := \sum_{i=1}^l \delta_{x_i},
\end{equation*}
where the $\delta_{x_i}$'s are point sources (delta distributions) centered at a collection of scattered points $x_i$. That is, we compute
\begin{equation}
\label{eq:eta_intro}
	\eta := \left\langle \mathcal{A}, \xi \right\rangle^* = \sum_{i=1}^l A(~\cdot~, x_i).
\end{equation}
Here $\left\langle \mathcal{A}, \xi \right\rangle$ denotes the result of applying the operator $\mathcal{A}$ to the distribution $\xi$, and $\left\langle \mathcal{A}, \xi \right\rangle^*$ denotes the Riesz representation of $\langle\mathcal{A}, \xi\rangle$. The distribution $\xi$ is called a \emph{Dirac comb}. Although the domain of $\mathcal{A}$ is defined as $L^2(\Omega)$, which does not contain the Dirac comb distribution, the action of $\mathcal{A}$ is extended to distributions in a natural way, as described in Appendix \ref{app:distributions}. We take the Riesz representative of the linear functional $\left\langle \mathcal{A}, \xi \right\rangle \in L^2(\Omega)'$ to convert it into a function $\left\langle \mathcal{A}, \xi \right\rangle^* \in L^2(\Omega)$.  
In computations, the process of applying $\mathcal{A}$ to the Dirac comb, then forming the Riesz representative, amounts to computing $\boldsymbol{\eta} = \mathbf{M}^{-1}\mathbf{A} \mathbf{M}^{-1} \left(\sum_{i=1}^m \boldsymbol{\delta}_{x_i}\right)$, where $\boldsymbol{\eta}$, $\mathbf{A}$, and $\boldsymbol{\delta}_{x_i}$ are discretized versions of $\eta$, $A$, and $\delta_{x_i}$, respectively, and $\mathbf{M}$ is a mass matrix. 

The batch of points $x_i$ are chosen so that the support of $\left\langle \mathcal{A}, \delta_{x_i}\right\rangle^*$ and the support of $\left\langle \mathcal{A}, \delta_{x_j}\right\rangle^*$ do not overlap (or do not overlap much) if $i \neq j$. Since these supports do not overlap, we may post-process $\eta$ to recover $\varphi_i$ for all of the points $x_i$ in the batch---by applying $\mathcal{A}$ to one distribution, we recover many $\varphi_i$. This is illustrated in Figure FIG. The process is repeated to get more batches of $\varphi_i$'s, until a desired number of batches is reached. 

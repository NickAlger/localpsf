...
\begin{description}
\item[Matrix-free:] One cannot easily access kernel entries $A(y,x)$. Access to $\mathcal{A}$ is available only through application of $\mathcal{A}$ and $\mathcal{A}^T$ to arbitrary functions. That is, evaluation of the maps
\begin{equation*}
...
After discretization (see Appendix \ref{app:discretized_operations}), the operator $\mathcal{A}$ becomes a dense matrix, $\mathbf{A}$, that is typically too large to be built and stored. The first property is called ``matrix-free'' because at the discrete level it means that we can perform matrix-vector products of $\mathbf{A}$ and $\mathbf{A}^T$ with arbitrary vectors, but cannot easily access matrix entries $\mathbf{A}_{ij}$. In the matrix-free regime, to compute $\mathbf{A}_{ij}$ one could compute $\mathbf{v} = \mathbf{A}\mathbf{e}_j$, then extract the $i^\text{th}$ entry, $\mathbf{A}_{ij} = \mathbf{v}_i$. However, this process is wasteful because one computes the entire vector $\mathbf{v}$, then discards all but one component. Here $\mathbf{e}_j=(0,\dots,0,1,0,\dots,0)$ is the unit vector which has $j^\text{th}$ component equal to one and all other components equal to zero. In PDE constrained optimization and inverse problems, Hessians are typically available matrix free; we do not have direct access to these operators, but we can apply them to vectors via a process that involve solving two PDEs \cite{HESSIANADJOINT}. More generally, an operator is matrix-free if the process of applying the operator to a function involves solving a large linear system, using a timestepping procedure, or performing an iterative process.
...
\end{equation}
Here $\sum_{i=1}^m \delta_{x_i}$ is the Dirac comb distribution, $\left\langle \mathcal{A}, \sum_{i=1}^m \delta_{x_i} \right\rangle$ denotes the application of $\mathcal{A}$ to the Dirac comb, and $\left\langle \mathcal{A}, \sum_{i=1}^m \delta_{x_i} \right\rangle^*$ denotes the Riesz representation of the application of $\mathcal{A}$ to the Dirac comb. Although the domain of $\mathcal{A}$ is defined as $L^2(\Omega)$, which does not contain the Dirac comb distribution, the action of $\mathcal{A}$ may be extended to distributions in a natural way, as described in Appendix \ref{app:distributions}. We write $\langle \mathcal{A}, \xi \rangle$ to denote the application of $\mathcal{A}$ to the distribution $\xi$. Because $\mathcal{A}$ maps $L^2(\Omega) \rightarrow L^2(\Omega)'$, applying $\mathcal{A}$ to a function or distribution results in a linear functional. We take the Riesz representative of the linear functional $\left\langle \mathcal{A}, \sum_{i=1}^m \delta_{x_i} \right\rangle \in L^2(\Omega)'$ to convert it into a function $\left\langle \mathcal{A}, \sum_{i=1}^m \delta_{x_i} \right\rangle^* \in L^2(\Omega)$.  In computations, the process of applying $\mathcal{A}$ to the Dirac comb, then forming the Riesz representative, amounts to computing $\boldsymbol{\eta} = \mathbf{M}^{-1}\mathbf{A} \mathbf{M}^{-1} \left(\sum_{i=1}^m \boldsymbol{\delta}_{x_i}\right)$, where $\boldsymbol{\eta}$, $\mathbf{A}$, and $\boldsymbol{\delta}_{x_i}$ are discretized versions of $\eta$, $A$, and $\delta_{x_i}$, respectively, and $\mathbf{M}$ is a mass matrix. 

The batch of points $x_i$ are chosen so that the support of $\left\langle \mathcal{A}, \delta_{x_i}\right\rangle^*$ and the support of $\left\langle \mathcal{A}, \delta_{x_j}\right\rangle^*$ do not overlap (or do not overlap much) if $i \neq j$. Since the impulse responses do not overlap, we may post-process $\eta$ to recover $\varphi_i$ for all of the points $x_i$ in the batch---by applying $\mathcal{A}$ to one distribution, we recover many $\varphi_i$. This is illustrated in Figure FIG. The process is repeated to get more batches of $\varphi_i$'s, until a desired number of batches is reached. 
...
\end{equation}
where $\tau$ is a small number of standard deviations; we use $\tau=3$. 
...
The ellipsoid $E_x$ is the set of points within $\tau$ standard deviations of the mean of the Gaussian distribution which has the same mean and covariance as the normalized version of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$. 
The fraction of the mass of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$ residing outside of $E_x$ is less than $1/\tau^2$ by Chebyshev's inequality, though we find this bound is conservative and far less mass resides in this region in practice. 
...
In order to use the product-convolution approximation $\widetilde{\mathcal{A}}$, we convert $\widetilde{\mathcal{A}}$ to hierarchical matrix (H-matrix) format. Many classes of hierarchical matrices exist---H1, uniform H1, H2, HODLR, HSS, HBS, and more \cite{HMATRIX}. We use H1 matrices in our numerical results, but any of the other H-matrix formats could be used instead. The basic ideas behind H-matrices are as follows:
\begin{itemize} 
...
We convert $\widetilde{\mathcal{A}}$ into H1 matrix format using the standard geometrical clustering/adaptive cross method implemented within the HLIBpro software package \cite{HLIBPRO}. Briefly, this method uses geometric information about the spatial locations of the degrees of freedom associated with the rows and columns of the matrix to construct a recursive block partitioning of $\widetilde{\mathcal{A}}$, such that all blocks at the finest level of the partitioning are either low rank, or are small. Then, the method forms low rank approximations of the blocks that are expected to be low rank by sampling specially chosen rows and columns of those blocks. The small high rank blocks are formed directly by evaluating their entries. The primary cost of constructing the approximation is the evaluation of $O(N \log N)$ matrix entries of $\widetilde{\mathcal{A}}$, which we perform using Algorithm \ref{alg:eval_y_x}. We describe this H-matrix construction method in detail in Appendix \ref{app:h_matrix}.

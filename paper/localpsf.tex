...
\end{equation*}
is a Bilaplacian regularization term. Specifically, $\basalfriction_0$ is the constant function $\basalfriction_0(x)=10.5$, and $\mathcal{L}$ is the inverse of the solution operator for the following elliptic PDE:
\begin{align*}
...
\end{equation}
and the step length $\alpha_k \in \mathbb{R}$ is chosen to satisfy the Armijo conditions \cite{ARMIJO}. Here $g_k$ is the gradient of $J$, $H$ is the Hessian, and $H_\text{gn}$ is the Gauss-Newton Hessian, and these derivatives are evaluated at $\basalfriction_k$. 

...
\end{equation*}
The value of $\gamma$ satisfying the Morozov discrepancy principle is $\gamma=7.3 \times 10^3$. The exception is Figure \ref{fig:gamma_sweep}, where we vary $\gamma$ to study the effect of changing $\gamma$ on the quality of our preconditioner. The constant $\delta$ is chosen so that the correlation length, $L=\sqrt{\gamma/\delta}$, of functions drawn from the normal distribution with covariance $\mathcal{L}^{-2}$, is $1/10$th of the radius of the domain. The function $\basalfriction_0$ is the constant function with value $\basalfriction_0(x)=10.5$.
%, which is halfway between the maximum and minimum values of $\basalfriction_\text{true}$.

The inverse problem is solved with a inexact Newton preconditioned conjugate gradient (PCG) scheme \cite{NOCEDAWRIGHT}. The constant function $\basalfriction_0$ is used as the initial guess. The parameter value is updated iteratively according to the formula
$

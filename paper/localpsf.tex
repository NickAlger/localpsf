...
For background on product-convolution approximations, we recommend reading the following papers: [CITES]. 
...
\subsection{Background: discretization}
...
\begin{algorithm2e}
	...
	{	
		\For{$i=1,2, \dots, r$}{
			Compute $\psi_i$ as the solution to following Poisson problem with pure Neumann boundary conditions and a point source at $x_i$:
			$$\begin{cases}
			\Delta \psi_i = \delta_{x_i}, & \text{in }\Omega,\\
			\nu \cdot \nabla \psi_i = 0, & \text{on }\partial \Omega,\\
			\int_\Omega \psi_i(x) dx = 0.
			\end{cases}$$
			
			Compute $\theta_i$ as the solution to the following Poisson problem with pure Neumann boundary conditions and source $\psi_i$:
			$$\begin{cases}
			\Delta \theta_i = \psi_i, & \text{in }\Omega,\\
			\nu \cdot \nabla \theta_i = 0, & \text{on }\partial \Omega,\\
			\int_\Omega \theta_i(x) dx = 0.
			\end{cases}$$
		}

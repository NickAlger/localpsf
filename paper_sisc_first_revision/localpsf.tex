...
Here we define a PDE-free spatially varying blurring problem, used in
Sections~\ref{sec:intro} and~\ref{sec:prelims} for visual illustration
of various stages and aspects of the proposed method.  To study the
...
this example is set up so that the impulse response, $\phi_x$,
corresponding to the integral kernel is a bumpy blob that is centered
near $x$, and is rotated and scaled in a manner that depends on $x$
(see Figure \ref{fig:frog_kernel_impulse_responses}). The closed form
expression of the integral kernel is given by
%
...
each method. The results reveal that superior performance of the PSF

...
If $\Aop$ is symmetric positive definite, we symmetrize $\mathbf{A}_H$ as follows
\begin{equation*}
...
\begin{equation*}
	\lambda_i < c < 0.
\end{equation*}
These outlying eigenpairs may be computed using a Krylov method, or the randomized singular value decomposition.
We then flip the sign of these ``bad'' negative eigenvalues to be positive as follows:
\begin{equation*}
...
The cutoff $c$ is required when $\Aop$ has a large number of zero eigenvalues, or a sequence of eigenvalues that cluster at zero, a situation that occurs commonly in practice. We seek to correct negative eigenvalues caused by errors in our approximation procedure. But even without our approximation, numerical rounding and discretization errors may cause large numbers of zero or near-zero eigenvalues to become slightly negative. Numerically, we cannot hope to correct these small magnitude negative eigenvalues because there are so many of them, and because the spectral gap between the smallest negative eigenvalue and the first non-negative eigenvalue is small. 

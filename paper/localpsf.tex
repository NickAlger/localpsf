...
If $\mathcal{A}$ were translation invariant---i.e., if the exact equality $A(y+h, x+h) = A(y,x)$ held for arbitrary $x$, $y$, $h$---then it is straightforward to show that $\mathcal{A}$ would take the form of the convolution operator $\left(\mathcal{A}u\right)(v) = \left\langle v, \varphi_p \ast u\right\rangle_{L^2(\Omega)}$. Here $f \ast g$ denotes convolution of the functions $f$ and $g$. The convolution kernel,
\begin{equation*}
	\varphi_x(y) := A(y+x,x),
\end{equation*}
is the result of applying $\mathcal{A}$ to a point source at some fixed point $x$, then translating the resulting impulse response function re-center it at zero instead of $x$. Locally translation invariant operators act like convolution operators locally, but the convolution kernel, $\varphi_x$, varies as $x$ changes. We therefore sample $\varphi_{x_i}$ at a collection of points $\{x_i\}_{i=1}^r \subset \Omega$, and patch together the local convolution operators $u \mapsto \varphi_{x_i} \ast u$ using smooth weighting functions $w_i(x)$. This yields a product-convolution approximation, $\widetilde{\mathcal{A}} \approx \mathcal{A}$, given by
\begin{equation*}
...
\end{equation*}
Here $f \cdot g$ denotes pointwise multiplication of the functions $f$ and $g$.

While kernel entries $\mathcal{A}(y,x)$ are not easily computable, kernel entries $\widetilde{A}(y,x)$ are given by the formula
\begin{equation*}

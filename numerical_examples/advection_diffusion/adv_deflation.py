...
# X_impulse, Y_impulse = np.meshgrid(np.linspace(0.0, 1.0, 11), np.linspace(0.0, 1.0, 11))
# impulse_points = np.array([X_impulse.reshape(-1), Y_impulse.reshape(-1)]).T.copy()
impulse_points = np.array([[0.1, 0.8]])
np.savetxt(save_dir_str + '/impulse_points.txt', impulse_points)

krylov_threshold = 1e-6

krylov_iters_none = np.ones((len(all_noise_levels),
                             len(all_kappa),
                             len(all_t_final)), dtype=int) * np.nan

krylov_iters_reg = np.ones((len(all_noise_levels),
                            len(all_kappa),
                            len(all_t_final)), dtype=int) * np.nan

krylov_iters_psf = np.ones((len(all_num_batches),
...
for ii, ns in enumerate(all_noise_levels):
    for jj, kp in enumerate(all_kappa):
        for kk, tf in enumerate(all_t_final):
            ...
            plt.colorbar(cm)
            plt.axis('off')
            plt.title('Noisy observations' + str(tf))
            ...
            iter_count_none = np.min(np.argwhere(np.array(errs_none) < krylov_threshold))
            iter_count_reg = np.min(np.argwhere(np.array(errs_reg) < krylov_threshold))

            krylov_iters_none[ii, jj, kk] = iter_count_none
            krylov_iters_reg[ii, jj, kk] = iter_count_reg
            np.save(save_dir_str + '/krylov_iters_none', krylov_iters_none)
            np.save(save_dir_str + '/krylov_iters_reg', krylov_iters_reg)
            ...
            for ll, num_batches in enumerate(all_num_batches):
                ...
                np.savetxt(save_dir_str + '/krylov_iters_psf', krylov_iters_psf)

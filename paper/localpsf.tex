...
\label{eq:product_convolution}
	\left(\AopPc~u\right)(v) := \left( v,~ \sum_{i=1}^r \convkernel_i \ast \left(w_i \cdot u\right) \right)_{L^2(\Omega)},
\end{equation}
...
\begin{equation}
	...
	\AkerPc(y,x) = \sum_{i=1}^r \convkernel_i(y-x) w_i(x).
\end{equation}
...
\begin{algorithm2e}
	...
	{	
		...
		\For{$b=1,2,\dots,\text{max\_batches}$}{
			Choose a batch of sample points, $\pointbatch_b$, using Algorithm \ref{alg:point_choice}
			
			Compute $\combresponse_b$ by applying $\Aop$ to the dirac comb for $\pointbatch_b$ (Section \ref{sec:get_impulse_response})
...
\begin{theorem}[Impulse response moments]
	...
	\end{equation*}
	where $\widehat{\impulseresponse}_x := \impulseresponse_x / \spatialvol(x)$ denotes the normalized version of $\impulseresponse_x$, $\mathbb{E}(\widehat{\impulseresponse}_x)$ denotes the mean of $\widehat{\impulseresponse}_x$, and $\Var(\widehat{\impulseresponse}_x)$ denotes the variance of $\widehat{\impulseresponse}_x$.	Let  $x^i$ denote the $i^\text{th}$ component of $x$, i.e., $x = \left(x^1, x^2, \dots, x^d\right)$. Let $C$, $\{L^i\}_{i=1}^d$, and ${\{Q^{ij}\}_{i=1}^d}_{j=1}^d$ be the following constant, linear, and quadratic functions:
	\begin{equation*}
		C(x) := 1, \qquad
		L^i(x) := x^i, \qquad
		Q^{ij}(x) = x^i x^j.
...
\begin{algorithm2e}
	...
	\Input{Finite set of candidate points $P\subset \Omega$, \\spatialmean $\spatialmean(x)$ and covariance $\spatialcov(x)$, \\domain boundary information $\partial \Omega$, \\previous sample point batches $\pointbatch_1, \dots, \pointbatch_{b-1}$}
	\Output{Batch of new sample points $\pointbatch_b$}
	
	Initialize empty new batch of sample points, $\pointbatch_b = \{\}$
	
	\While{$P$ is not empty}{
		Determine the point $p \in P$ that is farthest from all points in the previous sample point batches $\pointbatch_1,\dots,\pointbatch_{b-1}$
		
		Remove $p$ from $P$

	
		\If{$E_p \cap E_q \neq \{\}$ for all $q \in \pointbatch_b$ and $E_p \cap \partial \Omega = \{\}$}{
			\tcp{$E_p$ and $E_q$ are the ellipsoids defined in \eqref{eq:support_ellipsoid}}
			
			Add $p$ to $\pointbatch_b$
	...
	\caption{Choosing one batch of sample points, $\pointbatch_b$}
...
We compute convolution kernels $\convkernel_{x_i}$ in batches by applying $\Aop$ to a small number Dirac combs. Each application of $\Aop$ to a Dirac comb yields one batch of impulse responses, $\impulseresponse_{x_i}$, which yields one batch of convolution kernels, $\convkernel_{x_i}$. The Dirac comb, $\dcomb_b$, associated with a batch of sample points, $\pointbatch_b$, is the sum of Dirac distributions (point sources) centered at the points $x_i \in \pointbatch_b$. That is,
\begin{equation*}
	\dcomb_b := \sum_{x_i \in \pointbatch_b} \delta_{x_i}.
\end{equation*}
For each batch $\pointbatch_b$, we compute the action of $\Aop$ on the associated Dirac comb:
\begin{equation}
	\label{eq:dirac_comb_H_action}
	\combresponse_b := \left(\Aop \dcomb_b^*\right)^*.
\end{equation}
By linearity, the resulting function $\combresponse_b$ may be written as
\begin{equation}
	\label{eq:phi_b}
	\combresponse_b = \left(\Aop \left(\sum_{x_i \in \pointbatch_b} \delta_{x_i}\right)^*\right)^* = \sum_{x_i \in \pointbatch_b} \left(\Aop \delta_{x_i}^*\right)^* = \sum_{x_i \in \pointbatch_b} \impulseresponse_{x_i}.
\end{equation}
...
\begin{enumerate}
	...
	\begin{equation}
		\label{eq:optimal_v}
		v_*(x) := \beta + \sum_{i=1}^r c_i \secondgreens_i(x),
	\end{equation}
	...
	\begin{equation}
		...
		\end{bmatrix}.
	\end{equation}
	Here $\mathbf{c} \in \mathbb{R}^r$ is the vector with $i$th component $c_i$, $\mathbf{1} \in \mathbf{R}^r$ is the column vector of all ones, $\mathbf{\interpolatedvalues} \in \mathbb{R}^r$ is the vector with $i$th component $\interpolatedvalues_i$, and $S \in \mathbb{R}^{r \times r}$ is the matrix with entries given by
	\begin{equation}
		\label{eq:S_matrix}
		S_{ij} := \left( \firstgreens_i, \firstgreens_j \right)_{L^2(\Omega)}.
	\end{equation}
\end{enumerate}
In Theorem \ref{thm:wi_no_breakdown}, we prove that this procedure does not break down, so long as there is no repetition in the points $x_i$. Linear system \eqref{eq:interpolation_linear_system} is the KKT system that arises if one solves optimization problem \eqref{eq:wi_optimization_problem_generic} on the $r+1$-dimensional subspace spanned by the functions $\firstgreens_i$, $i=1,\dots,r$, and the constant function $C(x)=1$, rather than solving the optimization problem on the infinite dimensional space $H^2_N(\Omega)$. In Theorem \ref{thm:wi_alg_is_correct}, we prove that the function $v_*$ produced by this process solves optimization problem \eqref{eq:wi_optimization_problem_generic} on $H^2_N(\Omega)$.

Constructing all $r$ of the functions $w_i$ thus requires solving the Poisson equation $2r$ times with different right hand side sources to form the functions $\firstgreens_i$ and $\secondgreens_i$, then post-processing these functions. This is shown in Algorithm \ref{alg:weighting_functions_incremental}. The Poisson PDE solves, which are the most expensive part of this procedure, can be performed cheaply with multigrid. 
...
\begin{thm}[Procedure for computing $v_*$ does not break down]
	...
	If all the sample points $x_i$, $i=1,\dots,r$, are distinct, then PDE \eqref{eq:psi_eq} is uniquely solvable for $\firstgreens_i$, PDE \eqref{eq:theta_eq} is uniquely solvable for $\secondgreens_i$, and linear system \eqref{eq:interpolation_linear_system} is uniquely solvable for $\mathbf{c}$ and $\beta$. That is, the procedure for computing $v_*$ does not break down. Furthermore, $v_* \in H^2_N(\Omega)$.
\end{thm}

\begin{proof}
	Existence and uniqueness for the solutions to PDEs \eqref{eq:psi_eq} and \eqref{eq:theta_eq} follow from the standard elliptic PDE theory. By standard elliptic regularity theory, $\firstgreens_i \in H^1(\Omega)$, so $\firstgreens_i \in L^2(\Omega)$, and so all matrix entries $S_{ij}$ are finite. 
	
	Using the definition of $S$ and basic linear algebra, it is straightforward to show that if $S$ were singular, there would exist constants $a_1, a_2, \dots a_r$, which are not all zero, such that
	\begin{equation*}
		0 = a_1 \firstgreens_1 + a_2 \firstgreens_2 + \dots a_r \firstgreens_r.
	\end{equation*}
	Applying the Laplacian to this equation yields
	\begin{equation*}
		0 = a_1 \delta_{x_1} + a_2 \delta_{x_2} + \dots a_r \delta_{x_r} - \left(\sum_{i=1}^r a_i\right)\bigg/|\Omega|,
	\end{equation*}
	which is impossible because a nonzero linear combination of delta distributions at different points cannot equal a constant function. Thus we must conclude that $S$ is non-singular. By block matrix factorization, non-singularity of $S$ implies that linear system \eqref{eq:interpolation_linear_system} is uniquely solvable for $\mathbf{c}$ and $\beta$.
...
\begin{proof}
	Let
	\begin{equation*}
		X := \{v \in H^2_N(\Omega) : v(x_i) = \interpolatedvalues_i \text{ for }i=1,\dots,r\}
	\end{equation*}
	...
	\paragraph{Feasibility} To show that $v_*$ is feasible (i.e., $v_* \in X$), we must show that $v_*(x_i) = \interpolatedvalues_i$ for $i=1,\dots,r$. We have
	\begin{align*}
		v_*(x_i) &= \beta + \sum_{j=1}^r c_j \secondgreens_j(x_i) \\
		&= \beta + \sum_{j=1}^r c_j \left(\firstgreens_i, \Delta \secondgreens_j\right)_{L^2(\Omega)} + \avg\left(\secondgreens_j\right) \\
		&= \beta + \sum_{j=1}^r c_j \left(\firstgreens_i, \firstgreens_j\right)_{L^2(\Omega)} \\
		&= \beta + \sum_{j=1}^r c_j S_{ij}
	\end{align*}
	In the first line we used the definition of $v_*$ in \eqref{eq:optimal_v}. In the second line we used Lemma \ref{lem:psi_delta_eval}. In the third line we used the facts that, by construction, $\Delta \secondgreens_j = \firstgreens_j$ and $\int_\Omega \secondgreens_j(x) dx = 0$. In the fourth line we used the definition of $S$ in \eqref{eq:S_matrix}. Hence the condition $v_*(x_i) = \interpolatedvalues_i$ for $i=1,\dots,r$ may be written in matrix form as
	\begin{equation*}
		S \mathbf{c} + \beta \mathbf{1} = \mathbf{\interpolatedvalues},
	\end{equation*}
	...
	\begin{align*}
		(\Delta v_*, \Delta p)_{L^2(\Omega)} =& \sum_{i=1}^r c_i(\Delta \secondgreens_i, \Delta p)_{L^2(\Omega)} \\
...
\begin{thm}[Poisson weighting functions are optimal]
	...
	Let $y \in \mathbb{R}^d$, $d \in \{1,2,3\}$, and let $\horizinterpolant_y$ be the function
	\begin{equation}
		\label{eq:u_defn_in_thm}
		\horizinterpolant_y(x) := \sum_{i=1}^r \convkernel_i(y)w_i(x) = \AkerPc\left(y+x,x\right),
	\end{equation}
...
\begin{algorithm2e}
	...
	{	
		...
		\For{$i=1,2,\dots,r$}{
			...
			\begin{equation*}
			w_i(x) = \beta^{(i)} + \sum_{k=1}^r c_k^{(i)} \secondgreens_k(x).
			\end{equation*}
...
Let $\febasis_1, \febasis_2, \dots, \febasis_N$ be a set of finite element basis functions used to discretize the problem on a mesh with mesh size parameter $h$, and let $V_h := \Span\left(\febasis_1, \febasis_2, \dots, \febasis_N\right)$ be the corresponding finite element space. Further, let $p_i \in \mathbb{R}^d$, $i=1,\dots, N$ be the Lagrange nodes associated with the functions $\febasis_i$. That is, the points such that $\febasis_i(p_j)$ equals one if $i=j$, and zero otherwise.
...
\begin{equation}
	\label{eq:eval_y_x}
	\AkerPc(y,x) = \sum_{i=1}^r w_i(x) \convkernel_{x_i}(y-x).
\end{equation}
We now define a further approximation, $\AkerPcMesh \approx \AkerPc$, as follows:
\begin{equation}
	\label{eq:defn_of_Akerpcmesh}
	\AkerPcMesh(y,x) := \sum_{i=1}^N \sum_{j=1}^N \AkerPc(p_i,p_j) \febasis_i(y) \febasis_j(x).
\end{equation}
...
\end{align*}
where $\spatialmean_p, \spatialmean_q \in \mathbb{R}^d$, and $\spatialcov_p, \spatialcov_q \in \mathbb{R}^{d \times d}$ are positive definite. Let $K$ be the following one dimensional convex function:
\begin{equation*}
...
Let 
\begin{equation*}
	g(\ratpole) := c_0(\ratpole) + c_1(\ratpole) \lambda_\text{min} + \frac{c_2(\ratpole)}{\lambda_\text{min}+\ratpole} - |\lambda_\text{min}|
\end{equation*}
denote the left hand side of \eqref{eq:rational_extra_eq}, but with $c_0(\ratpole)$, $c_1(\ratpole)$, and $c_2(\ratpole)$ being the solution of \eqref{eq:rational_3_linear_system} for the given value of $\ratpole$. I.e., within $g(\ratpole)$ the quantities $c_0$, $c_1$, and $c_2$ are no longer free variables; they now depend on $\ratpole$ implicitly through the solution of a linear system. We solve the combined system, \eqref{eq:rational_3_linear_system} and $\eqref{eq:rational_extra_eq}$, by solving the one-dimensional nonlinear root finding problem
\begin{equation}
	\label{eq:g_mu_zero}
	g(\ratpole)=0
\end{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:smoothest_interpolant}]
	Let $f = (\convkernel_1(y), \convkernel_2(y), \dots, \convkernel_r(y))$ and let $e_i = (0,\dots,0,1,0,\dots,0)$ be the length $r$ vector with $i^\text{th}$ component equal to one and all other components equal to zero. The solution to optimization problem \eqref{eq:uy_optimization_problem} is given by
	\begin{align*}
		\mathcal{T}(f) &= \convkernel_1(y) \mathcal{T}(e_1) + \convkernel_2(y) \mathcal{T}(e_2) + \dots + \convkernel_r(y) \mathcal{T}(e_r) \\
		&= \convkernel_1(y) w_1 + \convkernel_2(y) w_2 + \dots + \convkernel_r(y) w_r \\
		&= \horizinterpolant_y.
	\end{align*}
...
\begin{defn}[Moments of $\impulseresponse_x$]
	...
	\begin{align*}
		...
		\spatialmean:\Omega \rightarrow \mathbb{R}^d, &\qquad \spatialmean(x) := \int_\Omega z \widehat{\impulseresponse}_x(z) dz, \\
		\spatialcov:\Omega \rightarrow \mathbb{R}^{d \times d}, &\qquad\spatialcov(x) := \int_\Omega (z - \spatialmean(x))(z - \spatialmean(x))^T \widehat{\impulseresponse}_x(z) dz.
	\end{align*}
\end{defn}
...
Let $\febasis_i$, $i=1,\dots,N$ be a set of finite element basis functions, let $h$ denote the mesh size parameter for the finite element mesh, and let
\begin{equation*}
	V_h = \Span\left(\febasis_1, \febasis_2, \dots, \febasis_N\right)
\end{equation*}
...
\begin{description}
	...
	\begin{equation*}
		\boldsymbol{\sigma}_i = \sigma_h(\febasis_i), \quad i=1,\dots,N.
	\end{equation*}
	...
	\begin{equation*}
		\left(\mathbf{g}^\pi\right)^*_i = \int_\Omega g(x) \febasis_i(x) dx, \quad i=1,\dots,N.
	\end{equation*}
	...
	\end{equation*}
	\item[Finite element interpolation:] Let $p_i \in \mathbb{R}^d$, $i=1,\dots,N$, be the Lagrange nodes associated with the finite element basis functions $\febasis_i$. The finite element interpolant of a function $g \in C\left(\overline{\Omega}\right)$ is the function $g^I \in V_h$ given by
	\begin{equation*}
		g_h^I(x) = \sum_{i=1}^N g(p_i) \febasis_i(x).
	\end{equation*}
	...
	\item[Distributions:] Let $\genericdistribution:C\left(\overline{\Omega}\right)\rightarrow \mathbb{R}$ be a distribution. If $\genericdistribution(\febasis_i)$ is well-defined for $i=1,\dots,N$, then the restriction of $\genericdistribution$ to domain $V_h$ is a linear functional $\genericdistribution_h\in V_h'$. The functional $\genericdistribution_h$ has a coefficient dual vector $\boldsymbol{\genericdistribution} \in \mathbb{R}^N_{\mathbf{M}^{-1}}$ with entries 
	\begin{equation*}
		\boldsymbol{\genericdistribution}_i=\genericdistribution(\febasis_i), \quad i=1,\dots,N.
	\end{equation*} 
	...
	\begin{equation*}
		\left(\boldsymbol{\delta}_p\right)_i = \febasis_i(p), \quad i=1,\dots,N.
	\end{equation*}
...
\begin{algorithm2e}
	...
	{	
		...
		\Output{$\boldsymbol{\spatialvol}\in \mathbb{R}^N$, $\boldsymbol{\spatialmean}\in \mathbb{R}^{d \times N}$, and $\boldsymbol{\spatialcov} \in \mathbb{R}^{d \times d \times N}$}
...
In computations, $\spatialvol$, $\spatialmean^{i}$, and $\spatialcov^{ij}$ are replaced with finite element approximations $\spatialvol_h, \spatialmean_h^i, \spatialcov_h^{ij}\in V_h$, respectively, which have coefficient vectors $\boldsymbol{\spatialvol}, \boldsymbol{\spatialmean}^{ij}, \boldsymbol{\spatialcov}^{ij}\in\mathbb{R}^N_\mathbf{M}$, respectively. The functions $C$, $L^i$, and $Q^{ij}$ are replaced with either finite element projections onto $V_h$ (if the finite element basis does not satisfy the Kronecker property), or finite element interpolations onto $V_h$ (if the finite element basis satisfies the Kronecker property). In our numerical examples we use finite element basis functions that satisfy the Kronecker property, and we use finite element interpolations. The coefficient vectors corresponding to either the interpolations or projections are denoted $\mathbf{C}$, $\mathbf{L}^i$, and $\mathbf{Q}^{ij}$, respectively. The discretized versions of \eqref{eq:vol_mean_var_thm1}, \eqref{eq:vol_mean_var_thm2}, and \eqref{eq:vol_mean_var_thm3} are
\begin{align*}
...
In computations, the Dirac comb coefficient dual vector for $\dcomb_h^b$ is the vector $\boldsymbol{\dcomb}^b \in \mathbb{R}^N_{\mathbf{M}^{-1}}$ which has entries
\begin{equation*}
	\boldsymbol{\dcomb}^b_i = \sum_{x_j \in \pointbatch_b} \febasis_i(x_j), \quad i=1,\dots,N.
\end{equation*}
The Dirac comb impulse response coefficient vector for $\combresponse^h_b$ is the vector $\boldsymbol{\combresponse}_b \in \mathbb{R}^N_\mathbf{M}$ given by
\begin{equation*}
	\boldsymbol{\combresponse}_b = \mathbf{M}^{-1} \mathbf{A}^T \mathbf{M}^{-1}\boldsymbol{\dcomb}^b.
\end{equation*}

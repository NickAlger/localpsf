...
Here we provide an overview of the operator approximation algorithm. Then, in the subsections of this section (Sections \ref{sec:sample_point_selection}--\ref{sec:flip_eigs}) we present the technical details of the algorithm that were not already covered in Section \ref{sec:prerequisites}.
...
If high accuracy is not required (for example, if the method is being used to construct a preconditioner) we recommend a conservative choice of $\epsilon=3.0$ in order to make the method more robust to nonsmoothness of the kernel, discretization error, errors caused by violations of our assumptions, and issues related to boundaries. We use $\epsilon=3.0$ in our numerical results involving Hessians for this reason. In our numerical results for blur kernels, we use a smaller value of $\epsilon=0.5$ so that the RBF interpolation accuracy is not a limiting factor as we study convergence of the method.
...
\subsection{Example 1: Spatially variant blur}
\label{sec:frog}

Before delving into technical details, we provide an overview of the method. Along the way, we illustrate steps using a spatially variant blur example, in which the integral kernel is given as follows:
\begin{equation}
\label{eq:frog_kernel}
\Phi(y,x) = (1 - a f(y,x)) g(x) \exp \left(-\frac{1}{2}h^T(y,x) C^{-1} h(y,x)\right).
\end{equation} 
Here, $h(y,x) = R(\theta(x)) (y-x)$, $R(\theta(x))$ is the 2D rotation matrix by angle $\theta(x) = (x^1 + x^2) \pi / 2$, $C = L^2 \operatorname{diag}(c_1, c_2)$, $g(x) = x^1(1-x^1)x^2(1-x^2)$,  and $f(x,y) = \cos\left(p^1(y,x) / \sqrt{c_1 / 2.0}\right) \sin\left(p^2(y,x) / \sqrt{c_2 / 2.0}\right)$ for some constants $a, L, c_1, c_2$. In this section we use $a=1.0$, $L^2=1.0$, $c_1=0.01$, and $c_2=0.0025$. Using this kernel, the impulse response $\phi_x$ is a bumpy blob that is centered near $x$, and is rotated and scaled in a manner that depends on $x$ (see Figure \ref{fig:frog1}). The constant $L$ controls how wide the blob is, and $a$ controls the bumpiness. With $a=0$, $\phi_x$ is a Gaussian. As $a$ increases, $\phi_x$ becomes more bumpy. When $a > 1$, the integral kernel contains negative values, which violates the assumptions of our method. Note that for this operator, using our method is unnecessary because entries of the kernel are given directly via a formula. The purpose of this example is to illustrate the method using an operator which is known exactly and may be easily visualized.
...
\begin{table}
	...
	\end{center}
	\caption{(Spatially variant blur) Comparison of cost to approximate the blur kernel from Equation \eqref{eq:frog_kernel} using the PSF method, the randomized HODLR (hierarchical off diagonal low rank) method, and GLR (global low rank) approximation using randomized SVD. The quantity $L$ scales the width of the impulse responses, hence it influences the rank of the operator. Large $L$ means low rank, and small $L$ means high rank. The second column (``Error tol'') is the relative error in the approximation of the kernel measured in the Frobenius norm, $||\Phi - \widetilde{\Phi}||_\text{Fro} / ||\Phi||_\text{Fro}$. The remaining three columns show the number of operator applies required to achieve the given error tolerances, using the PSF, HODLR, and GLR methods.}

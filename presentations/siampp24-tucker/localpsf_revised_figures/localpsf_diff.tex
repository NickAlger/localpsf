...
%DIF DEL localpsf_submitted.tex   Sun Feb 18 15:33:51 2024
%DIF ADD localpsf.tex             Mon Feb 19 14:51:41 2024
...
We present an efficient \emph{matrix-free} point spread function (PSF) method for approximating operators $\Aop:L^2(\Omega) \rightarrow L^2(\Omega)'$ that have locally supported non-negative integral kernels. Here $\Omega \subset \mathbb{R}^\gdim$ is a bounded domain, and $L^2(\Omega)'$ is the space of real-valued continuous linear functionals on $L^2(\Omega)$. \DIFaddbegin \DIFadd{By ``non-negative integral kernel,'' we mean that entries of $\Aop$'s integral kernel 
%DIF > (defined in Equation \ref{eq:kernel_representation})
are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$. }\DIFaddend Such operators appear \DIFdelbegin \DIFdel{, for instance, as }\DIFdelend \DIFaddbegin \DIFadd{as, for example, }\DIFaddend Hessians in optimization and inverse problems governed by partial differential equations (PDEs)~\cite{BorziSchulz11,Delosreyes15,HinzePinnauUlbrichUlbrich08}, 
%AntilEtAl18
Schur complements in Schur complement methods for solving partial differential equations \DIFdelbegin \DIFdel{(PDEs) }\DIFdelend and Poincare-Steklov operators in domain decomposition methods (e.g., Dirichlet-to-Neumann maps)~\cite{ChanMathew94,SmithBjorstadGropp96,ToselliWidlund04}, covariance operators in spatial statistics~\cite{ChenStein21,GentonKeyesTurkiyyah18,GeogaEtAl20,LindgrenRueLindstrom11}, and blurring operators in imaging~\cite{DenisEtAl11,NagyOleary98}. 
\DIFdelbegin \DIFdel{Here }\DIFdelend \DIFaddbegin 

\DIFadd{Here, }\DIFaddend ``matrix-free'' means that we may apply $\Aop$ and its transpose\footnote{Recall that $\Aop^T:L^2(\Omega)\rightarrow L^2(\Omega)'$ is the unique operator satisfying $\left(\Aop u\right)(w) = \left(\Aop^T w\right)(u)$ for all $u,w \in L^2(\Omega)$, where $\Aop u \in L^2(\Omega)'$ is the result of applying $\Aop$ to $u\in L^2(\Omega)$, and $\left(\Aop u\right)(w)$ is the result of applying that linear functional to $w \in L^2(\Omega)$, and similar for operations with $\Aop^T$. }, $\Aop^T$, to functions,
\begin{equation}
\label{eq:eval_maps}
u \mapsto\Aop u \qquad \text{and} \qquad w \mapsto \Aop^T w,
\end{equation} 
via a black box computational procedure, 
but cannot easily access entries of $\Aop$'s integral kernel. Evaluating the maps in~\eqref{eq:eval_maps} may require solving a subproblem that involves PDEs, or performing other costly computations. \DIFdelbegin \DIFdel{By ``non-negative integral kernel,'' we mean that entries of the integral kernel are non-negative numbers; this is not the same as positive semi-definiteness of $\Aop$}\DIFdelend \DIFaddbegin \DIFadd{Matrix-free does not mean that matrices are never used; our approximation is ultimately converted into a hierarchical matrix (H-matrix), a compressed matrix format that permits fast and scalable linear algebra operations}\DIFaddend . 
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \begin{figure}
	\begin{center}
		%DIF > 	\begin{subfigure}[b]{0.58\textwidth}
		%DIF > 		\centering
		\includegraphics[scale=0.35]{kernel_impulse_illustration2.pdf}
		%DIF > %		\caption{Kernel and impulse responses}
		%DIF > 		\label{fig:kernel_impulse_illustration}
		%DIF > 	\end{subfigure}
		%DIF > 	\begin{subfigure}[b]{0.41\textwidth}
		%DIF > 		\centering
		%DIF > 		\includegraphics[scale=0.55]{frog_ellipsoid_width2.pdf}
		%DIF > %		\caption{Impulse response support ellipsoid}
		%DIF > 		\label{fig:frog_ellipsoid_width}
		%DIF > 	\end{subfigure}
	\end{center}

	\caption{\DIFaddFL{Left: Matrix created by evaluating the integral kernel $\Phi$ for $\Aop$ (Equation~\ref{eq:kernel_representation}) at all pairs of mesh vertices. This illustration is for the integral kernel given in Equation~\ref{eq:frog_kernel} in Section~\ref{sec:frog}. Dark colors indicate large entries and light colors indicate small entries. Rows and columns are ordered according to a kd-tree hierarchical clustering. Right: Impulse responses
%DIF > 	$\phi_{x_1}=\Phi(~\cdot~, x_1)$ and $\phi_{x_1}=\Phi(~\cdot~, x_2)$
...
\DIFadd{The idea of the proposed PSF-based method is to }\DIFaddend use \emph{impulse response interpolation} to form a high rank approximation of $\Aop$ using a small number of operator applications. The impulse response, $\phi_x$, associated with a point, $x$, is the Riesz representation\footnote{Recall that the Riesz representative of a functional $\rho \in L^2(\Omega)'$ with respect to the $L^2$ inner product is the unique function $\rho^* \in L^2(\Omega)$ such that $\rho(w) = \left(\rho^*,w\right)_{L^2(\Omega)}$ for all $w \in L^2(\Omega)$.} of the linear functional that results from applying $\Aop$ to a delta distribution (i.e., point source, impulse) centered at $x$. We compute batches of impulse responses by applying $\Aop$ to weighted sums of delta distributions associated with batches of points scattered throughout the domain (see Figure~\ref{fig:batches_intro}). \DIFdelbegin \DIFdel{Then we interpolate translated and scaled versions of these impulse responses to approximate entries of the operator's integral kernel . 
Picking the batchesof points requires us to estimate the supports of the impulse responses $\phi_x$ }\emph{\DIFdel{before}} %DIFAUXCMD
\DIFdel{they are computed. Ellipsoid }\DIFdelend \DIFaddbegin \DIFadd{Batches of impulse responses may be thought of intuitively as sets of ``columns'' of the kernel (Figure \ref{fig:frog_kernel_impulse_responses}). To choose the batches, we form ellipsoid }\DIFaddend estimates for the supports of all $\phi_x$ \DIFdelbegin \DIFdel{are determined a-priori }\DIFdelend via a moment method \DIFaddbegin \DIFadd{(Figure~\ref{fig:frog_moments_ellipsoid}) }\DIFaddend that involves applying $\Aop^T$ to a small number of polynomial functions (see Section~\ref{sec:intromoments}). We \DIFdelbegin \DIFdel{are inspired by resolution analysis in seismic imaging, in which
$\Aop^T$ is applied to a random noise function, and the width of the support $\phi_x$ is estimated to be the autocorrelation length of the resultant function near $x$~\mbox{%DIFAUXCMD
\cite{FichtnerLeeuwen15,TrampertFichtnerRitsema13}}\hskip0pt%DIFAUXCMD
.
The moment method that we use estimates the support of $\phi_x$ more accurately than random noise probing in resolution analysis, at the cost of the additional constraint that $\Aop$ has a non-negative integral kernel }\DIFdelend \DIFaddbegin \DIFadd{then use a greedy ellipsoid packing algorithm (Figure~\ref{fig:frog_batches}) to maximize the number of impulse responses per batch.
%DIF >  which requires us to estimate the supports of the impulse responses $\phi_x$ \emph{before} they are computed. 
Then we interpolate translated and scaled versions of these impulse responses to approximate entries of the operator's integral kernel (Figure~\ref{fig:hmatrix_neighbors_frog})}\DIFaddend . Adding more batches yields impulse responses at more points, increasing the accuracy of the approximation at the cost of one operator application per batch \DIFaddbegin \DIFadd{(Figure~\ref{fig:frog_column_errors})}\DIFaddend . 
...
%DIF > 		
%DIF > 		Impulse response moments are used to choose batches of non-overlapping impulse responses by solving an ellipsoid packing problem using a greedy algorithm. Batches of impulse responses 
%DIF > 		
%DIF > 		Moments of the impulse responses used to form ellipsoid shaped a-priori estimates for the supports of the impulse responses.
	}}
...
\begin{figure}
	\begin{subfigure}[b]{0.30\textwidth}
		\centering
		\includegraphics[scale=0.35]{frog_column_errors_psf5.png}
		%DIF > 		\caption{PSF(5)}
		\label{fig:blur_error_psf5}
	\end{subfigure}
	\begin{subfigure}[b]{0.30\textwidth}
		\centering
		\includegraphics[scale=0.35]{frog_column_errors_psf10.png}
		%DIF > 		\caption{PSF(10)}
		\label{fig:blur_error_psf10}
	\end{subfigure}
	\begin{subfigure}[b]{0.30\textwidth}
		\centering
		\includegraphics[scale=0.35]{frog_column_errors_psf20.png}
		%DIF > 		\caption{PSF(20)}
		\label{fig:blur_error_psf20}
	\end{subfigure}
	\begin{subfigure}[b]{0.08\textwidth}
		\centering
		\includegraphics[scale=0.35]{frog_colorbar_edited1.pdf}
		%DIF > 			\label{fig:blur_colorbar}
	\end{subfigure}

	\caption{\DIFaddFL{Relative error, $||\Phi(\cdot,x) - \widetilde{\Phi}(\cdot,x)||/||\Phi(\cdot,x)||$, in the approximation of the ``column'' of the integral kernel associated with $x$, using 5 (left), 10 (center) and 20 (right) impulse response batches. Sample points are indicated by black dots. The error associated with the point $x$ is the shade of the image at location $x$, with white indicating zero error and black indicating $100\%$ error. At the sample points, the error is zero. The further the point $x$ is from the samples points, the larger the error. Adding more batches yields a more accurate approximation.
%DIF > 		columnwise error plots for kernel reconstruction left 5, center 10, right 20
	}}
	\label{fig:frog_column_errors}
\end{figure}


\DIFaddend The proposed operator approximation method may be categorized as a PSF method that is loosely based on ``product convolution'' (PC) approximations, which are approximations of an operator by weighted sums of convolution operators with spatially varying weights. PC and PSF methods have a long history dating back several decades. We note the following papers (among many others) in which the convolution kernels are constructed from sampling impulse responses of the operator to scattered point sources:~\cite{Adorf94,AlgerEtAl19,BigotEscandeWeiss19,EscandeWeiss15,EscandeWeiss22,EscandeWeiss12,FishEtAl96,NagyOleary98,ZhuLiFomelEtAl16}. For background on PC and PSF methods, we recommend the following papers: \cite{DenisEtAl15,EscandeWeiss17,GentileCourbinMeylan13}. The proposed method improves upon existing PC methods in the following ways: (1) While PC approximations are based on an assumption of local translation invariance, the method we propose is based on a more general assumption we call ``local mean displacement invariance'' (Section~\ref{sec:local_mean_displacement_invariance} and Figure~\ref{fig:mean_displacement_invariance}), which improves the interpolation of the impulse responses. (2) In our previous work~\cite{AlgerEtAl19}, we chose point sources \DIFdelbegin \DIFdel{optimally }\DIFdelend \DIFaddbegin \DIFadd{in an adaptive grid }\DIFaddend via a sequential \DIFdelbegin \DIFdel{adaptive procedure}\DIFdelend \DIFaddbegin \DIFadd{procedure; the refinements to the adaptive grid were chosen to maximally reduce the error at each step}\DIFaddend . However, in that work each point source required a separate operator application, making the previous method expensive when a large number of impulse responses is desired. In this paper, we use a new moment method (Section \ref{sec:intromoments}) which permits computation of many impulse responses (e.g., 50) per operator application. \DIFaddbegin \DIFadd{We are inspired by resolution analysis in seismic imaging, in which
$\Aop^T$ is applied to a random noise function, and the width of the support $\phi_x$ is estimated to be the autocorrelation length of the resultant function near $x$~\mbox{%DIFAUXCMD
\cite{FichtnerLeeuwen15,TrampertFichtnerRitsema13}}\hskip0pt%DIFAUXCMD
. The moment method that we use estimates the support of $\phi_x$ more accurately than random noise probing in resolution analysis, at the cost of the additional constraint that $\Aop$ has a non-negative integral kernel. }\DIFaddend (3) The method we propose never evaluates computed impulse responses outside of their domain of definition. This eliminates \DIFdelbegin \DIFdel{boundary artifacts which }\DIFdelend \DIFaddbegin \DIFadd{``boundary-artifact'' errors (see \mbox{%DIFAUXCMD
\cite[Section 1.1]{AlgerEtAl19}}\hskip0pt%DIFAUXCMD
) that }\DIFaddend plague conventional PC methods.
...
In this work, we form a hierarchical matrix\DIFdelbegin \DIFdel{(H-matrix)}\DIFdelend ~\cite{BormGrasedyckHackbusch03,Hackbusch99} approximation of a discretized version of $\Aop$. H-matrices are a \DIFaddbegin \DIFadd{compressed }\DIFaddend matrix format in which the rows and columns of the matrix are re-ordered, then the matrix is recursively subdivided into blocks in such a way that many off-diagonal blocks are low rank, even though the matrix as a whole may be high rank. 
...
The work and memory required to perform these operations for an $\fedim \times \fedim$ H-matrix \DIFdelbegin \DIFdel{scales 
}\emph{\DIFdel{nearly linearly}} %DIFAUXCMD
\DIFdel{in $N$ (i.e., $o(N^{1+\epsilon})$ for any $\epsilon > 0$). The exact cost depends }\DIFdelend \DIFaddbegin \DIFadd{with rank $\hrank$ blocks scales as $O\left(\hrank^a \fedim \log(\fedim)^b\right)$ where $a,b \in \{0,1,2,3\}$ are constants which depend }\DIFaddend on the type of H-matrix used \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the operation being performed\DIFdelbegin \DIFdel{, and the rank of the off-diagonal blocks~\mbox{%DIFAUXCMD
\cite{GrasedyckHackbusch03}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{GrasedyckHackbusch03}}\hskip0pt%DIFAUXCMD
\mbox{%DIFAUXCMD
\cite[Section 2.1]{Kriemann13}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
\DIFaddbegin \DIFadd{While the PSF-based method in this paper may be used to approximate any operator that has a locally supported non-negative integral kernel, we are primarily motivated by approximation of high-rank Hessians in distributed parameter inverse problems governed by PDEs. In this section, we provide a brief background on this topic, and explain why existing Hessian approximation methods are not satisfactory.
}

\DIFaddend In distributed parameter inverse problems governed by PDEs, one seeks to infer an unknown spatially varying parameter field from limited observations of a state variable that depends on the parameter implicitly through the solution of a PDE. 
...
the required number of operator applications is still large (e.g., hundreds to thousands). \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{For example, using the method in \mbox{%DIFAUXCMD
\cite{LinYing11}}\hskip0pt%DIFAUXCMD
, the required number of operator applies to construct an $H^1$ matrix with hierarchical rank $r$ for problems in a 2D domain discretized with a regular grid is $\#\text{levels} \cdot 64 \cdot (r + c)$, where $\#\text{levels}$ is the depth of the hierarchical partitioning, $r$ is the rank of the blocks (hierarchical rank), and $c$ is an oversampling parameter (see \mbox{%DIFAUXCMD
\cite[Section 2.4]{LinYing11}}\hskip0pt%DIFAUXCMD
). On a $64 \times 64$ grid with depth $4$, hierarchical rank $10$, and oversampling parameter $c=5$, this works out to $4 \cdot 64 \cdot (10 + 5) = 3840$ operator applies. In Section \ref{sec:frog} see numerically that the randomized hierarchical off-diagonal low rank (HODLR) method in \mbox{%DIFAUXCMD
\cite{Martinsson16} }\hskip0pt%DIFAUXCMD
requires hundreds to thousands of matrix-vector products to construct approximations of the integral kernel for a blur problem example with modest (e.g., $10\%$) relative error.
%DIF > while the PSF method in this paper can achieve the same level of error in one to two dozen operator applies.
...
In }\DIFaddend this paper, we also form an H-matrix \DIFdelbegin \DIFdel{Hessian }\DIFdelend approximation. However, to reduce the required number of operator applications, we first form a PSF approximation of the data misfit Hessian by exploiting locality and non-negative integral kernel properties, then form the H-matrix using classical \DIFdelbegin \DIFdel{H-matrix techniques. By using }\DIFdelend \DIFaddbegin \DIFadd{techniques. Using }\DIFaddend this two stage approach, we reduce the number of operator applications to a few dozen at most. 
\DIFdelbegin \DIFdel{One limitation of our method is that not }\DIFdelend \DIFaddbegin 

\DIFadd{Not }\DIFaddend all data misfit Hessians satisfy the local non-negative integral kernel properties\DIFdelbegin \DIFdel{(for example, }\DIFdelend \DIFaddbegin \DIFadd{. We note, in particular, that }\DIFaddend the wave inverse problem data misfit Hessian \DIFdelbegin \DIFdel{has a substantial amount }\DIFdelend \DIFaddbegin \DIFadd{and Gauss-Newton Hessian have a substantial proportion }\DIFaddend of negative entries in \DIFdelbegin \DIFdel{its integral kernel). }\DIFdelend \DIFaddbegin \DIFadd{their integral kernels. In this case more specialized techniques have been developed using, eg., pseudodifferential operator theory \mbox{%DIFAUXCMD
\cite{DemanetEtAl12,Symes08}}\hskip0pt%DIFAUXCMD
, and sparsity in the wavelet domain \mbox{%DIFAUXCMD
\cite{HerrmannMoghaddamStolk08}}\hskip0pt%DIFAUXCMD
. }\DIFaddend However, many data misfit Hessians of practical interest do satisfy \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the local non-negative integral kernel }\DIFaddend properties (either exactly or approximately), and the proposed method is targeted at approximating these Hessians.


\section{Preliminaries}
\DIFaddbegin \label{sec:prelims}
\DIFaddend 
...
The operator $\Aop$ may be applied to distributions\DIFaddbegin \footnote{\DIFadd{I.e., generalized functions such as the Dirac delta distribution. See, for example, \mbox{%DIFAUXCMD
\cite[Chapter 5]{ArbogastBona08}}\hskip0pt%DIFAUXCMD
.}} \DIFaddend if $\Aker$ is sufficiently regular. 
...
\end{equation}
that is formed by applying $\Aop$ to $\delta_x$ \DIFaddbegin \DIFadd{(per the generalized notion of operator ``application'' defined above)}\DIFaddend , then taking the Riesz representation of the resulting linear functional. Using~\eqref{eq:action_on_distribution} and the definition of the delta distribution, we see that $\phi_x$ may also be written as the function $\impulseresponse_x(y) = \Aker(y, x)$.
...
\begin{enumerate}
	...
	\item The integral kernel is non-negative\footnote{Note that having a non-negative integral kernel is different from positive semi-definiteness. The operator $\Aop$ need not be positive semi-definite to use the proposed method, and positive semi-definite operators need not have a non-negative integral kernel.} in the sense that $$\Aker(y,x) \ge 0 \quad \text{for all} \quad (y,x) \in \Omega \times \Omega.$$
\end{enumerate}
The proposed method may still perform well if these conditions are relaxed slightly. 
\DIFaddbegin 

%DIF > If these conditions are violated, the method will incur additional error, and depending on the severity of the violation, the proposed method may fail. 

\DIFaddend It is acceptable if the support of $\impulseresponse_x$ is not perfectly contained in a localized region \DIFaddbegin \DIFadd{(violating Assumption 2)}\DIFaddend , so long as the bulk of the ``mass'' of $\impulseresponse_x$ is contained in a localized region. \DIFdelbegin \DIFdel{The integral kernel does not need to be non-negative for all pairs of points $(y,x) \in \Omega \times \Omega$ so long as it is non-negative for the vast majority of pairs of points $(y,x)$, and so long as }\DIFdelend \DIFaddbegin \DIFadd{In principle, the proposed method can be applied even if the impulse responses are not localized. However, in this case only a small number of impulse responses can be computed per batch, which means more batches, and hence more operator applies, are needed to form an accurate approximation.
}

%DIF > The integral kernel does not need to be non-negative for all pairs of points $(y,x) \in \Omega \times \Omega$ (Assumption 3) so long as it is non-negative for the vast majority of pairs of points $(y,x)$, and so long as the negative numbers are comparatively small. 

\begin{figure}
	\centering
	{
		%DIF > 	\setlength{\tabcolsep}{0.1em}
		%DIF > 	\renewcommand{\arraystretch}{2.0}
		\begin{tabular}{cccc}
			\includegraphics[scale=0.3, valign=c]{frog_ellipsoid_a=1.0.png} & 
			\includegraphics[scale=0.3, valign=c]{frog_1d_a=1.0.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_ellipsoid_a=0.0.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_1d_a=0.0.png} \\
			%DIF > 
			\includegraphics[scale=0.3, valign=c]{frog_ellipsoid_a=20.0.png} & 
			\includegraphics[scale=0.3, valign=c]{frog_1d_a=20.0.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_ellipsoid_a=0.23.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_1d_a=0.23.png} \\
			%DIF > 
			\includegraphics[scale=0.3, valign=c]{frog_ellipsoid_a=27.0.png} & 
			\includegraphics[scale=0.3, valign=c]{frog_1d_a=27.0.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_ellipsoid_a=0.249.png} & 
			\includegraphics[scale=0.3, valign=c]{ricker_1d_a=0.249.png} 
		\end{tabular}
	}
	\caption{\DIFaddFL{Illustration of the influence of negative numbers in the integral kernel on the robustness of the ellipsoid estimates for the supports of impulse responses. Left two columns: Blur kernel given in Equation \ref{eq:frog_kernel}. Right two columns: Ricker wavelet-type kernel given by $\Aker(y,x) = (1 - a \gamma)\exp\left(-\gamma/2\right)$, where $\gamma=(y-x)^T \Sigma^{-1} (y-x)$, and $\Sigma=\operatorname{diag}\left(0.0025, 0.01\right)$. Ordered from top to bottom, the results are obtained with $a\in\{1.0, 20.0, 27.0\}$ for the left two columns, and $a\in \{0.0, 0.23, 0.249\}$ for the right two columns. Columns 1 and 3: impulse responses with estimated support ellipsoids indicated by the black ellipses. Red and blue represent positive and negative numbers in the integral kernel, respectively. Columns 2 and 4: one-dimensional slice along the horizontal line indicated in the two-dimensional plots. The dashed gray line is at zero.
	}}
	\label{fig:robustness_to_negatives}
\end{figure}

\DIFadd{If there are negative numbers in the integral kernel (violating Assumption 3), the ellipsoid estimation procedure may incur errors or fail, leading to poor performance or failure of }\DIFaddend the \DIFaddbegin \DIFadd{proposed method. In Figure \ref{fig:robustness_to_negatives} we investigate the robustness of the ellipsoid support estimation procedure to violations of Assumption 3. We study two integral kernel examples, both of which are parameterized by a quantity that controls how negative the kernels are. We make the following observations:
}\begin{itemize}
	\item \DIFadd{The larger and more numerous the }\DIFaddend negative numbers are\DIFdelbegin \DIFdel{comparatively small.
	If these conditions areviolated, the method will incur additional error, and depending on the severity of }\DIFdelend \DIFaddbegin \DIFadd{, the more inaccurate the ellipsoid support estimate is.
	}\item \DIFadd{The further away from the center of the ellipsoid the negative numbers are, }\DIFaddend the \DIFdelbegin \DIFdel{violation, the proposed method may fail. }\DIFdelend \DIFaddbegin \DIFadd{more influence they have on the quality of the ellipsoid support estimate. This is because moment formulas (Equations \ref{eq:define_mean} and \ref{eq:define_cov}) assign more weight to entries in the kernel that are further from the center.
	}\item \DIFadd{Negative numbers in the integral kernel affect the proposed method more if they are isolated, and less if they are balanced by nearby positive numbers.
	}\item \DIFadd{As kernels become more negative, the ellipsoid estimation performs well up to a certain threshold that depends on the spatial distribution of negative and positive entries. After that threshold is crossed, the estimation rapidly transitions to performing poorly and ultimately failing.
}\end{itemize} 
%DIF > The location of the negative numbers also affects the accuracy of the ellipsoid estimates substantially, but in a more complicated manner. 
\DIFadd{For the kernel in the left two columns of Figure \ref{fig:robustness_to_negatives}, negative numbers are interspersed with positive numbers, allowing us to include a large amount of negative numbers in the integral kernel before the proposed method fails. For the kernel in the right two columns, the ellipsoid estimate fails with tiny amounts of negative numbers because the negative numbers are far away from the mean and not balanced by positive numbers at similar distances and angles. In the bottom two rows, we see the aforementioned threshold effect, in which the proposed method rapidly transitions from performing reasonably well to performing poorly with only a small change to the integral kernel. 
}\DIFaddend 

%DIF > Location and magnitude of negatives matter. Larger negatives: bad, smaller negatives good.  Negatives closer to center: good, further from center: bad. Correlated negatives: bad, randomly distributed: good. Good Good Good Fail quick
%DIF > 
%DIF > Non-local: will work, but choose few ellipsoids, so inefficient
\DIFaddbegin 



\DIFaddend \subsection{Finite element discretization}
\label{sec:finite_element_kernel}

In computations, functions are discretized and replaced by finite-dimensional vectors, and operators mapping between infinite-dimensional spaces are replaced by operators mapping between finite-dimensional spaces. In this paper we discretize \DIFdelbegin \DIFdel{using continuous Galerkin }\DIFdelend \DIFaddbegin \DIFadd{the functions that $\mathcal{A}$ and $\mathcal{A}^T$ are applied to using continuous }\DIFaddend finite elements satisfying the Kronecker property (defined below). With minor modifications, the proposed method could be used with more general finite element methods, or other discretization schemes such as finite differences or finite volumes. \DIFaddbegin \DIFadd{These restrictions on discretization only apply to functions $u$ that $\mathcal{A}$ and $\mathcal{A}^T$ are applied to. Other functions that arise internally during the process of computing actions of $\mathcal{A}$, such as state variables in a PDE that is solved in a subproblem, may be discretized with any method.
}\DIFaddend 

Let $\febasis_1, \febasis_2, \dots, \febasis_\fedim$ be a set of continuous \DIFdelbegin \DIFdel{Galerkin }\DIFdelend finite element basis functions used to discretize the problem on a mesh with mesh size parameter $h$, let $V_h := \Span\left(\febasis_1, \febasis_2, \dots, \febasis_\fedim\right)$ be the corresponding finite element space under the $L^2$ inner product, and let $\fepoint_i \in \mathbb{R}^\gdim$, $i=1,\dots, \fedim$ be the Lagrange nodes associated with the functions $\febasis_i$. We assume that the finite element basis satisfies the Kronecker property, i.e., $\febasis_i(\fepoint_i)=1$ and $\febasis_i(\fepoint_j)=0$ if $i\neq j$. 
...
%
\begin{align}
\spatialmean^i(x) :=& \DIFaddbegin \DIFadd{\frac{1}{V(x)}}\DIFaddend \int_\Omega \DIFdelbegin \DIFdel{(}\DIFdelend \impulseresponse_x(y) \DIFdelbegin \DIFdel{/ V(x)) }\DIFdelend y^i ~\mathrm{d}y \label{eq:define_mean} \\
\spatialcov^{ij}(x) :=& \DIFaddbegin \DIFadd{\frac{1}{V(x)}}\DIFaddend \int_\Omega \DIFdelbegin \DIFdel{(}\DIFdelend \impulseresponse_x(y) \DIFdelbegin \DIFdel{/ V(x)) }\DIFdelend \left(y^i - \spatialmean^i(x)\right) \left(y^j - \spatialmean^j(x)\right) ~\mathrm{d}y, \label{eq:define_cov}
\end{align}
%
%
where $\spatialmean^i(x)$ \DIFdelbegin \DIFdel{denotes }\DIFdelend \DIFaddbegin \DIFadd{and $y^i$ denote }\DIFaddend the $i^\text{th}$ \DIFdelbegin \DIFdel{component of the vector }\DIFdelend \DIFaddbegin \DIFadd{components of the vectors }\DIFaddend $\spatialmean(x)$ \DIFdelbegin \DIFdel{, }\DIFdelend and \DIFaddbegin \DIFadd{$y$, respectively, and }\DIFaddend $\spatialcov^{ij}(x)$ denotes the $(i,j)$ entry of the matrix $\spatialcov(x)$.
...
\end{align}
These are four different ways to approximate an impulse response by a nearby impulse response, with each successive approximation building upon the previous ones. The proposed method uses~\eqref{eq:translate4}, which is the most sophisticated. Approximation~\eqref{eq:translate1} says that $\phi_x$ can be approximated by $\phi_{x'}$ when $x$ and $x'$ are close. Operators satisfying~\eqref{eq:translate1} can be well approximated via low rank CUR approximation.
...
\DIFaddbegin \DIFadd{Before presenting the technical details of the algorithm in Sections \ref{sec:sample_point_selection}--\ref{sec:flip_eigs}, we first provide an overview.
%DIF > Here we provide an overview of the operator approximation algorithm. 
%DIF > Then, in the subsections (Sections \ref{sec:sample_point_selection}--\ref{sec:flip_eigs}) we present technical details of the algorithm that were not already covered in Section \ref{sec:prerequisites}.
}

\DIFaddend We use~\eqref{eq:vol_mean_var} to compute $\spatialvol$, $\spatialmean$, and $\spatialcov$ by applying $\Aop^T$ to polynomial functions. Then we use~\eqref{eq:support_ellipsoid} to form ellipsoid shaped estimates for the support of the $\phi_x$'s, \emph{without} computing them \DIFaddbegin \DIFadd{(see Figure \ref{fig:frog_moments_ellipsoid})}\DIFaddend . This allows us to compute large numbers of $\impulseresponse_{x_i}$ in ``batches,'' $\eta_b$ (see \DIFdelbegin \DIFdel{Figure~\ref{fig:batches_intro}}\DIFdelend \DIFaddbegin \DIFadd{Figures~\ref{fig:batches_intro}~and~\ref{fig:frog_batches}}\DIFaddend ). We compute one batch, denoted $\eta_b$, by applying $\Aop$ to a weighted sum of point sources (Dirac comb) associated with a batch, $S_b$, of points $x_i$ scattered throughout $\Omega$ (Section~\ref{sec:get_impulse_response}). The batch of points, $S_b$, is chosen via a greedy ellipsoid packing algorithm so that, for $x_i,x_j \in S_b$, the support ellipsoid for $\impulseresponse_{x_i}$ and the support ellipsoid for $\impulseresponse_{x_j}$ do not overlap if $i \neq j$ (Section~\ref{sec:sample_point_selection}). Because these supports do not overlap (or do not overlap much), we can post process $\eta_b$ to recover the functions $\impulseresponse_{x_i}$ associated with all points $x_i \in S_b$. With one application of $\Aop$, we recover many $\impulseresponse_{x_i}$ (Section~\ref{sec:get_impulse_response}). The process is repeated until a desired number of batches is reached.

Once the batches $\eta_b$ are computed, we approximate the integral kernel $\Aker(y,x)$ at arbitrary points $(y,x)$ by interpolation of translated and scaled versions of the computed $\impulseresponse_{x_i}$ (Section~\ref{sec:approximate_kernel_entries} \DIFaddbegin \DIFadd{and Figure \ref{fig:hmatrix_neighbors_frog}}\DIFaddend ). The key idea behind the interpolation is the normalized local mean displacement invariance assumption discussed in Section~\ref{sec:local_mean_displacement_invariance}. Specifically, we approximate $\Aker(y,x) = \phi_x(y)$ by a weighted linear combination of the values $\frac{\spatialvol(x)}{\spatialvol(x_i)}\phi_{x_i}(y - \spatialmean(x) + \spatialmean(x_i))$ for a small number of sample points $x_i$ near $x$. The weights are determined by radial basis function (RBF) interpolation.
...
\DIFdelbegin \DIFdel{(i.e., if $\candidatepoint_i$ is sufficiently far from other points in the batch, and $\spatialvol(\candidatepoint_i)$ and $\spatialcov(\candidatepoint_i)$ are acceptable)
}\DIFdelend %DIF > (i.e., if $\candidatepoint_i$ is sufficiently far from other points in the batch, and $\spatialvol(\candidatepoint_i)$ and $\spatialcov(\candidatepoint_i)$ are acceptable)
...
\DIFdelbegin \DIFdel{$\rbf(r):= \exp\left(-C_\text{RBF}^2\frac{r^2}{2 L^2}\right)$
}\DIFdelend \DIFaddbegin \DIFadd{$\rbf(r):= \exp\left(-\frac{1}{2}\left(\epsilon\frac{r}{L}\right)^2\right)$
}\DIFaddend is a Gaussian kernel radial basis function. Here $L:=\diam\left(\{x_i\}_{i=1}^{\numnbr}\right)$ is the diameter of the set of sample points used in the interpolation, and \DIFdelbegin \DIFdel{$C_\text{RBF}$ }\DIFdelend \DIFaddbegin \DIFadd{$\epsilon$ }\DIFaddend is a user-defined shape parameter that controls the width of the kernel function\DIFdelbegin \DIFdel{(we use $C_\text{RBF}=3.0$)}\DIFdelend . The vector of weights, $\rbfweight = (\rbfweight_1, \rbfweight_2, \dots, \rbfweight_{\numnbr})^T$, is found as the solution to the $\numnbr \times \numnbr$ linear system
\begin{equation}
B \rbfweight = f,
\end{equation}
where $B \in \mathbb{R}^{\numnbr \times \numnbr}$,
...
%DIF > \begin{figure}
%DIF > 	\begin{subfigure}[b]{0.24\textwidth}
%DIF > 		\centering
%DIF > 		\includegraphics[scale=0.25]{rbf_cosine_shape_parameter=1.0.pdf}
%DIF > 	\end{subfigure}
%DIF > 	\begin{subfigure}[b]{0.24\textwidth}
%DIF > 		\centering
%DIF > 		\includegraphics[scale=0.25]{rbf_cosine_shape_parameter=10.0.pdf}
%DIF > 	\end{subfigure}
%DIF > 	\begin{subfigure}[b]{0.24\textwidth}
%DIF > 		\centering
%DIF > 		\includegraphics[scale=0.25]{rbf_cosine_shape_parameter=100.0.pdf}
%DIF > 	\end{subfigure}
%DIF > 	\begin{subfigure}[b]{0.24\textwidth}
%DIF > 		\centering
%DIF > 		\includegraphics[scale=0.25]{cosine_RBF_convergence.pdf}
%DIF > 	\end{subfigure}
%DIF > 	
%DIF > 	\caption{asdf
%DIF > 	}
%DIF > 	\label{fig:RBF_width}
%DIF > \end{figure}
\DIFaddbegin 

\DIFadd{The shape parameter, $\epsilon$, mediates a tradeoff between accuracy and stability. Small $\epsilon$ is required for RBF interpolation with Gaussian kernels to achieve high accuracy, but small $\epsilon$ also makes RBF interpolation less robust to errors or nonsmoothness in the function being interpolated. 
%DIF > This is illustrated in Figure \ref{fig:RBF_width} for a simple one dimensional interpolation problem. 
For our numerical results involving Hessians in inverse problems governed by PDEs (Sections~\ref{sec:ice}~and~\ref{sec:adv}), high accuracy is not required because the proposed method is used to construct a preconditioner. Hence, for these Hessian approximations we use a conservative choice of $\epsilon=3.0$ to ensure robustness. For our numerical results for the blur problem example (Section~\ref{sec:frog}), we use a smaller value of $\epsilon=0.5$ so that the RBF interpolation accuracy is not a limiting factor as we study convergence of the proposed method.
}


\DIFaddend \subsection{Hierarchical matrix construction}
...
where $\Aop$ is a symmetric positive semi-definite operator that we approximate with the \DIFdelbegin \DIFdel{PSF }\DIFdelend \DIFaddbegin \DIFadd{proposed }\DIFaddend method to form an H-matrix $\mathbf{A}_H$, and $\mathcal{R}$ is a symmetric positive definite operator that may be easily converted to an H-matrix $\mathbf{R}_H$ without using the \DIFdelbegin \DIFdel{PSF }\DIFdelend \DIFaddbegin \DIFadd{proposed }\DIFaddend method. For example, in inverse problems $\mathcal{H}$ is the Hessian, $\Aop$ is the data misfit term in the Hessian which is dense and available only matrix-free, and $\mathcal{R}$ is the regularization term, which is typically an elliptic differential operator that becomes a sparse matrix after discretization.
...
using a Cayley shift-and-invert Krylov scheme \cite{LehoucqEtAl98}. We flip the signs of these eigenvalues to be positive instead of negative (i.e., $\lambda \rightarrow |\lambda|$) by performing a low rank update to $\mathbf{A}_H^{\text{sym}}$. \DIFaddbegin \DIFadd{We observe that the eigenvectors associated with large erroneous negative eigenvalues tend to be directions that are nevertheless ``important'' to $\mathcal{A}$, so flipping the eigenvalues instead of setting them to zero tends to yield better approximations. }\DIFaddend The primary computational task in the Cayley shift-and-invert scheme is the solution of shifted linear systems of the form
...
All operations that do not require applications of $\Aop$ to vectors are \DIFdelbegin \DIFdel{nearly }\DIFdelend \DIFaddbegin \DIFadd{polylog }\DIFaddend linear, and therefore scalable, in the size of the problem, $N$.
...
\paragraph{(1) Computing impulse response moments and batches} Computing $\spatialvol$, $\spatialmean$, and $\spatialcov$ requires applying \DIFdelbegin \DIFdel{$\Aop$ }\DIFdelend \DIFaddbegin \DIFadd{$\Aop^T$ }\DIFaddend to $1$, $\gdim$, and $\gdim(\gdim+1)/2$ vectors, respectively. \DIFaddbegin \DIFadd{This works out to $3$ applications of $\Aop^T$ in one spatial dimension, $6$ in two dimensions, and $10$ in three dimensions. 
}\DIFaddend Computing each $\eta_i$ requires applying $\Aop$ to one vector, so computing $\{\eta_i\}_{i=1}^{\nbatch}$ requires $\nbatch$ operator applications. In total, computing all impulse response moments and batches therefore requires
\begin{equation*}
...
\DIFdelbegin \DIFdel{In the numerical results (Section~\ref{sec:numerical_results}}\DIFdelend \DIFaddbegin \DIFadd{For our numerical results involving Hessians (Sections~\ref{sec:ice} and~\ref{sec:adv}}\DIFaddend ), we use one matrix-matrix addition to add the H-matrix approximation of the data misfit term in the Hessian to the regularization term in the Hessian.
...
In both \DIFdelbegin \DIFdel{cases}\DIFdelend \DIFaddbegin \DIFadd{inverse problems}\DIFaddend , to reconstruct the unknown parameter fields, denoted $q$, the inverse problems are formulated as nonlinear least squares
...
\subsection{Example 1: \DIFdelbegin \DIFdel{Inversion for the basal friction coefficient in an ice sheet flow }\DIFdelend \DIFaddbegin \DIFadd{Spatially varying blurring }\DIFaddend problem}
\DIFaddbegin \label{sec:frog}

\DIFadd{Here we define a PDE-free spatially varying blur problem, in which the impulse response, $\phi_x$,
...
various stages and aspects of the proposed method, and robustness (or lack thereof) of the proposed method to violations of the non-negative kernel assumption (Section~\ref{sec:conditions_2}). In this section, the blur problem is used to compare the proposed method to hierarchical off-diagonal low rank (HODLR) and global low rank (GLR) methods, to study convergence of the proposed method, and to investigate the effect of the ellipsoid size parameter $\tau$.
%DIF > To study the
%DIF > robustness of the proposed method to violations of the non-negative
%DIF > kernel assumption discussed in Section~\ref{sec:impulse_response_2},
The closed form
expression for the integral kernel is given by
%DIF > 
%DIF > Before delving into technical details, we provide an overview of the
%DIF > method. Along the way, we illustrate steps using a spatially variant
%DIF > blur example, in which the integral kernel is given as follows:
%DIF > \begin{equation}
%DIF > \label{eq:frog_kernel}
%DIF > \Phi(y,x) = (1 - a f(y,x)) x^1(1-x^1)x^2(1-x^2) \exp \left(-\frac{1}{2}h^T(y,x) C^{-1} h(y,x)\right).
%DIF > \end{equation}
%DIF > Here, $h(y,x) = R(\theta(x)) (y-x)$, $R(\theta(x))$ is the 2D rotation
%DIF > matrix by angle $\theta(x) = (x^1 + x^2) \pi / 2$, $C = L^2
%DIF > \operatorname{diag}(c_1, c_2)$, $g(x) = x^1(1-x^1)x^2(1-x^2)$, and
%DIF > $f(x,y) = \cos\left(p^1(y,x) / \sqrt{c_1 / 2.0}\right)
%DIF > \sin\left(p^2(y,x) / \sqrt{c_2 / 2.0}\right)$ for some constants $a,
%DIF > L, c_1, c_2$. In this section we use $a=1.0$, $L^2=1.0$, $c_1=0.01$,
%DIF > and $c_2=0.0025$.
%DIF > 
}\begin{align}
\DIFadd{\label{eq:frog_kernel}
\Phi(y,x) = (1 - a f(y,x)) g(x) \exp \left(-\frac{1}{2}(h(y,x)^T
C^{-1} h(y,x) \right),
}\end{align}
\DIFadd{where $f(y,x) = \cos\left(h^1(y,x) / \sqrt{c_1 /
...
study the robustness of the proposed method to violations of Assumption 3
(Section~\ref{sec:conditions_2}).
}

%DIF > The impulse response $\phi_x$ corresponding to this kernel is a bumpy
%DIF > blob that is centered near $x$, and is rotated and scaled in a manner
%DIF > that depends on $x$ (see Figure
%DIF > \ref{fig:frog_kernel_impulse_responses}). The constant $L$ controls
%DIF > how wide the blob is (hence the support of the impulse response), and
%DIF > $a$ controls the magnitude of the bumps. 

%DIF > Note that for this
%DIF > operator, using our method is unnecessary because entries of the
%DIF > kernel are given directly via a formula. The purpose of this example
%DIF > is to illustrate the method using an operator which is known exactly
%DIF > and may be easily visualized.
	...
	\begin{center}
		{\small
			...
			\begin{tabular}{c| c | c c c c c}
				& \DIFaddFL{Error tol.    }&	\DIFaddFL{\#applies PSF   }& \DIFaddFL{\#applies HODLR 	}& \DIFaddFL{\#applies RSVD 	}\\%DIF > & rank \\ 
				\hline 
				& \DIFaddFL{20\%    		}& \DIFaddFL{11  				}& \DIFaddFL{592				}& \DIFaddFL{354				}\\%DIF > & 111  \\ 
				\DIFaddFL{$L=1$ 		}& \DIFaddFL{10\% 			}& \DIFaddFL{15  				}& \DIFaddFL{772				}& \DIFaddFL{520				}\\%DIF > & 178  \\ 
				& \DIFaddFL{5\% 			}& \DIFaddFL{23   				}& \DIFaddFL{924				}& \DIFaddFL{674				}\\%DIF > & 244 \\ 
				\hline
				& \DIFaddFL{20\%    		}& \DIFaddFL{8  				}& \DIFaddFL{852 				}& \DIFaddFL{1316				}\\%DIF > & 417 \\ 
				\DIFaddFL{$L=1/2$ 	}& \DIFaddFL{10\%    		}& \DIFaddFL{10  				}& \DIFaddFL{1144				}& \DIFaddFL{1916				}\\%DIF > & 665 \\ 
				& \DIFaddFL{5\% 	  		}& \DIFaddFL{12   				}& \DIFaddFL{1404				}& \DIFaddFL{2456				}\\%DIF > & 903 \\ 
				\hline
				& \DIFaddFL{20\%    	 	}& \DIFaddFL{7					}& \DIFaddFL{932				}& \DIFaddFL{2624				}\\%DIF > & 845  \\ 
				\DIFaddFL{$L=1/3$ 	}& \DIFaddFL{10\%    		}& \DIFaddFL{8  				}& \DIFaddFL{1264 				}& \DIFaddFL{3734				}\\%DIF > & 1321  \\ 
				& \DIFaddFL{5\% 			}& \DIFaddFL{8   				}& \DIFaddFL{1520 				}& \DIFaddFL{4660				}\\%DIF > & 1759  \\ 
			\end{tabular}
			\endgroup
		}
	\end{center}
	\caption{\DIFaddFL{(Blur) Comparison of cost to
		approximate the blur kernel from Equation
		\eqref{eq:frog_kernel} using the proposed PSF-based method, the randomized
...
\DIFadd{In Table~\ref{tab:frog_psf_vs_hodlr_vs_glr} we compare the
computational cost to approximate the blur kernel from
Equation~\eqref{eq:frog_kernel} using the proposed PSF-based method, the
randomized HODLR (hierarchical off diagonal low rank)
method~\mbox{%DIFAUXCMD
\cite{Martinsson16,Hartland_2023}}\hskip0pt%DIFAUXCMD
, and GLR (global low rank)
approximation using randomized SVD~\mbox{%DIFAUXCMD
\cite{HalkoMartinssonTropp11}}\hskip0pt%DIFAUXCMD
. For
...
performance of the the HODLR and GLR methods deteriorates.
}
...
\begin{figure}
	...
	\begin{subfigure}{0.47\textwidth}
		...
		\end{center}
	\end{subfigure}
	\caption{\DIFaddFL{(Blur) Relative error 
		%DIF > 		in the PSF approximation of the kernel
		%DIF > 	$||\boldsymbol{\Phi} - \widetilde{\boldsymbol{\Phi}}||_\text{Fro}/||\boldsymbol{\Phi}||_\text{Fro}$,
		for different ellipsoid size parameters, $\tau$, vs. the total number of impulse responses (left), and the number of batches (right). 
		%DIF > 	Left: Relative error vs. the total number of impulse responses used in the approximation. Right: Relative error vs. the number of impulse response batches. 
...
\DIFadd{In Figure \ref{fig:PSF_tau_convergence} we show the convergence of the proposed method on the blur kernel as a function of the total number of impulse responses (left), and the number of batches (right). We show convergence for several ellipsoid size parameters $\tau$, ranging from $2.0$--$4.0$. The results in Figure \ref{fig:PSF_tau_convergence} (left) show that the relative error decreases as $\text{constant} \times \left(\# \text{impulse responses}\right)^{-1}$
%DIF >  or $\text{constant} \times \# \text{batches}^{-1}$, 
suggesting linear convergence. The linear convergence stalls at a limit which depends on $\tau$. Increasing $\tau$ lowers this limit, allowing the proposed method to achieve higher accuracy. 
In Figure \ref{fig:PSF_tau_convergence} (right), the results show that before this limit is reached, the convergence is faster for smaller $\tau$ in terms of the number of batches. This is expected because smaller $\tau$ results in more impulse responses per batch. Larger $\tau$ causes the proposed method converge more slowly than smaller $\tau$, but with larger $\tau$ the proposed method stalls at a lower level of error than it does with smaller $\tau$ (see, e.g., $\tau=4.0$ vs. $\tau=2.5$). 
}

%DIF > ----
%DIF > , but allows the method to eventually achieve a lower level of error.
%DIF > 
%DIF > ----
%DIF > 
%DIF > For smaller $\tau$ regime, we observe faster convergence
%DIF > 
%DIF > Before this limit is reached, the convergence rate is the same for all $\tau$ as a function the number of impulse responses. This means that the convergence is faster for smaller $\tau$ as a function of the number of batches, since smaller $\tau$ means more impulse responses per batch. In summary, taking $\tau$ larger makes the method converge more slowly, but allows it to eventually achieve a lower level of error.


%DIF > Define HODLR. Define randomized HODLR algo. Ref to Tucker's paper and "Fast randomized eigensolver with structured LDL..."

%DIF > Define randomized SVD, refer to Halko Martinsson Tropp.

%DIF > Discuss why PSF does better with smaller $L$.





\subsection{\DIFadd{Example 2: Inversion for the basal friction coefficient in an ice sheet flow problem}}
\DIFaddend \label{sec:ice}
...
The pressure, $\pressure$, is discretized with first order scalar continuous Galerkin finite elements defined on a mesh of tetrahedra. The velocity, $\velocity$, is discretized with second order continuous Galerkin finite elements on the same mesh. The parameter $\basalfriction$ is discretized with first order scalar continuous Galerkin finite elements on the mesh of triangles that results from restricting the tetrahedral mesh to the basal boundary, $\Gamma_b$. Note that $\Gamma_b$ is a two-dimensional surface embedded in three dimensions due to the mountain topography. The proposed \DIFdelbegin \DIFdel{PSF }\DIFdelend \DIFaddbegin \DIFadd{PSF-based }\DIFaddend method involves translating impulse responses. Hence it requires either a flat domain, or a notion of local parallel transport. We therefore generate a flattened version of $\Gamma_b$, denoted by $\Omega \subset \mathbb{R}^2$, by ignoring the height coordinate. The parameter $\basalfriction$ is viewed as a function on $\Gamma_b$ for the purpose of solving the Stokes equations, and as a function on $\Omega$ for the purpose of building Hessian approximations and defining the regularization.
...
\begin{figure}
	...
	\end{center}
    \end{subfigure}
	\caption{(Ice sheet) Left: \DIFdelbeginFL \DIFdelFL{convergence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Convergence }\DIFaddendFL history for solving $\mathbf{H} \mathbf{x} = \mathbf{b}$ using PCG, where $\mathbf{b}$ has i.i.d.\ random entries drawn from the standard Gaussian distribution and $\mathbf{H}$ is evaluated at the solution of the inverse problem. 
	...
	Right: \DIFdelbeginFL \DIFdelFL{generalized }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Generalized }\DIFaddendFL eigenvalues for generalized eigenvalue problem $\mathbf{H} \mathbf{u}_{k}=\lambda_{k} \mathbf{\preconditioner} \mathbf{u}_{k}$. Here $\mathbf{H}$ is the Hessian and the matrices $\mathbf{\preconditioner}$ are the same Hessian approximations used in the left sub-figure, with NONE corresponding to the identity matrix.}
...
\subsection{Example \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend : Inversion for the initial condition in an advective-diffusive transport problem}
\label{sec:adv}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{(Advective-diffusive transport) Left: \DIFdelbeginFL \DIFdelFL{convergence }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Convergence }\DIFaddendFL history for solving $\mathbf{H} \mathbf{x} = \mathbf{b}$ using PCG, where $\mathbf{b}$ has i.i.d.\ random entries drawn from the standard Gaussian distribution. Right: \DIFdelbeginFL \DIFdelFL{generalized }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Generalized }\DIFaddendFL eigenvalues for generalized eigenvalue problem $\mathbf{H} \mathbf{u}_{k}=\lambda_{k} \mathbf{\preconditioner} \mathbf{u}_{k}$. Here $\mathbf{H}$ is the Hessian and the preconditioner, $\mathbf{\preconditioner}$, is the PSF approximation for 1, 5, or 25 batches (PSF (1), PSF (5), and PSF (25), respectively), the regularization Hessian (REG), or the identity matrix (NONE).}
...
We presented an efficient matrix-free \DIFdelbegin \DIFdel{PSF }\DIFdelend \DIFaddbegin \DIFadd{PSF-based }\DIFaddend method for approximating operators with locally supported non-negative integral kernels. The \DIFaddbegin \DIFadd{proposed }\DIFaddend method requires access to the operator only via application of the operator to a small number of vectors. The idea of the \DIFaddbegin \DIFadd{proposed }\DIFaddend method is to compute batches of impulse responses by applying the operator to Dirac combs of scattered point sources, then interpolate these impulse responses to approximate entries of the operator's integral kernel. The interpolation is based on a new principle we call ``local mean displacement invariance,'' which generalizes classical local translation invariance. The ability to quickly approximate arbitrary integral kernel entries permits us to form an H-matrix approximation of the operator. Fast H-matrix arithmetic is then used to perform further linear algebra operations that cannot be performed easily with the original operator, such as matrix factorization and inversion. The supports of the impulse responses are estimated to be contained in ellipsoids, which are determined a-priori via a moment method that involves applying the operator to a small number of polynomial functions. Point source locations for the impulse response batches are chosen using a greedy ellipsoid packing procedure, in which we choose as many impulse responses per batch as possible, while ensuring that the corresponding ellipsoids do not overlap. We applied the \DIFaddbegin \DIFadd{proposed }\DIFaddend method to approximate the Gauss-Newton Hessians in an ice sheet flow inverse problem governed by a linear Stokes PDE, and an advective-diffusive transport inverse problem governed by an advection-diffusion PDE. We saw that preconditioners based on the proposed approximation cluster the eigenvalues of the preconditioned Hessian near one, and allow us to solve the inverse problems using roughly $5\times$--$10\times$ fewer PDE solves. 
For larger domains with more observations, the rank of the data misfit Hessian will increase, while the locality of impulse responses will remain the same. Hence, we expect the speedup will be even greater for such problems. Although the \DIFdelbegin \DIFdel{method we presented }\DIFdelend \DIFaddbegin \DIFadd{proposed method }\DIFaddend is not applicable to all Hessians, it is applicable to many Hessians of practical interest. For these Hessians, the proposed method offers a \emph{data scalable} alternative to conventional low rank approximation, due to the ability to form high rank approximations of an operator using a small number of operator applications, and thus PDE solves.


\section*{Acknowledgments}
We thank J.J. Alger, Longfei Gao, Mathew Hu, and Rami Nammour for helpful discussions. We thank \DIFdelbegin \DIFdel{Trevor Heise }\DIFdelend \DIFaddbegin \DIFadd{Nicole Aretz, Trevor Heise, and the anonymous reviewers }\DIFaddend for editing suggestions. We thank Georg Stadler for help with the domain setup for the ice sheet problem.

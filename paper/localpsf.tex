...
where $\{x_i\}_{i=1}^k$ are the $k$ nearest sample points to $x$, excluding sample points for which $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is undefined. That is, sample points such that $y-\mu(x)+\mu(x_i) \notin \Omega$. One should choose a small $k$; in numerical experiments we use $k=10$. We find the $k$ nearest sample points by querying a precomputed KD-tree \cite{kdtree} of all sample points. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) \cite{aabbtree} of the mesh cells used to discretize the problem. 
Per Section \ref{eq:local_mean_displacement_invariance}, $\Phi(y,x) \approx f_i$ when $x_i$ is close to $x$. The factor $\frac{V(x)}{V(x_i)}$, which was not discussed in Section \ref{eq:local_mean_displacement_invariance}, improves the approximation if $V(x)$ varies widely. 

...
\begin{equation*}
	f(\lambda) \approx \widetilde{f}(\lambda) := \frac{\lambda}{1 + \left(\frac{2 \lambda - M}{M}\right)^{2^k} },
\end{equation*}
where $k$ is a positive integer, and $M$ is a positive real number that is larger than the largest eigenvalue of $\mathbf{A}_H^{\text{sym}}$. In practice we use $k=5$, and $M=1.5~ \lambda_\text{max}$, where $\lambda_\text{max}$ is the largest eigenvalue of $\mathbf{A}_H^{\text{sym}}$, which is computed using the Lanczos iteration. If $k$ is sufficiently large, then $\widetilde{f}(\lambda) \approx \lambda$ for $\lambda \in (0,M)$ and $\widetilde{f}(\lambda) \approx 0$ for $\lambda$ outside of $[0,M]$, so $\widetilde{f}(\mathbf{A}_H^{\text{sym}})$ is a good approximation of  $f(\mathbf{A}_H^{\text{sym}})$. This is illustrated in Figure \ref{fig:spd_rat}. 
\begin{figure}
	\centering
...
where
\begin{equation*}
	\mathbf{X} := \left(2 \mathbf{A}_H^{\text{sym}} - M \mathbf{I}\right)/M.
\end{equation*}

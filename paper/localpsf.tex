...
Hessian approximations and preconditioners are highly desirable because the Hessian is central to efficient solution of inverse problems in both the deterministic and Bayesian settings. When solving the deterministic optimization problem with Newton-type methods, the Hessian is the coefficient operator for the linear system that must be solved at every Newton iteration. Good Hessian preconditioners reduce the number of iterations required to solve these Newton linear systems with Krylov methods such as conjugate gradient. In the Bayesian setting, the inverse of the Hessian is the covariance of the local Gaussian approximation of the posterior generated by truncating the Taylor series of the negative log posterior after the quadratic term. Hessian-based local Gaussian approximations of the posterior may be used as surrogate models of the posterior, or may be used as proposal distributions for Markov chain Monte-Carlo (MCMC) methods for drawing samples from the posterior. 
...
Matrix-free high rank Hessian approximation methods are needed.

Recently there have been improvements in matrix-free H-matrix construction methods in which the operator is applied to structured random vectors, and the response of the operator to those random vectors is then processed to construct an H-matrix approximation  \cite{LinYing11,Martinsson11,Martinsson16,MartinssonTropp20}. 
...
Matrix-free H-matrix construction methods are currently an area of active research (see, e.g., \cite{LevittMartinsson}), so more efficient matrix-free H-matrix methods may be discovered in the future. In this paper, we form an H-matrix Hessian approximation. But rather than use these modern methods to form the H-matrix directly, instead we exploit locality and non-negative integral kernel properties to form a PSF approximation of the data misfit Hessian, then convert the PSF approximation to H-matrix format and combine it with the regularization Hessian using classical H-matrix techniques. Not all data misfit Hessians satisfy the local non-negative integral kernel properties (for example, the wave inverse problem data misfit Hessian has a substantial amount of negative entries in its integral kernel). However, many data misfit Hessians of practical interest do satisfy these properties (either exactly or approximately), and our method is targeted at approximating these Hessians.

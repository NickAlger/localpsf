...
\end{aligned}
\end{equation}
where $v_j \in \mathbb{R}$, $j=1,\dots,r$ are now arbitrary function values that $v$ must take at the points $x_j$. We may efficiently solve optimization problem \eqref{eq:wi_optimization_problem_generic} via the following procedure:
\begin{enumerate}
...
\begin{proof}
	...
	\end{equation*}	
	denote the tangent space to $X$. Note that pointwise evaluation of functions in $H^2_N(\Omega)$ is well-defined because the Sobolev impedding theorem implies that $H^2_N(\Omega)$ is continuously imbedded in the spacde of continuous functions $C^0(\Omega)$ whenever the spatial dimension satisfies $d < 4$, and here we consider $d \in \{1,2,3\}$. Hence the optimization problem, the feasible set $X$ and the tangent space $X_0$ are well-defined.
\end{proof}

...
For the kernel in the left two columns of Figure \ref{fig:robustness_to_negatives}, negative numbers are interspersed with positive numbers, allowing us to include a large amount of negative numbers in the integral kernel before the method fails. For the kernel in the right two columns, the ellipsoid estimate fails with tiny amounts of negative numbers because the negative numbers are far away from the mean and not balanced by positive numbers at similar distances and angles. In the bottom two rows, we see that the method rapidly transitions from performing reasonably well to performing poorly with only a small change to the integral kernel. 
...
\begin{figure}
	...
	\caption{Illustration of the influence of negative numbers in the integral kernel on the robustness of the ellipsoid estimates for the supports of impulse responses. Left two columns: spatially variant blurring kernel given in Equation \ref{eq:frog_kernel}. Right two columns: Ricker wavelet-type kernel given by $\Aker(y,x) = (1 - a \theta)\exp\left(-\theta/2\right)$, where $\theta=(y-x)^T \Sigma^{-1} (y-x)$, and $\Sigma=\operatorname{diag}\left(0.0025, 0.01\right)$. Ordered from top to bottom, the results are obtained with $a\in\{1.0, 20.0, 27.0\}$ for the blur kernel, and $a\in \{0.0, 0.23, 0.249\}$ for the Ricker wavelet-type kernel. Columns 1 and 3: impulse responses for the blur and Ricker wavelet-type kernels, respectively, with estimated support ellipsoids indicated by the black ellipses. Red and blue represent positive and negative numbers in the integral kernel, respectively. Columns 2 and 4: one-dimensional slice along the horizontal line indicated in the two-dimensional plots. The dashed gray line is at zero.
	}

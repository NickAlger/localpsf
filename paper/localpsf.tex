...
Bw = f,
\end{equation}
where the coefficient matrix $B \in \mathbb{R}^{k \times k}$ and right hand side vector $f \in \mathbb{R}^k$ have entries 
\begin{align*}
...
\end{equation}
If \eqref{eq:ellipsoid_check93} does not hold, then $y - \mu(x) + \mu(x_i)$ is outside the estimated support of $\impulseresponse_{x_i}$, so we set $f_{y,x}(x_i)=0$ for the purpose of interpolation. If \eqref{eq:ellipsoid_check93} does hold, then we look up the batch index $b$ for such that $x_i \in S_b$, and evaluate
\begin{equation}

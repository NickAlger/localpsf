...
We form a hierarchical matrix (H-matrix)~\cite{BormGrasedyckHackbusch03,Hackbusch99} approximation of a discretized version of $\Aop$. H-matrices are a matrix format in which the rows and columns of the matrix are re-ordered, then the matrix is recursively subdivided into blocks, in such a way that many off-diagonal blocks are low rank, even though the matrix as a whole may be high rank. 
...
%	\end{subfigure}
	...
	%		\label{fig:newtoniter4}
    \end{subfigure}
	\caption{\textbf{(Ice sheet)} The log basal sliding parameter, with colorscale as in Figure~\ref{fig:true_beta}, computed from the PDE-constrained optimization problem with noise levels: $25\%$ (left), $5.0\%$ (middle), and $1.0\%$ (right).}
...
%	\label{fig:generalized_eigenvalues}
    \end{subfigure}
	\caption{\textbf{(Ice sheet)} Left: convergence history for solving $\bm{H} \bm{x} = \bm{b}$ using PCG, where $\bm{b}$ has i.i.d. random entries drawn from the standard Gaussian distribution and $\bm{H}$ is evaluated at the solution of the inverse problem. 
...
$ 
The matrix $\bm{\preconditioner}$ is one of the PSF (1), PSF (5), or PSF (25) Gauss-Newton Hessian approximations, the regularization Hessian (REG), or the identity matrix (NONE). With the PSF preconditioners, the generalized eigenvalues cluster near one, with more batches yielding better clustering. 
...
\begin{table}
	\begin{center}
			...
			\endgroup
		\end{center}
	\caption{\textbf{(Ice sheet)} Condition number for $\bm{\preconditioner}^{-1} \bm{H}$ for our preconditioner with 1, 5, and 25 batches (PSF (1), PSF (5), and PSF(25), respectively), no preconditioner (NONE) and regularization (REG). All operators are evaluated at the $\bm{\basalfriction}$ that solves the inverse problem for 
...
\begin{figure}
	...
	%\end{subfigure}
	\caption{\textbf{(Advection-diffusion)} (\ref{fig:adv_initial_condition}) True initial condition. (\ref{fig:adv_velocity}) Velocity field. Color indicates magnitude of velocity vector. (\ref{fig:adv_observations}) Noisy observations of concentration at final time. (\ref{fig:adv_reconstruction}) Reconstructed initial condition.}
...
\begin{table}
	\begin{center}
		...
		\endgroup
	\end{center}
	\caption{\textbf{(Advection-diffusion)} Number of preconditioned conjugate gradient iterations required to achieve error tolerance $||\widehat{\bm{q}}_j - \widehat{\bm{q}}|| < 10^{-6}||\widehat{\bm{q}}||$, where $\widehat{\bm{q}}_j$ is the $j$th conjugate gradient iterate, and $\widehat{\bm{q}}$ is the solution of the Newton linear system in the advection-diffusion inverse problem. Iteration counts are shown for a variety of different diffusion parameters $\kappa$, simulation times $T$, and preconditioners.}
...
%			& \includegraphics[scale=0.15, valign=c]{impulse_response_P(0.3,0.6)_T1.0_K1.e-3.png} 
	...
	\caption{\textbf{(Advection-diffusion)} Impulse responses for small and large diffusion parameters $\kappa$ and simulation times $T$.}

...
We may further reduce the cost of performing many ellipsoid intersection tests by first checking whether the coordinate axis aligned bounding boxes for the ellipsoids intersect. Checking whether two axis-aligned bounding boxes intersect is extremely cheap. If the bounding boxes do not intersect, then the ellipsoids do not intersect, so no further action is necessary. If the bounding boxes do intersect, the ellipsoids may or may not intersect, so one must perform the more expensive ellipsoid intersection test described above

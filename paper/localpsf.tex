...
\begin{equation*}
\min_{p \in P} \max_{x \in S_1 \cup \dots \cup S_{b-1}} \|p - x\|
\end{equation*}
After we choose the candidate $p$, we remove $p$ from $P$.
If $p$ is sufficiently far from all of the previously chosen points in the current batch, in the sense that $E_p \cap E_q = \{\}$ for all $q \in S_b$, and if $p$ is sufficiently far from the boundary, in the sense that $E_p \cap \partial \Omega = \{\}$, then we add $p$ to $S_b$. Otherwise we discard $p$. This process repeats until it is no longer possible to pick a point from $P$ that is sufficiently far from the points in $S_b$. 

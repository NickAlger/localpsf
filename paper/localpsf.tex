...
Our work builds upon a history of ``product-convolution`` approximations---weighted sums of convolution operators with spatially varying weights. A single convolution operator may be either high rank or low rank, so product-convolution approximations can (where applicable) approximate high rank operators with a small number of convolution operators. Product-convolution approximations have been used in a wide variety of fields going back several decades \cite{PRODCONVLIST}.
For background, we recommend reading the following papers: \cite{PRODCONVGOOD}. We specifically build upon the class of product convolution approximations in which the convolution kernels are shifted impulse responses of $\Aop$ associated with a collection of points $x_i$ \cite{PRODCONVLIST}. A popular choice for methods in this class is to choose the points $x_i$ to be nodes in a regular grid, and interpolate the convolution operators with piecewise linear interpolation \cite{NAGY}, splines \cite{PRODCONVSPLINES}, or radial basis functions \cite{PRODCONVRBF}. With a regular grid, a large number of points $x_i$ is typically required to achieve an accurate product-convolution approximation. In our previous work, we reduced the computational cost by starting with a coarse grid of points $x_i$, then adaptively refining the grid in the regions where the error in the approximation is large \cite{PRODCONVMYPAPER}. But even with adaptive refinement, many matrix vector products with $\Aop$ are required. In this paper, rather than using adaptive refinement, we instead reduce the computational cost by picking points $x_i$ such that many $\phi_i$ are computed with each matrix vector product. 
...
In this section we present three key concepts that our method is based on. The method itself will be presented in Section \ref{sec:method}. First, we define moments of the impulse responses $\phi_x$, and show how these moments can be computed efficiently (Section \ref{sec:intromoments}). Our method will use these moments extensively. Second, we describe an impulse response approximation that we call ``normalized local mean displacement invariance'' (Section \ref{sec:local_mean_displacement_invariance}). Our method will interpolate impulse responses using this approximation. Third, we discuss how discretization of continuous functions by finite elements leads to matrix discretizations $\mathbf{A}$ and $\mathbf{\Phi}$ of $\Aop$ and $\Phi$, respectively (Section \ref{sec:finite_element_kernel}). The goal of our method is to compute H-matrix approximations of these matrices. 
...
where $\mathbf{\Phi} \in \mathbb{R}^{\fedim \times \fedim}$ is the dense matrix of kernel entries evaluated at all pairs of Lagrange nodes:
\begin{equation}
...
%\end{equation*}
Our matrix representation of $\Aop$ is therefore given by
\begin{equation}
\label{eq:Amatpc_defn}
\mathbf{A} = \bm{M} \mathbf{\Phi} \bm{M}.
\end{equation}
Note that $\mathbf{A}$ is not the Galerkin projection of $\Aop$ onto $V_h$ because of the approximation $\Phi^h \approx \Phi$.
%because of the approximation $\Phi^h \approx \Phi$. 
Hence, $\mathbf{A}$ may differ slightly from the implicitly defined matrix that performs the mapping $\mathbf{u} \rightarrow \mathbf{v}$ corresponding to the operator action $v=\Aop u$.

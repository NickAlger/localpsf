...
\begin{figure}[H]
	...
	\begin{subfigure}{0.48\textwidth}
		\includegraphics[scale=0.15]{meshHeight_view0_edges_withAxes.png}
...
\begin{subequations}\label{Stokeseqn}
	...
	\end{align}
\end{subequations}
The solution to these equations is the tuple $(\bm{u}, p)$, where $\bm{u}$ is the ice flow velocity, and $p$ is the pressure. Here, $\basalfriction$ is the unknown logarithmic
...
The inverse problem that we seek to solve is

\begin{align*}
\min_{\basalfriction}J(\basalfriction)&:=\frac{1}{2}\int_{\Gamma_{t}}||\bm{T}(\bm{u}-\bm{u}_{d})||_{2}^{2}\,dS+\frac{1}{2}\int_{\Omega}\basalfriction\mathcal{L}^{2}\basalfriction\,dx,\\
\text{where }\bm{u}& \text{ solves \ref{Stokeseqn}}.
\end{align*}
Here $\mathcal{L}$ is a regularizing invertible elliptic differential operator, where $s=\mathcal{L}^{-1}u$ if and only if

\begin{align*}
-\gamma\Delta u +\delta\,u&=s\,\,\,\,\text{ in }\Gamma_{b},\\
\gamma \bm{\nabla}u\cdot\bm{n}+\mu\,u&=0\,\,\,\text{ on }\partial\Gamma_{b}.
\end{align*}
The observation data $\bm{u}_{d}(\bm{x})$ is generated synthetically, as follows: we propose a true log basal friction field $\basalfriction_{\text{true}}(\bm{x})$, solve equation \ref{Stokeseqn} to obtain $\bm{u}_{\text{true}}(\bm{x})$, we then add random relative noise to obtain the noisy data $\bm{u}_{d}$

\begin{align*}
\left(\bm{u}_{d}(\bm{x})\right)_{i}&=\left(\bm{u}_{\text{true}}(\bm{x})\right)_{i}+\varepsilon\, \xi_{i}(\bm{x})|\left(\bm{u}_{\text{true}}(\bm{x})\right)_{i}|\,\,\,i=1,\dots,d,
\end{align*}
where $\xi_{i}(\bm{x})$ are standard normal random numbers and $\varepsilon>0$ sets the noise level. The inverse problem is solved with an inexact-Newton CG scheme, wherein each Hessian $\mathcal{A}$ action, requires the linearized PDE solutions, and so we apply the psf framework to generate Hessian preconditioners and compare the performance with other techniques, such as preconditioning by the regularization operator.

\begin{figure}[H]
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[scale=0.243]{mtrue_withColorBar.png}
		\caption{True $\basalfriction$.}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[scale=0.243]{trueVelocity2_glyphs.png}
		\caption{True $\bm{u}$.}
	\end{subfigure}
	\caption{True logarithmic basal sliding parameter $\basalfriction$ (left) and true velocity $\bm{u}$ (right). In the right plot, arrows indicate the direction of $\bm{u}$ and color indicates the magnitude of $\bm{u}$.}
\end{figure} 
...
\begin{figure}[H]
	...
	%\end{subfigure}
	\caption{The log basal friction parameter $\basalfriction$ after $1$ (left), $3$ (second from left), $4$ (third from left) Newton iterates and the final Newton iterate (right). The preconditioner is constructed between Newton iterations 3 and 4.} 
\end{figure} 

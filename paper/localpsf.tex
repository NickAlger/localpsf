...
We next test the local psf framework on an inverse problem governed by a PDE which models steady-state ice sheet flow.
The domain that is covered by ice is denoted by
$\Omega\subset\mathbb{R}^{d}$, $d=3$,
$\Gamma_{b}$, $\Gamma_{l}$, and $\Gamma_{t}$ denote
the basal, lateral and top parts of the domain boundary
$\partial\Omega$.
...
\begin{subequations}\label{Stokeseqn}
	...
	\bm{u}\cdot\bm{n} =0 \text{ and } \bm{T}\left(\bm{\sigma}_{\bm{u}}\bm{n}+\exp\left(\beta\right)\bm{u}\right)
	&=\bm{0}\,\,\,\,\,\,\text{ on }\Gamma_{b},\label{Stokeseqn:4} \\
	\left(\bm{\sigma}_{\bm{u}}\bm{n}+\omega\bm{u}\right)&=\bm{0}\,\,\,\,\,\,\text{ on }\Gamma_{l},\label{Stokeseqn:5}	
	\end{align}
\end{subequations}
whose solution determines the ice flow velocity $\bm{u}$ and pressure $p$. Here, $\beta$ is the logarithmic
basal sliding parameter field, $\rho\bm{g}$ the body force density due to gravity, $\bm{n}$ is the outward normal and $\bm{T}$ is a tangential projection operator. 
...
and $A$ is a flow rate factor. Glen's exponent has been chosen $n=1$, which provides for a linear Stokes model. The domain geometry $\Omega$ is shown in Figure \ref{fig:iceMountainMesh}. 

The inverse problem that we seek to solve is

\begin{align*}
\min_{\beta}J(\beta)&:=\frac{1}{2}\int_{\Gamma_{t}}||\bm{T}(\bm{u}-\bm{u}_{d})||_{2}^{2}\,dS+\frac{1}{2}\int_{\Omega}\beta\mathcal{L}^{2}\beta\,dx,\\
...
\end{align*}
The observation data $\bm{u}_{d}(\bm{x})$ is generated synthetically, as follows, we propose a true log basal friction field $\beta_{\text{true}}(\bm{x})$, solve equation \ref{Stokeseqn} to obtain $\bm{u}_{\text{true}}(\bm{x})$, we then add random relative noise to obtain the noisy data $\bm{u}_{d}$
...
\begin{figure}[H]
	...
	\end{subfigure}
\caption{TO DO: description of ice mountain}
\label{fig:iceMountainMesh}
\end{figure} 
...
%\end{subfigure}
\caption{The parameter after $1$ (left), $3$ (second from left), $4$ (third from left) Newton iterates and the final Newton iterate (right).} 
\end{figure} 

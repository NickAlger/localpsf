...
\begin{equation}
\label{eq:phi_b}
	\combresponse_b = \left\langle\mathcal{A},\sum_{x_i \in S_b} \delta_{x_i}\right\rangle^* = \sum_{x_i \in S_b} \langle\mathcal{A}, \delta_{x_i}\rangle^* = \sum_{x_i \in S_b} \phi_{x_i}.
\end{equation}
...
\item For each function $\psi_i$, we compute a function $\secondgreens_i$ by solving another Neumann Poisson problem:
\begin{equation}
\label{eq:theta_eq}
\begin{cases}
\Delta \secondgreens_i = \psi_i, & \text{in }\Omega, \\
...
\begin{proof}[Proof of Theorem \ref{alg:weighting_functions_incremental}]
	...
	= \beta - \sum_{k=1}^r \lambda_k \secondgreens_k(x_j) - e_j.
	\end{equation*}
	Using Lemma \ref{lem:psi_delta_eval} with $h=\secondgreens_k$, the equation for $\secondgreens_k$ in \eqref{eq:theta_eq}, and the definition of $S$ in \eqref{eq:defn_of_S_alg}, we have
	\begin{align*}
	\sum_{k=1}^r \lambda_k \secondgreens_k(x_j) &= \sum_{k=1}^r \lambda_k \left(\firstgreens_j, \Delta \secondgreens_k\right)_{L^2(\Omega)}\\
	&= \sum_{k=1}^r \lambda_k \left(\firstgreens_j, \firstgreens_k\right)_{L^2(\Omega)} = \left(S \lambda\right)_j.
	\end{align*}
	...
	&= \left(\Delta w, \Delta h\right)_{L^2(\Omega)} + \sum_{j=1}^r \lambda_j \left(\firstgreens_j, \Delta h\right)_{L^2(\Omega)} + \lambda_j \avg(h) \nonumber \\
	&= \left(\Delta w + \sum_{j=1}^r \lambda_j \firstgreens_j,~ \Delta h\right)_{L^2(\Omega)} + \avg(h) \mathbf{1}^T \lambda. \label{eq:L_w_38}
	\end{align}
	...
	\begin{equation*}
	\Delta w = -\sum_{j=1}^r \lambda_j \Delta \secondgreens_j 
	= -\sum_{j=1}^r \lambda_j \firstgreens_j,
	\end{equation*}

...
Once the scattered impulse responses $\impulseresponse_{x_i}$ are computed, we approximate the integral kernel $\Phi(y,x)$ at arbitrary points $(y,x)$ by interpolation. The details of the interpolation will be described in Section SECTION, but the key idea is something that we call \emph{local mean-displacement invariance}, which is the widely applicable assumption that
\begin{equation}
\Phi(y,x) = \phi_x(y) \approx \phi_{x_i}(y - \mu(x) + \mu(x_i))
\end{equation}
if $x_i$ is close to $x$. This is shown in Figure FIG, and discussed in greater detail in Section SEC. We approximate $\phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for the $k$-nearest points $x_i$ to $x$. Here $k$ is a user-defined parameter; we use $k=10$. The weights in the linear combination are determined via radial basis function interpolation.
...
\subsubsection{Impulse response moments and support estimation}
...
We compute impulse responses $\impulseresponse_{x_i}$ in batches by applying $\Aop$ to a small number Dirac combs. The Dirac comb, $\diraccomb_k$, associated with a batch of sample points, $\pointbatch_k$, is the following weighted sum of Dirac distributions (point sources) centered at the points $x_i \in \pointbatch_k$:
\begin{equation*}
	\diraccomb_k := \sum_{x_i \in \pointbatch_k} \delta_{x_i} / V(x_i).
\end{equation*}
The support of an impulse response is estimated to be contained within an ellipsoid, but this is just an approximation. A small portion of the impulse response will reside outside of the ellipsoid. If the impulse responses vary widely in scale, this neglected portion of an impulse response at a point $x_i$ with large $V(x_i)$ could overwhelm nearby impulse responses at points $x_j$ with small $V(x_j)$. Each point source is weighted by $1/V(x_i)$ so that the impulse responses within a batch are all comparable in scale.

For each batch $\pointbatch_k$, we compute the \emph{impulse response batch}, $\eta_k$, by applying $\Aop$ to the associated Dirac comb:
\begin{equation}

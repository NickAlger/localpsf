...
\end{equation}
if $x_i$ is close to $x$. This is shown in Figure FIG. We approximate $\phi_x(y)$ by a weighted linear combination of the values $\phi_{x_i}(y - \mu(x) + \mu(x_i))$ for the a small number of sample points $x_i$ near $x$. The weights in the linear combination are determined via radial basis function interpolation.
...
We define a further approximation, $\widetilde{\Phi}^h \approx \widetilde{\Phi}$, as follows:
\begin{equation}

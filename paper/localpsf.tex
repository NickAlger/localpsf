...
%If there are too many negative numbers, or if the negative numbers are too large, the proposed method will yield less accurate approximations and may potentially fail. 
...
basal sliding field (large $\basalfriction$ corresponds to high resistance to sliding) defined on the surface $\Gamma_b$. The quantity $\bodyforce$ is the body force density due to gravity, $\stokesrobincoeff=10^6$ is a Robin boundary condition constant,  $\normalvec$ is the outward normal and $\tangentop$ is the tangential projection operator that restricts a vector field to its tangential component along the boundary. 
...
%PCG converges much slower with no preconditioning and regularization preconditioning than it does with PSF-based preconditioning, with no preconditioning outperforming regularization preconditioning. 

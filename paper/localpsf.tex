...
Per the discussion in Section \ref{eq:local_mean_displacement_invariance}, if we ignore the factor $\frac{V(x)}{V(x_i)}$, then $\Phi(y,x) \approx f_i$ when $x_i$ is close to $x$. The factor $\frac{V(x)}{V(x_i)}$, which was not discussed in Section \ref{eq:local_mean_displacement_invariance}, is a form of preconditioning that improves this approximation if $V(x)$ varies widely. 
...
\end{equation}
at the point $x$. This is done using the following radial basis function interpolation scheme:
...
\end{cases}
\end{equation}
for $r>0$, and $b_i(0):=0$. Polyharmonic spline radial basis function interpolation yields the smoothest interpolant of the given points and values, in a certain least-squares sense. For more details on radial basis function interpolation and polyharmonic splines, see \cite{rbf}.
%\begin{equation}

...
\end{equation}
By linearity, the resulting function $\combresponse_k$ may be written as
\begin{equation}
	\label{eq:phi_b}
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
Using these weighting functions yields the ``flattest possible'' interpolation of the convolution kernels $\convkernel_i$, in a least-squares sense (Theorem \ref{thm:smoothest_interpolant}). Also, these weighting functions sum to one pointwise (Proposition \ref{thm:wi_sum_to_one}).

%For each $i=1,\dots,\convrank$, optimization problem \eqref{eq:wi_optimization_problem} is an example of the following generic optimization problem:
%\begin{equation}
%	\label{eq:wi_optimization_problem_generic}
%	\begin{aligned}
%		\min_{v \in H^2_N(\Omega)} &\quad \frac{1}{2}\norm{\Delta v}_{L^2(\Omega)}^2 \\
...
%	\end{aligned}
%\end{equation}
%where $\interpolatedvalues_j \in \mathbb{R}$, $j=1,\dots,\convrank$ are given function values that $v$ must take at the points $x_j$. 

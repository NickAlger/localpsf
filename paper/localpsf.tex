...
In order to use the product-convolution approximation $\widetilde{\mathcal{A}}$, we convert $\widetilde{\mathbf{A}}$ (the discretized version of $\widetilde{\mathcal{A}}$) to hierarchical matrix (H-matrix) format. Many classes of hierarchical matrices exist---H1, uniform H1, H2, HODLR, HSS, HBS, and more \cite{HMATRIX}. We use H1 matrices in our numerical results, but any of the other H-matrix formats could be used instead. The basic ideas behind H-matrices are as follows:
\begin{itemize} 
...
\begin{itemize}
	\item GLR vs PCH diffusion time (H)
	\item GLR vs PCH number of obs (Computational cost) (S)
	\item GLR vs PCH aspect ratio (CC) (S)
	\item mesh scalability of PCH (CC) (H)
	\item spectrum plots $P^{-1} H - I$ (Quality of approximation) (H, not Stokes?)
	\item number of krylov iterations: reg precond vs PCH (where?) (H)
	\item turn on preconditioner after number of krylov iterations exceeds 15 in an iteration. Rebuild "as needed". (H)
	\item $P - H = USV^T$ <- less good
	\item $P^{-1} H - I = USV^T$ <- better
	\item true parameter (H)
	\item deterministic reconstruction (H)
	\item noisy measurements (1\% noise). $u|_\text{top}$ (H)

...
\end{align}
Approximations \eqref{eq:translate1} and \eqref{eq:translate2} are standard. Approximation \eqref{eq:translate3}, which we use, is new. We call operators that satisfy \eqref{eq:translate3} ``locally mean-displacement invariant.'' We illustrate local mean-displacement invariance in Figure \ref{fig:mean_displacement_invariance}.
...
Each point source $\delta_{x_i}$ in the Dirac comb is scaled by $1/V(x_i)$ so that the resulting scaled impulse responses within $\eta_k$ are comparable in magnitude. Note that we are not in danger of dividing by zero, because our ellipsoid packing procedure from Section SEC excludes $x_i$ from consideration as a sample point if $V(x_i)$ is smaller than a predetermined threshold. Without this scaling, the portion of $\phi_{x_i}$ outside of $E_{x_i}$, which we neglect, may overwhelm $\phi_{x_j}$ for a nearby point $x_j$ if $V(x_i)$ is much larger than $V(x_j)$. 

...
We approximate kernel entries $\Phi(y,x)$ by preconditioned radial basis function interpolation of the impulse response values
\begin{equation}
\label{eq:phi_mu_shift32}
	\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right),
\end{equation}
where the points $x_i$ are sample points at which $\phi_{x_i}$ is known. Interpolation using all sample points $x_i$ is too expensive, so we only use the $k$ nearest sample points to $x$, excluding points $x_i$ for which \eqref{eq:phi_mu_shift32} is not defined. That is, we exclude points $x_i$ such that $y - \mu(x) + \mu(x_i) \notin \Omega$. In numerical experiments we use $k=10$. The $k$ nearest sample points are found by querying a precomputed KD-tree [CITE] of all sample points. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) [CITE] of the mesh cells used to discretize the problem.
...
By including the factor $\frac{V(x)}{V(x_i)}$, we are implicitly performing polyharmonic spline radial basis function interpolation of the ``volume preconditioned'' function $\frac{1}{V(x)}\Phi(y,x)$, then rescaling the result by $V(x)$ to get an approximation of $\Phi(y,x)$. Volume preconditioning improves the approximation considerably when $V(x)$ varies widely in magnitude, as is commonly the case for many operators of practical interest. In the Hessian for the Stokes inverse problem, for example, $V(x)$ varies over six orders of magnitude.

The point $y - \mu(x) + \mu(x_i)$ typically is not a gridpoint of the mesh used to discretize the problem, even if $y$, $x$, and $x_i$ are gridpoints. Although it is advisable to avoid evaluating finite element functions at arbitrary points [CITE], here we are forced to do this. We evaluate $\frac{1}{V(x_i)}\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
...
If $\Aop$ is symmetric, impulse responses of $\Aop$ are also impulse responses of $\Aop^T$, and we can use this fact to improve the interpolation accuracy at negligible additional cost.

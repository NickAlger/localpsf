...
\begin{align}
	\phi_x(y) &\approx \phi_{x_i}(y) \label{eq:translate1}\\
	\phi_x(y) &\approx \phi_{x_i}(y-x+x_i) \label{eq:translate2}\\
	\phi_x(y) &\approx \phi_{x_i}\left(y-\mu(x)+\mu(x_i)\right). \label{eq:translate3}
\end{align}
Operators with smooth integral kernels satisfy \eqref{eq:translate1}. These operators can be well-approximated by low-rank decompositions of the form
\begin{equation}
	\label{eq:low_rank_interpolation}
	\Phi(y,x) \approx \sum_{i=1}^r w_i(x) \phi_{x_i}(y)
\end{equation}
where $w_i$ are smooth weighting functions that interpolate known impulse responses $\phi_{x_i}$. Unfortunately, the required rank, $r$, for these interpolative-type decompositions can be extremely large in practical applications, such as Hessians in inverse problems with highly informative data. Moreover, if $\Aop$ is numerically low rank, then one would typically use more accurate and robust methods matrix-free methods for forming low-rank approximations, such as randomized SVD or Lanczos.

Operators that satisfy \eqref{eq:translate2} are called ``locally translation-invariant.'' Kernels for these operators can be well-approximated by decompositions of the form
\begin{equation}

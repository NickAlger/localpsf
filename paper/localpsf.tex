...
where $\{x_i\}_{i=1}^k$ are the $k$ nearest sample points to $x$, excluding sample points for which $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is undefined. We approximate $\Phi(y,x)$ by interpolating the (point,value) pairs
\begin{equation}
\left(x_i, f_i\right), \qquad i=1,\dots,k,
\end{equation}
at the point $x$. The idea here is that $\Phi(y,x) = \phi_x(y) \approx \phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ (see Section \ref{eq:local_mean_displacement_invariance}), so we approximate $\Phi(y,x)$ by interpolating the values $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$. Inclusion of the factor $\frac{V(x)}{V(x_i)}$ in \eqref{eq:fxyxp} is a form of preconditioning that makes this approximation more robust in the case where $V(x)$ varies widely.

To find the $k$ nearest sample points to $x$, we query a precomputed KD-tree \cite{kdtree} of all sample points. The number $k$ is a small user defined parameter; in numerical experiments we use $k=10$. The function $\phi_{x_i}\left(y - \mu(x) + \mu(x_i)\right)$ is defined if and only if $y-\mu(x)+\mu(x_i) \in \Omega$. We check whether $y - \mu(x) + \mu(x_i) \in \Omega$ by querying a precomputed axis aligned bounding box tree (AABB tree) \cite{aabbtree} of the mesh cells used to discretize the problem. 
...
For interpolation, we use the following radial basis function scheme:
...
\end{cases}
\end{equation}
for $r>0$, and $b_i(0):=0$. Polyharmonic spline radial basis function interpolation yields the smoothest interpolant of the given points and values, in a least-squares sense. For more details on radial basis function interpolation and polyharmonic splines, see \cite{rbf}.
%\begin{equation}

...
Computing $\mathbf{\Pi}=\Pi(\mathbf{A}_H^{\text{sym}})$ directly is difficult, but we may approximate $\Pi$ by the following high order rational function:
\begin{equation*}
	\Pi(\lambda) \approx \Pi_k(\lambda) := \frac{1}{1 + T(\lambda)^{2^k} }, \qquad \text{where} \qquad T(\lambda) := \frac{2 \lambda - (b+a)}{b-a},
\end{equation*}
and $k$ is a positive integer; the larger $k$ is, the better the approximation $\Pi \approx \Pi_k$. This is illustrated in Figure \ref{fig:spd_rat}. Replacing scalar operations with matrix operations, we have
\begin{equation}

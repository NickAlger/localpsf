...
\end{align}
We use approximation \eqref{eq:translate3}, which we call ``locally mean-displacement invariance.'' This is illustrated in Figure \ref{fig:mean_displacement_invariance}.
...
\section{Method}
\label{sec:method}
In this section we describe how we compute the impulse response moments $V,\mu,\Sigma$ and use these moments to estimate the supports of the impulse responses $\phi_x$ (Section \ref{eq:mean_and_covariance_estimation}), the greedy ellipsoid packing procedure we use to choose batches of sample points $x_i$ (Section \ref{sec:sample_point_selection}), how we compute impulse responses, $\phi_{x_i}$, in batches (Section \ref{sec:get_impulse_response}), how we interpolate the $\phi_{x_i}$ to approximate integral kernel entries $\Phi(y,x)$ (Section \ref{sec:approximate_kernel_entries}), finite element discretization of $\Phi$ (Section \ref{sec:finite_element_kernel}), and how we construct the H-matrix approximation $\mathbf{A}_h$ of the operator $\mathcal{A}$. We also describe how to (optionally) modify $\mathbf{A}_h$ to force it to be symmetric positive semi-definite. The complete algorithm is shown in Algorithm \ref{alg:construct_Atilde}. 
...
\end{equation}
The last equality in \eqref{eq:dirac_comb_H_action} follows from linearity and the definition of $\impulseresponse_{x_i}$ in \eqref{eq:impulse_response_delta_action}.
%\begin{equation}
%\label{eq:phi_b}
...
Each point source $\delta_{x_i}$ in the Dirac comb is scaled by $1/V(x_i)$ so that the resulting scaled impulse responses within $\eta_k$ are comparable in magnitude. Note that we are not in danger of dividing by zero, because our ellipsoid packing procedure from Section \ref{sec:sample_point_selection} excludes $x_i$ from consideration as a sample point if $V(x_i)$ is smaller than a predetermined threshold. Without this scaling, the portion of $\phi_{x_i}$ outside of $E_{x_i}$, which we neglect, may overwhelm $\phi_{x_j}$ for a nearby point $x_j$ if $V(x_i)$ is much larger than $V(x_j)$. 
...
In Section \ref{sec:intro} we noted that the integral kernel, $\Aker$, is inaccessible. Here we describe how we compute approximate kernel entries,
\begin{equation*}
...
We evaluate the approximate kernel entry $\widetilde{\Phi}(y,x)$ by performing radial basis function interpolation of the function $f_{y,x}(x')$ in the variable $x'$. The data used in the interpolation are the following (point,value) pairs
\begin{equation}
...
\begin{equation}
\widetilde{\Phi}(y,x) := \sum_{i=1}^k w_i~ b\left(\|x-x_i\|\right),
\end{equation}
...
\begin{equation*}
B_{ij} := b\left(\|x_i - x_j\|\right), \qquad i,j=1,\dots,k,
\end{equation*}
and the right hand side vector $f \in \mathbb{R}^k$ has entries 
\begin{equation*}
f_i = f_{y,x}(x_i), \qquad i=1,\dots,k.
\end{equation*}
We use polyharmonic spline radial basis functions \cite{polyspline}, which are defined as follows:
\begin{equation}
b_i(r) := \begin{cases}
r^d, & d=1,3,5,\dots \\
...
To generate the right hand side vector $f$, we evaluate $f_{y,x}(x_i)$ at the points $x_i$ via the following process. First, we determine whether $y - \mu(x) + \mu(x_i) \in E_{x_i}$ by checking if
\begin{equation}
...
\end{equation}
in the same manner as before, but with $x$ and $y$ now taking opposite roles. We combine both of these interpolation schemes to improve the approximation accuracy at negligible additional. The two interpolation schemes are combined by placing function values 
\begin{equation*}
...
\end{bmatrix},
\end{equation*}
be a $2k \times 2k$ block matrix and $2k \times 1$ block vectors, respectively, with the block entries of $\widehat{B}$ be given by
\begin{align*}
...
We solve the regularized least squares problem by QR factorization of the block $4k \times 2k$ matrix $\begin{bmatrix}\widehat{B} & I\end{bmatrix}^T$. We regularize the problem because of numerical issues that arise if there are points $\widehat{x}_i$ and $\widehat{y}_j$ in the interpolation that are very close to each other, a situation that arises often in practice. Indeed, situations where $\widehat{x}_i = \widehat{y}_j$ are inevitable when a regular grid is used to discretize the problem. In this case, $\widehat{B}$ is (nearly) singular, so it is numerically ill-advised to solve the standard interpolation equation $\widehat{B} \widehat{w} = \widehat{f}$. 
...
\subsection{Finite element discretization of the integral kernel}
...
We now define a further approximation, $\widetilde{\Phi}^h \approx \widetilde{\Phi}$, as follows:
\begin{equation}
	\label{eq:defn_of_Akerpcmesh}
	\widetilde{\Phi}^h(y,x) := \sum_{i=1}^\fedim \sum_{j=1}^\fedim \widetilde{\Phi}(p_i,p_j) \febasis_i(y) \febasis_j(x).
\end{equation}
The kernel $\widetilde{\Phi}^h$ is the interpolation of $\widetilde{\Phi}$ onto $V_h \otimes V_h$. Error in the overall kernel approximation, $\widetilde{\Phi}^h \approx \Aker$, arises both because of error in the product-convolution approximation, and because of finite element discretization error incurred from interpolation onto $V_h \otimes V_h$. 

Let $u_h \in V_h$ and $v_h \in V_h$, and let $\bm{u}$ and $\bm{v}$ be the coefficient vectors for $u_h$ and $v_h$, respectively, with respect to the finite element basis $\febasis_1, \dots, \febasis_\fedim$. I.e., 
\begin{equation}
\label{eq:fem_coeff_basis}
u_h(x) = \sum_{i=1}^\fedim \bm{u}_i \febasis_i(x),
\end{equation}
and similar for $v_h$. Replacing $\Aker$ with $\widetilde{\Phi}^h$ within the definition of $\Aop$ in \eqref{eq:kernel_representation}, then performing straightforward but tedious algebraic manipulations, yields the following approximation:
\begin{equation}
...
where $\mathbf{\Phi} \in \mathbb{R}^{\fedim \times \fedim}$ is the following matrix of approximate kernel entries:
\begin{equation}
	\label{eq:Akerpcmat_entries}
	\mathbf{\Phi}_{ij} := \widetilde{\Phi}(p_i, p_j),
\end{equation}
and $\bm{M} \in \mathbb{R}^{\fedim \times \fedim}$ is the finite element mass matrix which has entries
\begin{equation*}
...
%\end{equation*}
The finite element matrix representation of of our approximation of $\Aop$ is therefore given by
\begin{equation}
...
We form our H-matrix approximation $\mathbf{A}_H \approx \mathbf{A}$ by converting $\mathbf{\Phi}$ and $\bm{M}$ to H-matrix format, then using fast H-matrix methods to multiply these matrices according to \eqref{eq:Amatpc_defn}. Once $\mathbf{A}_H$ is formed, useful matrix operations such as matrix-vector products, matrix-matrix addition, matrix-matrix multiplication, and matrix inversion may be performed using fast scalable H-matrix methods \cite{BORMHACKBUSCHBOOK}. We use H1 matrices in our numerical results, but any of the other H-matrix formats (such as H2, HODLR, HSS, HBS, and others \cite{HMATRIX}) could be used instead. For more details on H-matrices, we recommend \cite{HMATRIXGOOD}. 

We convert $\mathbf{\Phi}$ into H1 matrix format using the standard geometrical clustering/adaptive cross method implemented within the HLIBpro software package \cite{HLIBPRO}. Although $\mathbf{\Phi}$ is a dense $\fedim \times \fedim$ matrix, constructing the H-matrix approximation only requires evaluation of $O(\hmatrixrank^2 \fedim \log \fedim)$ entries of $\mathbf{\Phi}$, and these entries are computed via interpolation procedure described in Section \ref{sec:approximate_kernel_entries}. Here $\hmatrixrank$ is the rank of the highest-rank block in the H-matrix. A dense representation of $\mathbf{\Phi}$ is never formed. We describe this H-matrix construction process in detail in Appendix \ref{app:h_matrix}. We convert $\bm{M}$ to H-matrix format using standard H-matrix methods for sparse matrices implemented within HLIBpro, using the same recursive block partitioning structure as was used for $\mathbf{\Phi}$. 

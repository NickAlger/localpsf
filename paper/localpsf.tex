...
\section{Constructing the approximation}
\label{sec:method}
This section describes how we compute the convolution kernels, $\varphi_i$, and weighting functions, $w_i$, for the product-convolution approximation, $\widetilde{\mathcal{A}} \approx \mathcal{A}$, given in \eqref{eq:product_convolution}.
\begin{itemize}
\item We extract $\varphi_{x_i}$ for many points $x_i$ by applying $\mathcal{A}$ to Dirac combs associated with ``batches'' of points (Section \ref{sec:get_impulse_response}). 
...
\item Ensuring that the points $x_i$ are well separated from each other requires a-priori estimates of the supports of the functions $\langle\mathcal{A},\delta_{x}\rangle^*$ (Section \ref{eq:mean_and_covariance_estimation}). 
\item The weighting functions are defined as the smoothest functions (in a least-squares sense) that interpolate the points $x_i$; constructing the weighting functions requires solving two Poisson PDEs per point $x_i$ (Section \ref{sec:weighting_functions}).
\end{itemize}
The complete algorithm for constructing $\widetilde{\mathcal{A}}$ is shown in Algorithm \ref{alg:construct_Atilde}.
...
Since $\varphi_x$, and thus $\langle\mathcal{A},\delta_{x}\rangle^*$, is nonnegative, $\langle\mathcal{A},\delta_{x}\rangle^*$ is a scaled probability distribution. We compute the scaling factor, denoted $\alpha(x)$, for all $x$ simultaneously by applying $\mathcal{A}$ to a constant function. We then compute the spatially varying mean, $\mu(x)$, and covariance, $\Sigma(x)$ of the normalized version of $\langle\mathcal{A},\delta_{x}\rangle^*$, for all $x$ simultaneously, by applying $\mathcal{A}^T$ to a small number of linear and quadratic functions. We make the approximation that the support of $\langle\mathcal{A},\delta_{x}\rangle^*$ is contained within the ellipsoid
\begin{equation}
\label{eq:support_ellipsoid}
E_x := \{x' \in \Omega: (x' - \mu(x))^T \Sigma(x)^{-1} (x' - \mu(x)) \le \tau^2\},
\end{equation}
where $\tau$ is a small number of standard deviations; we use $\tau=3$. The ellipsoid $E_x$ is the set of points within $\tau$ standard deviations of the mean for the Gaussian distribution that has the same mean and covariance as the normalized version of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$. The fraction of the mass of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$ residing outside of $E_x$ is less than $1/\tau^2$ by Chebyshev's inequality, though we find this bound is conservative and far less mass resides in this region in practice. 

\begin{defn}
	We define the spatially varying volume, $\alpha$, the scaled impulse response, $\rho_x$, the spatially varying mean, $\mu$, and the spatially varying covariance, $\Sigma$, as follows:
	\begin{align*}
		\alpha:\Omega \rightarrow \mathbb{R}, &\qquad \alpha(x) := \int_{\Omega} \langle\mathcal{A}, \delta_x\rangle^*(z) dz, \\
		\rho_x:\Omega \rightarrow \mathbb{R}, &\qquad \rho_x := \langle\mathcal{A}, \delta_x\rangle^* \big/ \alpha(x), \\
		\mu:\Omega \rightarrow \mathbb{R}^d, &\qquad \mu(x) := \int_\Omega z \rho_x(z) dz, \\
		\Sigma:\Omega \rightarrow \mathbb{R}^{d \times d}, &\qquad\Sigma(x) := \int_\Omega (z - \mu(x))(z - \mu(x))^T \rho_x(z) dz.
	\end{align*}
\end{defn}

%The support of $\langle\mathcal{A},\delta_{x}\rangle^*$ is estimated to be contained within a small number of standard deviations of the mean for a Gaussian distribution with the same mean and covariance as the normalized version of $\langle\mathcal{A},\delta_{x}\rangle^*$.
...
%		\Return{$\mu$, $\Sigma$}
	...
	\caption{Compute volume $\alpha$, mean $\mu$, and covariance $\Sigma$}
...
% sample points $x_i$ and $x_j$ should be far enough apart from each other that the supports of $\left\langle\mathcal{A}, \delta_{x_i}\right\rangle^*$ and $\left\langle\mathcal{A}, \delta_{x_j}\right\rangle^*$ do not overlap. 

%Since it is too expensive compute the support of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$ directly, we instead estimate that the support of $\left\langle\mathcal{A}, \delta_{x}\right\rangle^*$ is contained in the ellipsoid
%\begin{equation}
%\label{eq:support_ellipsoid}
%	E_x := \{x' \in \Omega: (x' - \mu(x))^T \Sigma(x)^{-1} (x' - \mu(x)) \le \tau^2\},
%\end{equation}
...
Starting from a finite set of candidate points $P$, we select batches of sample points, $S_b$, using a greedy algorithm. To build a batch $S_b$, first we initialize $S_b$ as an empty set. Then we select a candidate point $p \in P$ that is the farthest away from all points in previous sample point batches. That is, $p$ is the maximizer of the following optimization problem:
\begin{equation*}
\max_{p \in P} \min_{x \in S_1 \cup \dots \cup S_{b-1}} \|p - x\|.
\end{equation*}
Once $p$ is selected, we remove $p$ from $P$. If $p$ is sufficiently far from all of the previously chosen points in the current batch, in the sense that $E_p \cap E_q = \{\}$ for all $q \in S_b$, and if $p$ is sufficiently far from the boundary, in the sense that $E_p \cap \partial \Omega = \{\}$, then we add $p$ to $S_b$. Otherwise we discard $p$. This process repeats until there are no more points in $P$.  This is detailed in Algorithm \ref{alg:point_choice}. We determine whether $E_p \cap E_q = \{\}$ using the fast ellipsoid intersection test described in Appendix \ref{sec:fast_ellipsoid_intersection_test}.

%We select a batches of sample points, $S_b$, from a finite set of candidate points $P$ using a greedy algorithm. Starting from an empty set, we build $S_b$ from candidate points $p$ from $P$, one point at a time. The current candidate point, $p$, is the point in $P$  
...
\begin{algorithm2e}
	...
	Start with a finite set of candidate points $P \subset \Omega$
	
	Initialize new empty batch of sample points, $S_b$
...
%		\If{$\text{point\_is\_acceptable}(p, S_b)$}{
		\If{$E_p \cap E_q \neq \{\}$ for all $q \in S_b$ and $E_p \cap \partial \Omega = \{\}$}{
			
			Add $p$ to $S_b$
			
			Remove all points $x$ satisfying $\mu(x) \in E_p$ from $P$
		}
	}
...
%	
%	$E_p$ is the ellipsoid associated with $p$ (see \eqref{eq:support_ellipsoid})
%	
%	\For{$q \in S_b$}{
%		
%		$E_q$ is the ellipsoid associated with $q$
%		
%		\If{$E_p \cap E_q \neq \{\}$}{
%			
...
%		}
%	}
%	\textbf{return} $p\text{\_is\_acceptable}$ 
%}
...
\label{eq:varphi_eval}
	\varphi_{x_i}(z) = \langle\mathcal{A}, \delta_{x_i}\rangle^*(z+x_i) = \begin{cases}
		...
		\end{cases}
\end{equation}
for all $x_i \in S_b$. By performing one matrix-vector product, \eqref{eq:dirac_comb_H_action}, we recover $\varphi_{x_i}$ for every point $x_i \in S_b$. To minimize memory usage, the functions $\varphi_i$ are never isolated and stored individually. Instead, we store the functions $\eta^b$, and use formula \eqref{eq:varphi_eval} to perform function evaluations $z \mapsto \varphi_i(z)$ as needed.
...
Let $y \in \mathbb{R}^d$, and let $u$ be the function
\begin{equation}
\label{eq:u_defn_in_thm}
u(x) := \sum_{i=1}^r \varphi_i(y)w_i(x) = \widetilde{A}\left(y+x,x\right)
\end{equation}
where the weighting functions $w_i$ are the solutions to optimization problem \eqref{eq:wi_optimization_problem}. Then $u$ solves the optimization problem
\begin{equation}
\label{eq:uy_optimization_problem}
\begin{aligned}
\min_{u \in H^2_N(\Omega)} &\quad \frac{1}{2}\norm{\Delta u}_{L^2(\Omega)}^2 \\
...
\begin{algorithm2e}
	...
	{	
		\For{$i=1,2, \dots, r$}{
			...
			\begin{equation}
			\label{eq:psi_eq}
			\Delta \psi_i = \delta_{x_i} - C/|\Omega|, \quad \text{in }\Omega,
			\end{equation}
			with Neumann zero boundary conditions, $\nu \cdot \nabla \psi_i = 0$ on $\partial \Omega$, and the average value zero condition, $\int_\Omega \psi_i(x) dx = 0$. Recall $\delta_{x_i}$ is a point source (delta distribution) centered at $x_i$, $C$ is the constant function $C(x)=1$, and $|\Omega|$ is the Lebesgue measure of $\Omega$. 
		...
		\For{$i=1,2,\dots,r$}{
			...
			\label{eq:definition_of_wi_alg}
				w_i(x) = \alpha - \sum_{k=1}^r \lambda_k \theta_k(x).
			\end{equation}
		}
	}
...
%	
%	The standard weak form for $\psi_j$ in \eqref{eq:psi_eq} is given by the following saddle point system for $\psi_j \in H^1(\Omega)$ and $\rho \in \mathbb{R}$:
%	\begin{alignat}{5}
%		-\int_\Omega \nabla \psi_j \cdot \nabla h ~dx &~+~& \rho \int_\Omega h ~dx &~=~ h(x_j) &\quad \forall h \in H^1(\Omega) \label{eq:lem_delta_eval_feq} \\
%		\int_\Omega \psi_j ~dx & & &~=~ 0. & \nonumber
%	\end{alignat}
%	This follows from integration by parts, using the Neumann zero boundary conditions for $\psi_j$, and enforcing the average value zero constraint for $\psi_j$ with the Lagrange multiplier $\rho$. If $h$ is the constant function $h(x)=1$, the first equation becomes
%	\begin{equation*}
%		\rho \int_\Omega 1 ~ dx = 1,
%	\end{equation*}
%	which implies $\rho = \frac{1}{|\Omega|}$. Now suppose that $h\in H^2_N(\Omega)$. Of course, we still have $h\in H^1(\Omega)$ as well, so equation \eqref{eq:lem_delta_eval_feq} still holds. But now $h$ has additional regularity, which allows us to use integration by parts on the first term in \eqref{eq:lem_delta_eval_feq} to bring all of the derivatives onto $h$. Furthermore, $h$ has Neumann zero boundary conditions, so the boundary term that arises during integration by parts is zero. Performing this integration by parts, and substituting in the value for $\rho$, yields
%	\begin{equation*}
%		\int_\Omega \psi_j \Delta h ~dx + \frac{1}{|\Omega|} \int_\Omega h ~dx = h(x_j),
%	\end{equation*}
...
\section{Using the approximation}
\label{sec:using_approximation}

\subsection{Evaluating kernel entries of the approximation}
\label{sec:eval_matrix_entries}

We now describe how to efficiently evaluate $\widetilde{A}(y,x)$ at an arbitrary point $(x,y) \in \Omega \times \Omega$. From \eqref{eq:H_kernel_entries}, we have
\begin{equation}
\label{eq:eval_y_x}
...
%	
%	Compute $\boldsymbol{\omega} \in \mathbb{R}^{r \times n}$, $\boldsymbol{\omega}_{i,k} = w_i(x_k')$
%	
%	\mbox{}\\
%	
...
%		\Return{$\mathbf{z}$}
%	}
%	\caption{Compute many approximate kernel entries $z_k = \widetilde{A}(y_k',x_k')$}
%	\label{alg:H_entries}
%\end{algorithm2e}

\subsection{Conversion to hierarchical matrix format}

In order to use the product-convolution approximation $\widetilde{\mathcal{A}}$, we convert $\widetilde{\mathcal{A}}$ to hierarchical matrix (H-matrix) format. Many classes of hierarchical matrices exist---H1, uniform H1, H2, HODLR, HSS, HBS, and more [CITES]. We use H1 matrices in our numerical results, but any of the other H-matrix formats could be used instead. The basic ideas behind H-matrices are as follows:
\begin{itemize} 
...
After reordering the rows and columns of $\widetilde{\mathcal{A}}$ via the depth-first ordering described above, we partition the reordered version of $\widetilde{\mathcal{A}}$ into a tree of $2 \times 2$ block matrices recursively. We use a geometric admissibility condition (discussed below) to decide which blocks to subdivide, and use the cluster trees for the rows and columns to determine how to subdivide those blocks. For the first stage of subdivision, let $r_1$ and $r_2$ be the children row clusters for the root of the row cluster tree, let $c_1$ and $c_2$ be the children column clusters for the root of the column cluster tree. The matrix $\widetilde{\mathcal{A}}$ is partitioned into blocks as follows:
\begin{equation*}
	\begin{bmatrix}
		\widetilde{\mathcal{A}}_{11} & \widetilde{\mathcal{A}}_{12} \\
		\widetilde{\mathcal{A}}_{21} & \widetilde{\mathcal{A}}_{22},
	\end{bmatrix}
\end{equation*}
where $\widetilde{\mathcal{A}}_{11}$ denotes the block of $\widetilde{\mathcal{A}}$ with rows $r_1$ and columns $c_1$, $\widetilde{\mathcal{A}}_{12}$ denotes the block of $\widetilde{\mathcal{A}}$ with rows $r_1$ and columns $c_2$, and so on for $\widetilde{\mathcal{A}}_{21}$ and $\widetilde{\mathcal{A}}_{22}$.

We now loop through the four blocks, $\widetilde{\mathcal{A}}_{11}$, $\widetilde{\mathcal{A}}_{12}$, $\widetilde{\mathcal{A}}_{21}$, and $\widetilde{\mathcal{A}}_{22}$, and decide which blocks should be subdivided further. For the purpose of explanation, consider $\widetilde{\mathcal{A}}_{12}$. If
\begin{equation}
	\label{eq:weak_admissibility_cond}
	\dist\left(r_1, c_2\right) \ge \eta \min\left(\diam\left(r_1\right), \diam\left(c_2\right)\right),
\end{equation}
then we mark $\widetilde{\mathcal{A}}_{12}$ as \emph{admissible} (expected to be low rank) and leave it alone. Here $\dist\left(r_1, c_2\right)$ is the Euclidean distance between the axis-aligned bounding box for the point cloud associated with the row cluster $r_1$, and the axis aligned bounding box for the point cloud associated with the column cluster $c_2$. The quantity $\diam\left(r_1\right)$ is the diameter of the axis aligned bounding box for the point cloud associated with the row cluster $r_1$, and $\diam\left(c_2\right)$ is the analogous diameter associated with the column cluster $c_2$. The quantity $\eta$ is a scalar constant; we use $\eta=2.0$. Basically, if the point clouds associated with $r_1$ and $c_2$ are far away from each other relative to their diameters, then we expect that the corresponding block of the matrix will be low rank. This process is repeated for the other blocks to determine which blocks are admissible and which are not. For us, the diagonal blocks $\widetilde{\mathcal{A}}_{11}$ and $\widetilde{\mathcal{A}}_{22}$ are not admissible because the distance between a point cloud and itself is zero. 

Next, we subdivide all blocks that are not admissible and are larger than a predetermined size (we use size $32 \times 32$), using the same process that was used to subdivide $\mathcal{\widetilde{A}}$. But now we subdivide a block based on the two child row clusters and two child column clusters for the rows and columns of that block. This subdivision process continues recursively until all blocks are either admissible, or smaller than the predetermined size mentioned above. The resulting hierarchical partitioning of matrix blocks forms a tree (quad-tree in $2$D, or oct-tree in $3$D), which is called the \emph{block cluster tree}. The root of the tree is the whole matrix, internal nodes in the tree are blocks that are subdivided, and the leaves of the tree are blocks that are either expected to be low rank, or are small.

%Note that the cluster trees and block cluster tree are computed using only the geometric information about the degree of freedom locations, and do not depend on the values of the matrix entries of $\widetilde{\mathcal{A}}$.

\subsubsection{Adaptive cross low rank approximation of blocks}
\label{sec:adaptive_cross}

Once the block cluster tree has been constructed, low rank approximations of the admissible (low rank) blocks are formed using the adaptive cross method [CITE]. Let $X \in \mathbb{R}^{m \times m}$ be an admissible block of $\widetilde{\mathcal{A}}$. The idea of the adaptive cross method is to form a low rank approximation of $X$ by sampling a small number of rows and columns of $X$. 
\begin{itemize}
...
\begin{lem}[Preparation for Theorem \ref{thm:smoothest_interpolant}]
	...
	Let $f \in \mathbb{R}^r$. There exists a unique solution to the following optimization problem:
	\begin{equation}
	...
	\mathcal{T}(f) := u^*,
	\end{equation*}
	where $u^*$ is the minimizer of \eqref{eq:generic_optimization_initial}. The operator $\mathcal{T}$ is a linear operator.
\end{lem}
\begin{proof}
	asdf
\end{proof}
...
\begin{proof}[Proof of Theorem \ref{thm:smoothest_interpolant}]
	...
	&= u.
	\end{align*}
	In the first line we used linearity of the solution map, $\mathcal{T}$ (Lemma \ref{lem:T_is_linear}), in the second line we used the fact that $w_i = \mathcal{T}(e_i)$, and in the third line we used the definition of $u$ in \eqref{eq:u_defn_in_thm}.
\end{proof}
...
\begin{proof}[Proof of Theorem \ref{alg:weighting_functions_incremental}]
	...
	= \alpha - \sum_{k=1}^r \lambda_k \theta_k(x_j) - e_j.
	\end{equation*}
	...
	\begin{align*}
	\nabla_\lambda \mathcal{L}(w, \lambda) &= \alpha \mathbf{1} - e - S \lambda \\
	&= \alpha \mathbf{1} - e - S \left(-S^{-1}\left(e - \alpha \mathbf{1}\right)\right) = 0.
	\end{align*}
	...
	\end{equation*}
	which implies that the first term in \eqref{eq:L_w_38} is zero. By the definition of $\lambda$ in \eqref{eq:definition_of_mu_alg} and $\alpha$ in \eqref{eq:definition_of_alpha_alg}, we have
	\begin{align*}
	\mathbf{1}^T \lambda &= -\mathbf{1}^T S^{-1} \left(e - \alpha \mathbf{1}\right) \\
	&= -\mathbf{1}^T S^{-1} e + \alpha \mathbf{1}^T S^{-1} \mathbf{1} \\

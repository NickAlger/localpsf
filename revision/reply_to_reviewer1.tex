...
\noindent{
	...
	In Figure \ref{fig:marmousi_impulse} here (but not in the paper due to page limitations), we show an impulse response for the Gauss-Newton Hessian for the Marmousi wave inverse problem. Note that the non-local component of the impulse response causes the ellipsoid impulse response support estimate to be effectively the entire domain.
	...
	\begin{figure}
		...
		\caption{Impulse response for the Gauss-Newton Hessian for the Marmousi wave inverse problem. Data provided by Mathew Hu. On the left we show the impulse response on the whole domain. The color scale is adjusted for better visualization of the non-local transmission components. Estimated ellipsoids with $\tau\in \{1.0, 2.0, 3.0\}$  are shown. On the right we show a zoomed in impulse response, showcasing the negative numbers in the integral kernel.
		}

...
\item \referee{Interesting work, generally well-written. Perhaps a
  ...
  \noindent{
  	Thank you for this suggestion, which we agree can help the reader better understand the intuition behind the method. We added a 2D blurring kernel example, with figures illustrating the end-to-end PSF method. These are shown in the introduction and the numerical results sections (Sections 1 and 7.1).
%  	Thank you for this suggestion! We added a 2D blurring kernel example, with figures illustrating the end-to-end PSF method. These are shown in the introduction and the numerical results sections (Sections 1 and 7.1). We think that adding these illustrations helps the reader better understand the intuition behind the method.
%  	Frog example with a=1.
%  	Structure: local non-negative kernel
%(0) Matrix itself column highlighted, show impulse response (kernel\_impulse\_illustration.pdf)
%(1) moments. Show one impulse with ellipsoid with widths illustrated
%(2) impulse response batches
%(3) RBF interpolation (?), evaluation of entries. Columns we know, and entry we don't know. 
%(4) H-matrix (structure only)
}

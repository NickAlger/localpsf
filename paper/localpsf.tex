...
In order to use the product-convolution approximation $\widetilde{\mathcal{A}}$, we convert $\widetilde{\mathcal{A}}$ to hierarchical matrix (H-matrix) format. Many classes of hierarchical matrices exist---H, uniform H1, H2, HODLR, HSS, HBS, and more. We use H1 matrices in our numerical results, but any of the other H-matrix formats could be used instead. The basic ideas behind H-matrices are as follows:
\begin{itemize} 
...
We use the standard geometrical clustering/adaptive cross H1 matrix construction method implemented within the HLibPro software package [CITE] to convert $\widetilde{\mathcal{A}}$ into H1 matrix format. We now summarize this method.

\paragraph{Cluster trees} First, the degrees of freedom associated with the columns of the matrix are hierarchically clustered into a \emph{cluster tree} using recursive hyperplane splitting. Since we use finite elements to discretize the problem, we associate the $i^\text{th}$ column of $\widetilde{\mathcal{A}}$ with the nodal point in $\mathbb{R}^d$ associated with the $i^\text{th}$ finite element basis function. The columns of the matrix thus correspond to a point cloud in $\mathbb{R}^d$. We split this point cloud into two roughly equally sized \emph{child} point clouds, using a hyperplane which is perpendicular to the coordinate axis direction in which the point cloud is widest (e.g., either the $x$, $y$, or $z$ axis in $3$D). The two child point clouds are split in the same way. This splitting process repeats recursively until the point clouds have less than a preset number of points (we use $32$ points). This hierarchical partitioning of the point cloud into smaller and smaller point clouds corresponds to a hierarchical partitioning of the columns of the matrix into smaller and smaller \emph{clusters} of columns, which we call a cluster tree. A depth-first search ordering of the cluster tree is then generated. When the columns of the matrix are permuted into this ordering, the columns associated with each cluster in the cluster tree are contiguous.
...
\paragraph{Block cluster tree} Second, we partition the matrix (after reordering the rows and columns via the depth-first ordering described above) into a recursive hierarchy of mostly low rank blocks called the \emph{block cluster tree}.
The idea is that the interaction between far-away groups of degrees of freedom may be characterized by a small number of bulk properties of those groups.  

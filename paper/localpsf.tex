...
\end{equation*}
If $\Aop$ is also positive semi-definite, we adapt the high order rational function spectral projection method from \cite{HackbuschKress07} to flip (or approximately flip) the negative eigenvalues of $\mathbf{A}_H^{\text{sym}}$ to be positive. In detail, let $\Pi$ be the indicator function for the interval $(a,b)$,
\begin{equation*}
...
Computing $\mathbf{\Pi}=\Pi(\mathbf{A}_H^{\text{sym}})$ directly is difficult, but we may approximate $\Pi$ by a high order rational function $\Pi_k \approx \Pi(\lambda)$ which is defined as follows:
\begin{equation*}
...
where
\begin{equation*}
\mathbf{T} := \left(2 \mathbf{A}_H^{\text{sym}} - (b+a) \mathbf{I}\right)/(b-a).
\end{equation*}
...
First, we compute $\lambda_\text{min}$ using the Lanczos method, and set $a=\gamma \lambda_\text{min}$ and $b=0$, where $\gamma\ge 1$ is a parameter that must be chosen. We use $\gamma=1.5$, though the method is relatively insensitive to $\gamma$ and works well for any $\gamma \in [1,2]$. 
Next, we compute $\mathbf{T}$ using fast H-matrix scalar multiplication and addition. We form the $\mathbf{T}^{2^k}$ via repeated squaring, $\mathbf{T}^2 = \mathbf{T} \mathbf{T}$, $\mathbf{T}^4 = \mathbf{T}^2 \mathbf{T}^2$, $\mathbf{T}^8 = \mathbf{T}^4 \mathbf{T}^4$ and so on, where each of these matrix squarings is performed using fast H-matrix multiplication. We recommend using $k=1$ or $2$ (we use $k=1$ in our numerical results). Larger $k$ is unnecessary and may make the method unstable. The approximate spectral projector $\mathbf{\Pi}_k$ is formed from $\mathbf{T}^{2^k}$ per \eqref{eq:rational343}, using fast H-matrix operations to perform the required matrix-matrix addition and matrix inversion. Finally, a positive semi-definite approximation $\mathbf{A}_H^{\text{sym}+}$ of $\mathbf{A}_H^{\text{sym}}$ is formed as
\begin{equation*}
...
In applications, often one wants to use the H-matrix approximation $\mathbf{A}_H$ to build preconditioners for solving a sequence of slowly varying linear systems of the form
\begin{equation}
\label{eq:Ax_equals_b}
\mathbf{B} \mathbf{u} = \mathbf{f},
\end{equation}
where each of these linear systems is solved using an iterative method. If $\mathbf{B}=\mathbf{A}$, then $\mathbf{A}_H^{-1}$ may be used to precondition this linear system. If $\mathbf{B}$ is built from a combination of $\mathbf{A}$ and other operators, then $\mathbf{A}_H$ may be combined with H-matrix approximations of the other operators to construct a preconditioner for $\mathbf{B}$. For example, in a Newton-Krylov method for solving an optimization problem, one uses a Krylov method such as conjugate gradient to solve a sequence of systems of the form \eqref{eq:Ax_equals_b}, where $\mathbf{B}$ is the Hessian of the objective function in the optimization problem, and $\mathbf{f}$ is the negative gradient of the objective function. Both the Hessian and the gradient are evaluated at the current Newton iterate, so $\mathbf{B}$ and $\mathbf{f}$ change as the Newton iterations proceed. The Hessian is typically the sum of a data-misfit term which is available matrix-free, and a regularization term which is available directly as a sparse matrix. The data misfit term in the Hessian is approximated with our method to form $\mathbf{A}_H$, the regularization term is approximated using standard H-matrix methods for sparse matrices, and then these two terms are added together and inverted using H-matrix methods to form a preconditioner for the overall Hessian. Let 
\begin{equation*}
\mathbf{P} \approx \mathbf{B}^{-1}
\end{equation*}
denote the H-matrix preconditioner used to accelerate the solution of \eqref{eq:Ax_equals_b} with an iterative method. The iterative method will typically involve, as a sub-procedure, applying $\mathbf{B}$ to a collection of vectors $\{\mathbf{x}_i\}_{i=1}^m$, thereby generating vectors $\{\mathbf{y}_i = \mathbf{B} \mathbf{x}_i\}_{i=1}^m$. Instead of rebuilding the H-matrix preconditioner for each successive linear system, we recommend \emph{recycling} the information generated by the iterative method while solving the current linear system, in order to update the preconditioner $\mathbf{P}$ for the next linear system. In detail, let $\mathbf{X}$ and $\mathbf{Y}$ denote the $N \times m$ matrices with columns given by the vectors $\mathbf{x}_i$ and $\mathbf{y}_i$, respectively, so that the information learned about $\mathbf{B}$ while solving the current linear system may be summarized by the equation:
\begin{equation*}
\mathbf{B}\mathbf{X} = \mathbf{Y}.
\end{equation*}
For convenience, define 
\begin{equation*}
\mathbf{R} := \mathbf{X}-\mathbf{P}\mathbf{Y}, \qquad \text{and} \qquad \boldsymbol{\Theta} := \left(\mathbf{X}^T\mathbf{Y}\right)^{-1}.
\end{equation*}
One of the following three formulas may be used to perform a low rank update of $\mathbf{P}$:
\begin{align}
	\mathbf{P} &\gets \mathbf{P} + \mathbf{R}\left(\mathbf{Y}^T \mathbf{Y}\right)^{-1} \mathbf{Y}^T \label{eq:res_update} \\
	\mathbf{P} &\gets \mathbf{P} + \mathbf{R} \left(\mathbf{R}^T \mathbf{Y}\right)^{-1} \mathbf{R}^T \label{eq:srk_update} \\
	\mathbf{P} &\gets \left(\mathbf{I} - \mathbf{X}\boldsymbol{\Theta}\mathbf{Y}^T\right)\mathbf{P}\left(\mathbf{I} - \mathbf{Y}\boldsymbol{\Theta}\mathbf{X}^T\right) + \mathbf{X}\boldsymbol{\Theta}\mathbf{X}^T \label{eq:dfp_update}
\end{align}
In all three cases, direct calculation shows that we have $\mathbf{P}\mathbf{Y}=\mathbf{X}$ after the update, which implies $\mathbf{P}^{-1} \mathbf{X} = \mathbf{B}\mathbf{X}$. Formulas \eqref{eq:res_update} and \eqref{eq:srk_update} perform rank-$k$ updates of $\mathbf{P}$, while formula \eqref{eq:dfp_update} performs a rank-$2k$ update. Formula \eqref{eq:res_update} does not preserve symmetry, and should therefore be used when $\mathbf{B}$ is not symmetric. Formula \eqref{eq:srk_update} preserves symmetry but not positive definiteness, and should therefore be used when $\mathbf{B}$ is symmetric and indefinite. Formula \eqref{eq:dfp_update} preserves both symmetry and positive definiteness, and should therefore be used when $\mathbf{B}$ is symmetric positive definite. Formula \eqref{eq:srk_update} is known as the ``symmetric rank-$k$'' update formula. Formula \eqref{eq:dfp_update} is known as the DFP update formula, named after it's inventor Davidson, and polularizers Fletcher and Powell. For more details on these update formulas, we recommend the discussion in Chapter 6 of \cite{NocedalWright99}. In \cite{NocedalWright99}, one-vector versions of formulas \eqref{eq:srk_update} and \eqref{eq:dfp_update} are discussed thoroughly; the versions here are straightforward generalizations to many-vector updates.

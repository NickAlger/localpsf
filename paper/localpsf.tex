...
We use the framework described in this paper to generate Hessian preconditioners. We build H-matrix approximations of the data-misfit Gauss-Newton Hessian (the term in $H_\text{gn}$ that arises from the data misfit) using 1, 5, and 25 impulse responses batches. The resulting H-matrices, which may be indefinite due to approximation error, are modified to be symmetric positive definite via the procedure described in Appendix \ref{sec:make_spd}, with $l=2$. We approximate $H_\text{gn}$ rather than $H$ because $H$ often has negative values in its integral kernel when one is away from the optimal point. Our results show that our preconditioners are also good preconditioners for $H$. The H-matrix approximation of data misfit Gauss-Newton Hessian is added to a H-matrix approximation of the Hessian of the regularization term, $J_r$, to form a H-matrix approximation of the Gauss-Newton Hessian. For the Hessian of the regularization term, straightforward calculation shows that the discrete regularization term in both the Hessian and the Gauss-Newton Hessian is given by
$
(\gamma \bm{K} + \delta \bm{M}) \bm{M}^{-1} (\gamma \bm{K} + \delta \bm{M})
$
where $\bm{K}$ is a finite element stiffness matrix with appropriate boundary conditions, and $\bm{M}$ is the finite element mass matrix. These matrices are sparse, so we form H-matrix representations of these matrices using standard sparse H-matrix techniques. Then we invert $\bm{M}^{-1}$, and multiply these matrices together using H-matrix methods. We factor the overall Gauss-Newton Hessian approximation, denoted $\widetilde{H}$, using fast H-matrix methods, then and use the factorization as a preconditioner \cite{HLIBPRO}. The approximations for 1, 5, and 25 impulse response batches are denoted by PSF (1), PSF (5), and PSF (25), respectively. 

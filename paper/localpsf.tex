...
\end{equation*}
denote the spatially varying mean and covariance of $\widehat{\impulseresponse}_x$, respectively.

\subsection{Volume preconditioned local mean-displacement invariance}
...
the required rank in the low rank approximation can be large, which makes algorithms based on assumption \eqref{eq:translate1} computationally expensive. This is the case for Hessians in PDE constrained inverse problems with highly informative data, as discussed in Section \ref{sec:PDE_hessian_motivation}. 
...
\end{equation}
at the point $x$. The idea is that $\Phi(y,x) = \phi_x(y) \approx \phi_{x_i}\left(z_i\right)$ per the discussion in Section \ref{eq:local_mean_displacement_invariance}, so we approximate $\Phi(y,x)$ by interpolating the values $\phi_{x_i}\left(z_i\right)$. The factor $\frac{V(x)}{V(x_i)}$ in \eqref{eq:fxyxp} preconditions the approximation to make it more robust if $V(x)$ varies widely.

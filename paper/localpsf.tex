...
We present a fast matrix free method for approximating operators, $\Aop$, that have locally supported non-negative integral kernels. Such operators arise as Schur complements in Schur complements methods for solving partial differential equations (PDEs) \cite{TOBYPAPER}, Poincare-Steklov operators in domain decomposition methods (e.g., Dirichlet-to-Neumann maps), covariance operators in spatial statistics, blurring operators in imaging, and Hessians in distributed parameter PDE-constrained optimization and inverse problems.
...
\end{equation}
Here $\Aker:\Omega \times \Omega \rightarrow \mathbb{R}$ is the integral kernel, $\Aop u \in L^2(\Omega)'$ is the linear functional that results from applying $\Aop$ to $u\in L^2(\Omega)$, and $\left(\Aop u\right)(v)$ is the scalar that results from applying that linear functional to $v \in L^2(\Omega)$. We focus on operators $\Aop$ that are accessible \emph{matrix-free}. One cannot easily compute kernel entries $\Aker(y,x)$. Instead, 
...
\end{equation*} 
for arbitrary functions $u$ and $v$. 
...
In applications, evaluating these maps may involve iteratively solving a large linear system, timestepping, or performing some other nontrivial computational procedure. 
...
Our method belongs to a class of methods that approximate an operator by interpolating impulse responses of the operator at a scattered collection of points. The \emph{impulse response} of $\Aop$ associated with the point $x$ is the function $\impulseresponse_x:\Omega \rightarrow \mathbb{R}$, 
\begin{equation}
\label{eq:impulse_response_defn}
\impulseresponse_x(y) := \Aker(y, x),
\end{equation} 
which results from keeping $x$ fixed and reinterpreting $\Phi(y,x)$ as a function of $y$.
...
Our method assumes that the supports of the $\impulseresponse_x$ are contained (or approximately contained) in localized regions. Our method also assumes that the integral kernel is non-negative, i.e., $\Phi(y,x) \ge 0$ for all $(y,x) \in \Omega \times \Omega$. Because of the non-negativity assumption, we are able to estimate the supports of the functions $\phi_x$ a-priori by applying $\Aop$ to a small number of polynomial functions. This, in combination with the locality assumption, allows us to compute impulse responses in batches. We recover the impulse responses $\phi_{x_i}$ for a large number of points $x_i$ with a small number of operator actions $v \mapsto \Aop v$. We also leverage the non-negativity assumption to perform a more sophisticated and accurate interpolation of the impulse responses.

The output of our method is a hierarchical matrix (H-matrix) approximation, $\mathbf{A}_H$, of a discretization of $\Aop$. One may use H-matrix methods to perform further useful linear algebra operations involving $\mathbf{A}_H$ that cannot easily be done using $\Aop$ directly, because $\Aop$ is only available matrix-free. These operations include matrix-matrix addition, matrix-matrix multiplication, (faster) matrix-vector products, matrix factorization, and matrix inversion. H-matrix methods are fast and scale well to large problems. The ability to perform these operations permits, for example, fast solution of Newton linear systems in optimization, fast sampling of ill-conditioned posterior distribution in Bayesian inverse problems, and construction of high rank surrogate models that can be used for uncertainty quantification.
...
\subsection{Impulse responses and their moments}
...
\end{equation*}
to be the spatially varying mean and covariance of the normalized version of $\impulseresponse_x$, respectively. For each point $x$, imagine $\phi_x$ as a pile of sand. The scalar $V(x)$ is the mass of the sand pile, the vector $\mu(x)$ is the center of mass of the sand pile, and the matrix $\Sigma(x)$ contains information about how wide the sand pile is in each spatial direction. Throughout this paper we will use these spatially varying moment functions extensively.
...
Operators satisfying \eqref{eq:translate1} can be well-approximated via low-rank interpolative factorization/Nystrom approximation of $\Phi$ \cite{CUR,NYSTROM}.
% of the form
%\begin{equation}
...
\begin{equation*}
\Aop \longrightarrow V, \mu, \Sigma \longrightarrow \{x_i\}_{i=1}^r \longrightarrow \{\impulseresponse_{x_i}\}_{i=1}^r \longrightarrow \boldsymbol{\Phi}_H \longrightarrow \mathbf{A}_H.
\end{equation*}
...
Product-convolution approximations have been used in a wide variety of fields going back several decades \cite{PRODCONVLIST}.
For background on product-convolution approximations, we recommend reading the following papers: \cite{PRODCONVGOOD}. We build upon the class of product convolution approximations in which the convolution kernels are shifted impulse responses of $\Aop$ to point sources at a collection of points $x_i$ \cite{PRODCONVLIST}. A popular choice for methods in this class is to choose the points $x_i$ to be nodes in a regular grid, and interpolate the convolution operators with piecewise linear interpolation \cite{NAGY}, or splines \cite{PRODCONVSPLINES}. With a regular grid, a large number of points $x_i$ is typically required to achieve an accurate product-convolution approximation. In our previous work, we reduced the computational cost by starting with a coarse grid of points $x_i$, then adaptively refining the grid in the regions where the error in the approximation is large \cite{PRODCONVMYPAPER}. But even with adaptive refinement, many matrix vector products with $\Aop$ may be required. In this paper, rather than using adaptive refinement, we instead reduce the computational cost by picking points $x_i$ such that many $\convkernel_i$ are computed with each matrix vector product. 

Our method of estimating the support of the impulse responses was inspired by resolution analysis in seismic imaging \cite{RESOLUTION}. In resolution analysis, $\Aop$ is the Hessian for a seismic inverse problem, and the width of $\phi_p$ is used to estimate of the minimum length scale on which features of the parameter can be inferred near the point $p$. If the width of $\phi_p$ is ten meters, then features of the parameter near $p$ can be accurately inferred from data if those features are larger than roughly ten meters in size. In \cite{RESOLUTION}, the width of $\phi_p$ is estimated to be the local autocorrelation length of the function $\Aop^T \zeta$ near $p$, where $\zeta$ is a random noise funtion. In this paper, rather than probing $\Aop^T$ with random noise functions, we use polynomial functions. Our method estimates the support of $\phi_p$ more accurately and reliably than resolution analysis, but our method requires that $\Aop$ has a positive integral kernel, while the resolution analysis method does not have this requirement.
...
%\end{equation*}
The finite element matrix representation of our approximation of $\Aop$ is therefore given by
\begin{equation}
